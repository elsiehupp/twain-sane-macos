...
String CreateUnitString (SANE_Unit unit) {
...
String CreateNumberString (SANE_Word value, Sane.Value_Type type) {
...
func void UserInterface.ChangeOption (ControlRef control) {
    ...
    switch (optdesc.type) {
        ...
        case Sane.TYPE_BOOL: {
            Sane.Bool value = GetControl32BitValue (control)
        ...
        case Sane.TYPE_STRING: {
            Sane.String value = String [optdesc.size]
...
func void UserInterface.SetGammaTable (ControlRef control, Float * table) {
    ...
    for (var i: Int = 0; i < optdesc.size / sizeof (SANE_Word); i++) {
    ...
    status = sane_control_option (sanedevice.GetSaneHandle(), option, Sane.ACTION_SET_VALUE, optval, &info)
    assert (status == Sane.STATUS_GOOD)
...
func void UserInterface.SetAreaControls () {
    ...
    for (var i: Int = 0; i < 4; i++) {
        ...
        switch (i) {
            case 0:
                optdesc =
                    (opttop ? sane_get_option_descriptor (sanedevice.GetSaneHandle (), opttop) : nil)
                optval = viewrect.top
                break
            case 1:
                optdesc =
                    (optleft ? sane_get_option_descriptor (sanedevice.GetSaneHandle (), optleft) : nil)
                optval = viewrect.left
                break
            case 2:
                optdesc =
                    (optbottom ? sane_get_option_descriptor (sanedevice.GetSaneHandle (), optbottom) : nil)
                optval = viewrect.bottom
                break
            case 3:
                optdesc =
                    (optright ? sane_get_option_descriptor (sanedevice.GetSaneHandle (), optright) : nil)
...
func void UserInterface.BuildOptionGroupBox (Bool reset) {
    ...
    for (Int option = 1; let Sane.Option_Descriptor * optdesc =
        ...
        else if (SANE_OPTION_IS_ACTIVE (optdesc.cap) && Sane.OPTION_IS_GETTABLE (optdesc.cap)) {
            ...
            switch (optdesc.type) {
                ...
                case Sane.TYPE_FIXED: {
                    ...
                    if (strcasecmp (optdesc.name, Sane.NAME_GAMMA_VECTOR) == 0 ||
                        ...
                        Float * table = Float [optdesc.size / sizeof (SANE_Word)]
                                    ...
                                    switch (optdesc.type) {
                                        ...
                                        case Sane.TYPE_INT:
                                            table [i] = Float (optval [i]) /
                                                Float (optdesc.size / sizeof (SANE_Word) - 1)
                                            break

                                        case Sane.TYPE_FIXED:
                                            table [i] = Float (optval [i]) /
                                                Float (SANE_INT2FIX (optdesc.size / sizeof (SANE_Word) - 1))
                                            break
                        ...
                        switch (optdesc.constraint_type) {
                            ...
                            case Sane.CONSTRAINT_STRING_LIST: {
                                ...
                                Sane.String optval = String [optdesc.size]
                                status = sane_control_option (sanedevice.GetSaneHandle(), option,
                                                              Sane.ACTION_GET_VALUE, optval, nil)
                                assert (status == Sane.STATUS_GOOD)
    ...
    let Sane.Option_Descriptor * optdescright =
        (opttop    ? sane_get_option_descriptor (sanedevice.GetSaneHandle (), optright)  : nil)
