...
class SaneDevice {
    ...
    private var saneversion: Sane.Int
    ...
    func sane_constrain_value (opt: Sane.Option_Descriptor, value: any, info: Sane.Word) -> Sane.Status
    ...
    public func ChangeDevice (device: Int) -> Int {
        ...
        while (optdesc = sane_get_option_descriptor (GetSaneHandle (), option)) {
    ...
    public func SaneError (status: Sane.Status) {
        ...
        text = CFStringCreateWithCString (nil, sane_strstatus (status), kCFStringEncodingUTF8)
        MakeStaticTextControl (rootcontrol, &controlrect, text, teFlushLeft, false)
        CFRelease (text)
                ...
                switch (optdesc.type) {
                    ...
                    case Sane.TYPE_INT:
                        ...
                        if (optdesc.size > sizeof (SANE_Word)) {
                            Sane.Word * optval = Sane.Word [optdesc.size / sizeof (SANE_Word)]
                            status = sane_control_option (GetSaneHandle (), option,
                                                        Sane.ACTION_GET_VALUE, optval, nil)
                            assert (status == Sane.STATUS_GOOD)
                            let cfarray: CFMutableArrayRef =
                                CFArrayCreateMutable (nil, 0, &kCFTypeArrayCallBacks)
                            for i in optdesc.size / sizeof (SANE_Word) {
                    ...
                    case Sane.TYPE_STRING: {
                        ...
                        let optval: Sane.String = String [optdesc.size]
                        status = sane_control_option (GetSaneHandle (), option,
                                                    Sane.ACTION_GET_VALUE, optval, nil)
                        assert (status == Sane.STATUS_GOOD)
                ...
                switch (optdesc.type) {
                    ...
                    case Sane.TYPE_BOOL: {
                        ...
                        if (cfvalue && CFGetTypeID (cfvalue) == CFBooleanGetTypeID ()) {
                            let optval: Sane.Bool = CFBooleanGetValue (cfvalue)
                            status = sane_control_option (GetSaneHandle (), option,
    ...
    public func GetRect (viewrect: Rect) {
        ...
        optdesc = (option ? sane_get_option_descriptor (GetSaneHandle (), option) : nil)
        if (optdesc && Sane.OPTION_IS_ACTIVE (optdesc.cap) && OPTION_IS_GETTABLE (optdesc.cap)) {
            status = sane_control_option (GetSaneHandle (), option, Sane.ACTION_GET_VALUE,
                                        &rect.top, nil)
            assert (status == Sane.STATUS_GOOD)
    ...
    public func SetRect (viewrect: Rect) {
        ...
        if (rect.top < oldrect.bottom) {
            ...
            if (optdesc && Sane.OPTION_IS_ACTIVE (optdesc.cap) && Sane.OPTION_IS_SETTABLE (optdesc.cap)) {
                status = sane_constrain_value (optdesc, &rect.top, nil)
                assert (status == Sane.STATUS_GOOD)
                status = sane_control_option (GetSaneHandle (), option, Sane.ACTION_SET_VALUE,
                                            &rect.top, &info)
                assert (status == Sane.STATUS_GOOD)
            ...
            optdesc = (option ? sane_get_option_descriptor (GetSaneHandle (), option) : nil)
            if (optdesc && Sane.OPTION_IS_ACTIVE (optdesc.cap) && Sane.OPTION_IS_SETTABLE (optdesc.cap)) {
                status = sane_constrain_value (optdesc, &rect.bottom, nil)
                assert (status == Sane.STATUS_GOOD)
                status = sane_control_option (GetSaneHandle (), option, Sane.ACTION_SET_VALUE,
                                            &rect.bottom, &info)
                assert (status == Sane.STATUS_GOOD)
        ...
        if (rect.left < oldrect.right) {
            ...
            optdesc = (option ? sane_get_option_descriptor (GetSaneHandle (), option) : nil)
            if (optdesc && Sane.OPTION_IS_ACTIVE (optdesc.cap) && Sane.OPTION_IS_SETTABLE (optdesc.cap)) {
                status = sane_constrain_value (optdesc, &rect.left, nil)
                assert (status == Sane.STATUS_GOOD)
                status = sane_control_option (GetSaneHandle (), option, Sane.ACTION_SET_VALUE,
                                            &rect.left, &info)
                assert (status == Sane.STATUS_GOOD)
            ...
            optdesc = (option ? sane_get_option_descriptor (GetSaneHandle (), option) : nil)
            if (optdesc && Sane.OPTION_IS_ACTIVE (optdesc.cap) && Sane.OPTION_IS_SETTABLE (optdesc.cap)) {
                status = sane_constrain_value (optdesc, &rect.right, nil)
                assert (status == Sane.STATUS_GOOD)
                status = sane_control_option (GetSaneHandle (), option, Sane.ACTION_SET_VALUE,
                                            &rect.right, &info)
                assert (status == Sane.STATUS_GOOD)
    ...
    public func SetResolution (res: Resolution) {
        ...
        optdesc = (option ? sane_get_option_descriptor (GetSaneHandle (), option) : nil)
        if (optdesc && Sane.OPTION_IS_ACTIVE (optdesc.cap) && Sane.OPTION_IS_SETTABLE (optdesc.cap)) {
            status = sane_constrain_value (optdesc, &res.h, nil)
            assert (status == Sane.STATUS_GOOD)
            status = sane_control_option (GetSaneHandle (), option, Sane.ACTION_SET_VALUE,
                                        &res.h, &info)
            assert (status == Sane.STATUS_GOOD)
        ...
        optdesc = (option ? sane_get_option_descriptor (GetSaneHandle (), option) : nil)
        if (optdesc && Sane.OPTION_IS_ACTIVE (optdesc.cap) && Sane.OPTION_IS_SETTABLE (optdesc.cap)) {
            status = sane_constrain_value (optdesc, &res.v, nil)
            assert (status == Sane.STATUS_GOOD)
            status = sane_control_option (GetSaneHandle (), option, Sane.ACTION_SET_VALUE,
                                        &res.v, &info)
            assert (status == Sane.STATUS_GOOD)
    ...
    public func GetPixelType (capability: pTW_CAPABILITY, onlyone: Bool) -> TW_UINT16 {
        ...
        var option: Int = optionIndex [SANE_NAME_SCAN_MODE]
        if (!option) {
            return datasource.SetStatus (TWCC_CAPUNSUPPORTED)
        }

        var optdesc: Sane.Option_Descriptor = sane_get_option_descriptor (GetSaneHandle (), option)
        ...
        var optval: Sane.String = String [optdesc.size]
        var status: Sane.Status = sane_control_option (GetSaneHandle (), option, Sane.ACTION_GET_VALUE, optval, nil)
        assert (status == Sane.STATUS_GOOD)
    ...
    public func SetPixelType (capability: pTW_CAPABILITY) -> TW_UINT16 {
        ...
        while optdesc.constraint.string_list[i] {
            ...
            if ((pixeltype == TWPT_BW && (strncasecmp (optdesc.constraint.string_list [i], "binary", 6) == 0 ||
                ...
                var status: Sane.Status = sane_control_option (GetSaneHandle (), option, Sane.ACTION_SET_VALUE,
                                                        optval, nil)
                assert (status == Sane.STATUS_GOOD)
                // delete[] optval
    ...
    public func GetBitDepth (capability: pTW_CAPABILITY, onlyone: Bool) -> TW_UINT16 {
        ...
        var option: Int = optionIndex [SANE_NAME_BIT_DEPTH]
        if (!option) {
            return datasource.SetStatus (TWCC_CAPUNSUPPORTED)
        }
        ...
        var bitdepth: Sane.Int
        var status: Sane.Status = sane_control_option (GetSaneHandle (), option, Sane.ACTION_GET_VALUE, &bitdepth, nil)
        assert (status == Sane.STATUS_GOOD)
    ...
    public func SetBitDepth (capability: pTW_CAPABILITY) -> TW_UINT16 {
        ...
        var info: Sane.Int
        var status: Sane.Status = sane_control_option (GetSaneHandle (), option, Sane.ACTION_SET_VALUE, &bitdepth, &info)
        assert (status == Sane.STATUS_GOOD)
    ...
    public func GetXNativeResolution (capability: pTW_CAPABILITY) -> TW_UINT16 {
        ...
        var option: Int = optionIndex [SANE_NAME_SCAN_X_RESOLUTION]
        if (!option) {
            option = optionIndex [SANE_NAME_SCAN_RESOLUTION]
        }
        if (!option) {
        ...
        switch (optdesc.constraint_type) {
            ...
            case Sane.CONSTRAINT_WORD_LIST: {
                if (optdesc.type == Sane.TYPE_INT) {
                    return datasource.BuildOneValue (capability, TWTY_FIX32,
                                                    S2T (SANE_INT2FIX (optdesc.constraint.word_list[optdesc.constraint.word_list [0]])))
                } else {
                    return datasource.BuildOneValue (capability, TWTY_FIX32,
            ...
            default:
                ...
                var xres: Sane.Word
                var status: Sane.Status = sane_control_option (GetSaneHandle (), option, Sane.ACTION_GET_VALUE,
                                                        &xres, nil)
                assert (status == Sane.STATUS_GOOD)
    ...
    public func GetYNativeResolution (capability: pTW_CAPABILITY) -> TW_UINT16 {
        ...
        var option: Int = optionIndex [SANE_NAME_SCAN_Y_RESOLUTION]
        if (!option) {
    ...
    public func GetXResolution (capability: pTW_CAPABILITY, onlyone: Bool) -> TW_UINT16 {
        ...
        var xres: Sane.Word
        var status: Sane.Status = sane_control_option (GetSaneHandle (), option, Sane.ACTION_GET_VALUE, &xres, nil)
        assert (status == Sane.STATUS_GOOD)
    ...
    public func SetXResolution (capability: pTW_CAPABILITY) -> TW_UINT16 {
        ...
        var info: Sane.Int
        var status: Sane.Status = sane_control_option (GetSaneHandle (), option, Sane.ACTION_SET_VALUE, &xres, &info)
        assert (status == Sane.STATUS_GOOD)
        return (info ? TWRC_CHECKSTATUS : TWRC_SUCCESS)
    }
    ...
    public func GetYResolution (capability: pTW_CAPABILITY, onlyone: Bool) -> TW_UINT16 {
        ...
        var yres: Sane.Word
        var status: Sane.Status = sane_control_option (GetSaneHandle (), option, Sane.ACTION_GET_VALUE, &yres, nil)
        assert (status == Sane.STATUS_GOOD)
    ...
    public func SetYResolution (capability: pTW_CAPABILITY) -> TW_UINT16 {
        ...
        var info: Sane.Int
        var status: Sane.Status = sane_control_option (GetSaneHandle (), option, Sane.ACTION_SET_VALUE, &yres, &info)
        assert (status == Sane.STATUS_GOOD)
        return (info ? TWRC_CHECKSTATUS : TWRC_SUCCESS)
    }
    ...
    public func GetPhysicalWidth (capability: pTW_CAPABILITY) -> TW_UINT16 {
        ...
        let loption: Int = optionIndex [SANE_NAME_SCAN_TL_X]
        let roption: Int = optionIndex [SANE_NAME_SCAN_BR_X]
        if (!loption || !roption) {
            return datasource.SetStatus (TWCC_CAPUNSUPPORTED)
        }

        let loptdesc: Sane.Option_Descriptor = sane_get_option_descriptor (GetSaneHandle (), loption)
        let roptdesc: Sane.Option_Descriptor = sane_get_option_descriptor (GetSaneHandle (), roption)
    ...
    public func GetPhysicalHeight (capability: pTW_CAPABILITY) -> TW_UINT16 {
        ...
        let toption: Int = optionIndex [SANE_NAME_SCAN_TL_Y]
        let boption: Int = optionIndex [SANE_NAME_SCAN_BR_Y]
        if (!toption || !boption) {
            return datasource.SetStatus (TWCC_CAPUNSUPPORTED)
        }

        let toptdesc: Sane.Option_Descriptor = sane_get_option_descriptor (GetSaneHandle (), toption)
        let boptdesc: Sane.Option_Descriptor = sane_get_option_descriptor (GetSaneHandle (), boption)
    ...
    public func GetBrightness (capability: pTW_CAPABILITY, onlyone: Bool) -> TW_UINT16 {
        ...
        var option: Int = optionIndex [SANE_NAME_BRIGHTNESS]
        if (!option) {
            return datasource.SetStatus (TWCC_CAPUNSUPPORTED)
        }
        ...
        var status: Sane.Status = sane_control_option (GetSaneHandle (), option, Sane.ACTION_GET_VALUE, &brightness, nil)
        assert (status == Sane.STATUS_GOOD)
    ...
    public func SetBrightness (capability: pTW_CAPABILITY) -> TW_UINT16 {
        ...
        var info: Sane.Int
        var status: Sane.Status = sane_control_option (GetSaneHandle (), option, Sane.ACTION_SET_VALUE, &brightness, &info)
        assert (status == Sane.STATUS_GOOD)
        return (info ? TWRC_CHECKSTATUS : TWRC_SUCCESS)
    }
    ...
    public func GetContrast (capability: pTW_CAPABILITY, onlyone: Bool) -> TW_UINT16 {
        ...
        var option: Int = optionIndex [SANE_NAME_CONTRAST]
        if (!option) {
            return datasource.SetStatus (TWCC_CAPUNSUPPORTED)
        }
        ...
        var status: Sane.Status = sane_control_option (GetSaneHandle (), option, Sane.ACTION_GET_VALUE, &contrast, nil)
        assert (status == Sane.STATUS_GOOD)
    ...
    public func SetContrast (capability: pTW_CAPABILITY) -> TW_UINT16 {
        ...
        var info: Sane.Int
        var status: Sane.Status = sane_control_option (GetSaneHandle (), option, Sane.ACTION_SET_VALUE, &contrast, &info)
        assert (status == Sane.STATUS_GOOD)
        return (info ? TWRC_CHECKSTATUS : TWRC_SUCCESS)
    }
    ...
    public func SetLayout (imagelayout: pTW_IMAGELAYOUT) -> TW_UINT16 {
        ...
        var option: Int = optionIndex [SANE_NAME_SCAN_TL_Y]
        if (!option) {
            return datasource.SetStatus (TWCC_CAPUNSUPPORTED)
        }
    ...
    public func SetPreview (preview: Sane.Bool) {

        var option: Int = optionIndex [SANE_NAME_PREVIEW]
        if (!option) {
            return
        }
        ...
        if (optdesc && optdesc.type == Sane.TYPE_BOOL &&
            Sane.OPTION_IS_ACTIVE (optdesc.cap) && Sane.OPTION_IS_SETTABLE (optdesc.cap)) {
            var status: Sane.Status = sane_control_option (GetSaneHandle (), option,
    ...
    public func GetSaneVersion () -> Sane.Int {
        return saneversion
    }
    ...
    public func GetAreaOptions (top: Int = nil, left: Int = nil, bottom: Int = nil, right: Int = nil) {
        if (top) {
            *top    = optionIndex [SANE_NAME_SCAN_TL_Y]
        }
        if (left) {
            *left   = optionIndex [SANE_NAME_SCAN_TL_X]
        }
        if (bottom) {
            *bottom = optionIndex [SANE_NAME_SCAN_BR_Y]
        }
        if (right) {
            *right  = optionIndex [SANE_NAME_SCAN_BR_X]
        }
    }
}