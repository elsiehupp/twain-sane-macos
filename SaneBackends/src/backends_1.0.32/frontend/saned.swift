...
func Int poll(struct pollfd *ufds, unsigned Int nfds, Int timeout)
...
import avahi-client/publish

import avahi-common/alternative
...
typedef struct
{
  ...
  Sane.Handle handle;		/* backends handle */
}
Handle

static Sane.Net_Procedure_Number current_request
...
static Bool log_to_syslog = Sane.TRUE
...
static void
auth_callback(Sane.String_Const res,
	       Sane.Char *username,
	       Sane.Char *password)
{
  Sane.Net_Procedure_Number procnum
  Sane.Authorization_Req req
  Sane.Word word, ack = 0
...
check_host(Int fd)
{
  ...
  if(IN6_IS_ADDR_V4MAPPED((struct in6_addr *)sin6->sin6_addr.s6_addr))
      ...
      if(err)
	{
	  ...
	  IPv4map = Sane.FALSE; /* we failed, remote_ipv4_addr points to nothing */
	}
      else
	{
  ...
  if(err)
      ...
      for(resp = res; resp != NULL; resp = resp.ai_next)
	{
	  ...
	  if((strcasecmp(text_addr, remote_ip) == 0) ||
	      ((IPv4map == Sane.TRUE) && (strcmp(text_addr, remote_ipv4) == 0)))
#else
...
#ifdef ENABLE_IPV6
	  else if((IPv4map == Sane.TRUE) && (strcmp(config_line, remote_ipv4) == 0))
	    {
	      access_ok = 1
	      DBG(DBG_DBG,
	  ...
	  /* handle IP ranges, take care of the IPv4map stuff */
	      ...
	      else /* is a v4 address */
		{
		  ...
		  if((SS_FAMILY(remote_address.ss) == AF_INET) ||
			...
			{
			  DBG(DBG_DBG, "check_host: access granted from IP address %s(in subnet %s/%s)\n",
			       ((IPv4map == Sane.TRUE) ? remote_ipv4 : remote_ip), config_line, netmask)
			  access_ok = 1
			}
		      else
			{
...
init(Wire * w)
{
  Sane.Word word, be_version_code
  ...
  if(w.status || word != Sane.NET_INIT)
    {
  ...
  reply.version_code = Sane.VERSION_CODE(V_MAJOR, V_MINOR,
  ...
  if(status == Sane.STATUS_GOOD)
    {
      status = Sane.init(&be_version_code, auth_callback)
      if(status != Sane.STATUS_GOOD)
	DBG(DBG_ERR, "init: failed to initialize backend(%s)\n",
	     Sane.strstatus(status))

      if(Sane.VERSION_MAJOR(be_version_code) != V_MAJOR)
	{
	  DBG(DBG_ERR,
	       "init: unexpected backend major version %d(expected %d)\n",
	       Sane.VERSION_MAJOR(be_version_code), V_MAJOR)
  ...
  if(w.status || status != Sane.STATUS_GOOD)
    return -1
...
#ifdef SANED_USES_AF_INDEP
static Int
start_scan(Wire * w, Int h, Sane.Start_Reply * reply)
{
  union {
    struct sockaddr_storage ss
    struct sockaddr sa
  ...
  if(getsockname(w.io.fd, &data_addr.sa, (socklen_t *) &len) < 0)
    {
      DBG(DBG_ERR, "start_scan: failed to obtain socket address(%s)\n",
	   strerror(errno))
      reply.status = Sane.STATUS_IO_ERROR
      return -1
    }
  ...
  reply.status = Sane.start(be_handle)
...
static Int
store_reclen(Sane.Byte * buf, size_t buf_size, var i: Int, size_t reclen)
{
...
do_scan(Wire * w, Int h, Int data_fd)
{
  ...
  Sane.Handle be_handle = handle[h].handle
  ...
  Sane.set_io_mode(be_handle, Sane.TRUE)
  if(Sane.get_select_fd(be_handle, &be_fd) == Sane.STATUS_GOOD)
    {
  ...
  do
	       ...
	       && (timeout || FD_ISSET(be_fd, &rd_set)))
	{
	  ...
	  status = Sane.read(be_handle, buf + reader, nbytes, &length)
	  DBG(DBG_INFO,
  ...
  while(status == Sane.STATUS_GOOD || bytes_in_buf > 0 || status_dirty)
  DBG(DBG_MSG, "do_scan: done, status=%s\n", Sane.strstatus(status))

  if(handle[h].docancel)
    Sane.cancel(handle[h].handle)
...
process_request(Wire * w)
{
  ...
  switch(current_request)
    {
    case Sane.NET_GET_DEVICES:
      {
	Sane.Get_Devices_Reply reply

	reply.status =
	  Sane.get_devices((const Sane.Device ***) &reply.device_list,
			    Sane.TRUE)
	...
	if(!name)
	  {
	    ...
	    reply.status = Sane.STATUS_INVAL
	...
	if(strlen(resource) == 0) {
	  ...
	  if((i = Sane.get_devices(&device_list, Sane.TRUE)) !=
	      Sane.STATUS_GOOD)
	    {
	      DBG(DBG_ERR, "process_request: (open) Sane.get_devices failed\n")
	      memset(&reply, 0, sizeof(reply))
	      reply.status = i
	      sanei_w_reply(w, (WireCodecFunc) sanei_w_open_reply, &reply)
	      break
	...
	if(sanei_authorize(resource, "saned", auth_callback) !=
	    Sane.STATUS_GOOD)
	  {
	    ...
	    reply.status = Sane.STATUS_ACCESS_DENIED
	  }
	else
	  {
	    ...
	    free(resource)
	    memset(&reply, 0, sizeof(reply));	/* avoid leaking bits */
	    reply.status = Sane.open(name, &be_handle)
	    DBG(DBG_MSG, "process_request: Sane.open returned: %s\n",
	...
	if(reply.status == Sane.STATUS_GOOD)
	  {
	    h = get_free_handle()
	    if(h < 0)
	      reply.status = Sane.STATUS_NO_MEM
	    else
	      {
    ...
    case Sane.NET_GET_OPTION_DESCRIPTORS:
      {
	...
	if(h < 0)
	  return 1
	be_handle = handle[h].handle
	Sane.control_option(be_handle, 0, Sane.ACTION_GET_VALUE,
			     &opt.num_options, 0)

	opt.desc = malloc(opt.num_options * sizeof(opt.desc[0]))
	for(i = 0; i < opt.num_options; ++i)
	  opt.desc[i] = (Sane.Option_Descriptor *)
	    Sane.get_option_descriptor(be_handle, i)
	...
	if(w.status || (unsigned) req.handle >= (unsigned) num_handles
	    || !handle[req.handle].inuse)
	  {
        ...
        if(w.direction == WIRE_DECODE
            && req.value_type == Sane.TYPE_STRING
            && req.action     == Sane.ACTION_GET_VALUE)
          {
            if(req.value)
              {
	...
	reply.status = Sane.control_option(be_handle, req.option,
	...
	reply.status = Sane.get_parameters(be_handle, &reply.params)
	...
	memset(&reply, 0, sizeof(reply));	/* avoid leaking bits */
	reply.byte_order = Sane.NET_LITTLE_ENDIAN
	if(byte_order.w != 1)
	  reply.byte_order = Sane.NET_BIG_ENDIAN

	if(handle[h].scanning)
	  reply.status = Sane.STATUS_DEVICE_BUSY
	else
...
do_bindings_family(Int family, Int *nfds, struct pollfd **fds, struct addrinfo *res)
{
  ...
  for(resp = res, i = 0; resp != NULL; resp = resp.ai_next, i++)
	...
	{
          if(Sane.port != -1)
	      ((struct sockaddr_in *) resp.ai_addr)->sin_port = htons(Sane.port)
          else
	      Sane.port = ntohs(((struct sockaddr_in *) resp.ai_addr)->sin_port)
	}
#ifdef ENABLE_IPV6
      else if(resp.ai_family == AF_INET6)
	{
          if(Sane.port != -1)
              ((struct sockaddr_in6 *) resp.ai_addr)->sin6_port = htons(Sane.port)
          else
              Sane.port = ntohs(((struct sockaddr_in6 *) resp.ai_addr)->sin6_port)
	}
#endif /* ENABLE_IPV6 */
      else
	continue
...
static void
run_inetd(char __Sane.unused__ *sock)
{
