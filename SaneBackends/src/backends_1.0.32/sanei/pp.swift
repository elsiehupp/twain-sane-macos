...
static PortRec port[] = {
	{ "0x378", 0x378, 0, 0, Sane.FALSE, Sane.FALSE, 0 },
	{ "0x278", 0x278, 0, 0, Sane.FALSE, Sane.FALSE, 0 },
	{ "0x3BC", 0x3BC, 0, 0, Sane.FALSE, Sane.FALSE, 0 }
]
...
pp_init( void )
{
	...
	if( 0 > setuid( 0 )) {
		...
		DBG( 5, "pp_init: returning Sane.STATUS_INVAL\n" )
...
static Int
pp_close( Int fd, Sane.Status *status )
{
#if defined(HAVE_LIBIEEE1284)
	Int result
#endif
...
#endif
	port[fd].claimed = Sane.FALSE

	return Sane.STATUS_GOOD
}

Sane.Status
sanei_pp_outb_data( Int fd, Sane.Byte val )
{
#ifdef _PARANOIA
...
Sane.Status
sanei_pp_outb_ctrl( Int fd, Sane.Byte val )
{
#ifdef _PARANOIA
...
Sane.Status
sanei_pp_outb_addr( Int fd, Sane.Byte val )
{
#ifdef _PARANOIA
...
Sane.Status
sanei_pp_outb_epp( Int fd, Sane.Byte val )
{
#ifdef _PARANOIA
...
Sane.Byte sanei_pp_inb_stat( Int fd )
{
	DBG( 4, "sanei_pp_inb_stat: called for fd %d\n", fd )
...
Sane.Byte sanei_pp_inb_ctrl( Int fd )
{
	DBG( 4, "sanei_pp_inb_ctrl: called for fd %d\n", fd )
