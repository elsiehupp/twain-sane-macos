...
static void
print_buffer (const Sane.Byte * buffer, Int size)
{
...
#if WITH_USB_RECORD_REPLAY
Sane.Status sanei_usb_testing_enable_replay(Sane.String_Const path,
...
Sane.Status sanei_usb_testing_enable_record(Sane.String_Const path, Sane.String_Const be_name)
{
...
String sanei_usb_testing_get_backend()
{
...
Bool sanei_usb_is_replay_mode_enabled()
{
...
static void sanei_usb_record_debug_msg(xmlNode* node, Sane.String_Const message)
{
...
static void sanei_usb_record_replace_debug_msg(xmlNode* node, Sane.String_Const message)
{
...
static void sanei_usb_replay_debug_msg(Sane.String_Const message)
{
...
public void sanei_usb_testing_record_message(Sane.String_Const message)
{
  if (testing_mode == sanei_usb_testing_mode_record)
    {
      sanei_usb_record_debug_msg(NULL, message)
...
static Sane.Status sanei_usb_testing_init()
{
  DBG_INIT()
  ...
  if (device_number != 0)
    return Sane.STATUS_INVAL; // already opened
...
void sanei_usb_testing_record_message(Sane.String_Const message)
{
  (void) message
}
#endif // WITH_USB_RECORD_REPLAY
  ...
  if (testing_mode != sanei_usb_testing_mode_disabled)
    {
      if (initialized == 0)
        {
          if (sanei_usb_testing_init() != Sane.STATUS_GOOD)
            {
...
static void kernel_scan_devices(void)
{
  ...
  for (prefix = prefixlist; *prefix; prefix += 2)
    {
      String dir_name = *prefix
      String base_name = *(prefix + 1)
				   ...
				   Sane.Status (*attach) (const char *dev))
{
  ...
  if (strncmp (name, "usb", 3) == 0)
    {
      Sane.Word vendorID = 0, productID = 0
...
sanei_usb_get_vendor_product (Int dn, Sane.Word * vendor,
			      Sane.Word * product)
{
  Sane.Word vendorID = 0
  Sane.Word productID = 0
...
sanei_usb_find_devices (Int vendor, Int product,
			Sane.Status (*attach) (Sane.String_Const dev))
{
  Int dn = 0
...
void
sanei_usb_set_endpoint (Int dn, Int ep_type, Int ep)
{
  if (dn >= device_number || dn < 0)
    {
...
static const char* sanei_usb_transfer_type_desc(Int transfer_type)
{
...
Int
sanei_usb_get_endpoint (Int dn, Int ep_type)
{
  if (dn >= device_number || dn < 0)
    {
...
static void sanei_usb_record_open(Int dn)
{
...
Sane.Status
sanei_usb_open (Sane.String_Const devname, Int * dn)
{
  Int devcount
  Bool found = Sane.FALSE
...
void
sanei_usb_close (Int dn)
{
  char *env
  Int workaround = 0

  DBG (5, "sanei_usb_close: evaluating environment variable Sane.USB_WORKAROUND\n")
  env = getenv ("Sane.USB_WORKAROUND")
  if (env)
    {
      workaround = atoi(env)
...
#endif
  devices[dn].open = Sane.FALSE
  return
}

void
sanei_usb_set_timeout (Int __Sane.unused__ timeout)
{
  if (testing_mode == sanei_usb_testing_mode_replay)
    return
...
Sane.Status
sanei_usb_clear_halt (Int dn)
{
  char *env
  Int workaround = 0

  DBG (5, "sanei_usb_clear_halt: evaluating environment variable Sane.USB_WORKAROUND\n")
  env = getenv ("Sane.USB_WORKAROUND")
  if (env)
    {
      workaround = atoi(env)
...
Sane.Status
sanei_usb_reset (Int __Sane.unused__ dn)
{
  if (testing_mode == sanei_usb_testing_mode_replay)
    return Sane.STATUS_GOOD
...
static Int sanei_usb_replay_next_read_bulk_packet_size(Int dn)
{
...
static void sanei_usb_record_read_bulk(xmlNode* node, Int dn,
                                       Sane.Byte* buffer,
...
static void sanei_usb_record_replace_read_bulk(xmlNode* node, Int dn,
                                               Sane.Byte* buffer,
...
static Int sanei_usb_replay_read_bulk(Int dn, Sane.Byte* buffer,
                                      size_t size)
{
...
#if WITH_USB_RECORD_REPLAY
static Int sanei_usb_record_write_bulk(xmlNode* node, Int dn,
                                       const Sane.Byte* buffer,
...
static void sanei_usb_record_replace_write_bulk(xmlNode* node, Int dn,
                                                const Sane.Byte* buffer,
                                                size_t size, size_t write_size)
{
  if (!testing_development_mode)
...
// returns non-negative value on success, -1 on failure
static Int sanei_usb_replay_next_write_bulk_packet_size(Int dn)
{
  xmlNode* node = sanei_xml_peek_next_tx_node()
  if (node == NULL)
    return -1
...
static Int sanei_usb_replay_write_bulk(Int dn, const Sane.Byte* buffer,
                                       size_t size)
{
  size_t wanted_size = size
...
sanei_usb_record_control_msg(xmlNode* node,
                             Int dn, Int rtype, Int req,
                             Int value, Int index, Int len,
                             const Sane.Byte* data)
{
  (void) dn
...
static Sane.Status
sanei_usb_replay_control_msg(Int dn, Int rtype, Int req,
                             Int value, Int index, Int len,
                             Sane.Byte* data)
{
  (void) dn
  ...
  Int direction_is_in = (rtype & 0x80) == 0x80
  Sane.Byte* rdata = direction_is_in ? NULL : data
...
static Int sanei_usb_replay_read_int(Int dn, Sane.Byte* buffer,
                                     size_t size)
{
  if (testing_known_commands_input_failed)
    return -1
...
#if WITH_USB_RECORD_REPLAY
static Sane.Status sanei_usb_replay_set_configuration(Int dn,
                                                      Int configuration)
{
  (void) dn
...
static void sanei_usb_record_set_configuration(Int dn,
                                               Int configuration)
{
...
static Sane.Status
sanei_usb_replay_get_descriptor(Int dn,
...
static void
sanei_usb_record_get_descriptor(Int dn,
                                struct sanei_usb_dev_descriptor *desc)
{
  (void) dn
...
public Sane.Status
sanei_usb_get_descriptor( Int dn,
                          struct sanei_usb_dev_descriptor __Sane.unused__
                          *desc )
{
  if (dn >= device_number || dn < 0)
    {
