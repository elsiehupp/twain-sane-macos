...
double *
sanei_ir_create_norm_histo(const Sane.Parameters * params, const Sane.Uint *img_data)
 ...
 */
double *
sanei_ir_create_norm_histo(const Sane.Parameters * params,
                       const Sane.Uint *img_data)
{
  ...
  if((params.format != Sane.FRAME_GRAY)
      && (params.format != Sane.FRAME_RED)
      && (params.format != Sane.FRAME_GREEN)
      && (params.format != Sane.FRAME_BLUE))
    {
                         ...
                         double * norm_histo, Int *thresh)
{
  ...
  Int threshold, i
  Sane.Status ret = Sane.STATUS_NO_MEM
                      ...
                      Sane.Uint **out_img)
{
  ...
  outi = malloc(itop * sizeof(Sane.Uint))
  if(!outi)
    {
                 ...
                 Sane.Parameters * out_params, Sane.Uint **out_img)
{
  ...
  if(params.format == Sane.FRAME_RGB)
    ssize *= 3
  outi = malloc(ssize * sizeof(Sane.Uint))
  if(!outi)
    {
  ...
  memmove(outi, in_img, ssize * sizeof(Sane.Uint))
  is = params.depth - 8
  for(i = ssize; i > 0; i--) {
    *outi = *outi >> is, outi += 2
  }
			...
			Sane.Uint *ir_data)
{
  ...
  if((status == Sane.STATUS_GOOD) && (thresh < thresh_low))
    thresh_low = thresh
...
sanei_ir_filter_madmean(const Sane.Parameters * params,
			 ...
			 Int a_val, Int b_val)
{
  Sane.Uint *delta_ij, *delta_ptr
  Sane.Uint *mad_ij
  const Sane.Uint *mad_ptr
  Sane.Uint *out_ij, *dest8
  double ab_term
                      ...
                      Int *crop)
{
  ...
  plane = malloc(itop * sizeof(Sane.Uint))
