...
import Sane.sanei_codec_ascii
import Sane.sanei_codec_bin

static Wire w

static Sane.Word dpi_word_list[] = {
  4,				/* # of elements */
  3, 30, 300, -600
]
...
func Int main(Int __Sane.unused__ arg, char **argv)
{
  Sane.Option_Descriptor desc[2], *desc_ptr
  Sane.Word len
  ...
  desc[0].type = Sane.TYPE_FIXED
  desc[0].unit = Sane.UNIT_DPI
  desc[0].size = sizeof(Sane.Word)
  desc[0].cap = Sane.CAP_SOFT_SELECT | Sane.CAP_SOFT_DETECT
  desc[0].constraint_type = Sane.CONSTRAINT_WORD_LIST
  desc[0].constraint.word_list = dpi_word_list

  desc[1].name = "mode"
  desc[1].title = "Scan Mode"
  desc[1].desc = "Determines scan mode."
  desc[1].type = Sane.TYPE_STRING
  desc[1].unit = Sane.UNIT_NONE
  desc[1].size = 10
  desc[1].cap = (Sane.CAP_SOFT_SELECT | Sane.CAP_SOFT_DETECT
		 | Sane.CAP_AUTOMATIC)
  desc[1].constraint_type = Sane.CONSTRAINT_STRING_LIST
