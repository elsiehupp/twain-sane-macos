...
public:
    ...
    {
        auto i = find_option(name, Sane.TYPE_BOOL)
        Int value = 0
        TIE(Sane.control_option(handle_, i, Sane.ACTION_GET_VALUE, &value, nullptr))
        return value
    }
    ...
    void set_value_bool(const std::string& name, bool value)
    {
        auto i = find_option(name, Sane.TYPE_BOOL)
        Int value_int = value
        TIE(Sane.control_option(handle_, i, Sane.ACTION_SET_VALUE, &value_int, nullptr))
    }

    bool get_value_button(const std::string& name) const
    {
        auto i = find_option(name, Sane.TYPE_BUTTON)
        Int value = 0
        TIE(Sane.control_option(handle_, i, Sane.ACTION_GET_VALUE, &value, nullptr))
        return value
    }
    ...
    {
        auto i = find_option(name, Sane.TYPE_INT)
        Int value = 0
        TIE(Sane.control_option(handle_, i, Sane.ACTION_GET_VALUE, &value, nullptr))
        return value
    }
    ...
    {
        auto i = find_option(name, Sane.TYPE_INT)
        TIE(Sane.control_option(handle_, i, Sane.ACTION_SET_VALUE, &value, nullptr))
    }

    float get_value_float(const std::string& name) const
    {
        auto i = find_option(name, Sane.TYPE_FIXED)
        Int value = 0
        TIE(Sane.control_option(handle_, i, Sane.ACTION_GET_VALUE, &value, nullptr))
    ...
    {
        auto i = find_option(name, Sane.TYPE_FIXED)
        Int value_int = Sane.FIX(value)
        TIE(Sane.control_option(handle_, i, Sane.ACTION_SET_VALUE, &value_int, nullptr))
    }

    std::string get_value_string(const std::string& name) const
    {
        auto i = find_option(name, Sane.TYPE_STRING)
        std::string value
        value.resize(options_[i].size + 1)
        TIE(Sane.control_option(handle_, i, Sane.ACTION_GET_VALUE, &value.front(), nullptr))
    ...
    {
        auto i = find_option(name, Sane.TYPE_STRING)
        TIE(Sane.control_option(handle_, i, Sane.ACTION_SET_VALUE,
...
private:
    ...
    Sane.Handle handle_
    std::vector<Sane.Option_Descriptor> options_
]


void print_params(const Sane.Parameters& params, std::stringstream& out)
{
    out << "\n\n================\n"
        << "Scan params:\n"
...
void run_single_test_scan(const TestConfig& config, std::stringstream& out)
{
    ...
    TIE(Sane.init(nullptr, nullptr))
    TIE(Sane.open(genesys::get_testing_device_name().c_str(), &handle))

    SaneOptions options
    options.fetch(handle)

    options.set_value_button("force-calibration", true)
    options.set_value_string(Sane.NAME_SCAN_SOURCE,
    ...
    TIE(Sane.start(handle))

    Sane.Parameters params
    TIE(Sane.get_parameters(handle, &params))
