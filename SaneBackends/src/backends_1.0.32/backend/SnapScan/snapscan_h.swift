...
static Sane.Word usb_vendor_ids[] =
{
    /* USB Vendor IDs */
...
{
    Sane.Device dev
    Sane.Range x_range;           /* x dimension of scan area */
    Sane.Range y_range;           /* y dimension of scan area */
    SnapScan_Model model;         /* type of scanner */
    SnapScan_Bus bus;             /* bus of the device usb/scsi */
    Sane.Char *firmware_filename; /* The name of the firmware file for USB scanners */
...
import snapscan-sources

struct snapscan_scanner
{
    String devname;          /* the scsi device name */
    ...
    Sane.Pid child;               /* child reader process pid */
    ...
    Bool nonblocking;        /* wait on reads for data? */
    ...
    Sane.Byte chroma_offset[3];   /* chroma offsets */
    Int chroma
    Source *psrc;                 /* data source */
    Source *preadersrc;                 /* data source for reader thread */
    Sane.Option_Descriptor options[NUM_OPTS];  /* the option descriptors */
    Option_Value val[NUM_OPTS];  /* the options themselves... */
    Int res;                /* resolution */
    Int bpp;                /* bit depth supported by scanner*/
    Int bpp_scan;           /* bit depth used for scanning */
    Bool preview;           /* preview mode toggle */
    Bool highquality;       /* high quality mode toggle */
    String mode_s;          /* scanning mode */
    String source_s;        /* scanning source */
    String preview_mode_s;  /* scanning mode for preview */
    Sane.Fixed tlx;              /* window top left x */
    Sane.Fixed tly;              /* window top left y */
    Sane.Fixed brx;              /* window bottom right x */
    Sane.Fixed bry;              /* window bottom right y */
    ...
    Sane.Byte frame_no;          /* frame number for film scanner */
    Int focus_mode;         /* focus mode value */
    String focus_mode_s;    /* focus mode string */
    Sane.Word focus;             /* focus point */
]
