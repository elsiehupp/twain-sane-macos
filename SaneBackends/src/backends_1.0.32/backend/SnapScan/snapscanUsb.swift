...
static Sane.Status usb_request_sense(SnapScan_Scanner *pss)
...
static Sane.Status atomic_usb_cmd(Int fd, const void *src, size_t src_size,
                    void *dst, size_t * dst_size)
{
...
static Sane.Status snapscani_usb_open(const char *dev, Int *fdp,
...
#define RETURN_ON_FAILURE(x) if((status = x) != Sane.STATUS_GOOD) return status

static Sane.Status usb_write(Int fd, const void *buf, size_t n) {
    char dbgmsg[16384]
    Sane.Status status
...
static Sane.Status usb_read(Int fd, void *buf, size_t n) {
    char dbgmsg[16384]
...
static Sane.Status usb_read_status(Int fd, Int *scsistatus, Int *transaction_status,
                                   char command)
{
...
static Sane.Status usb_cmd(Int fd, const void *src, size_t src_size,
                    void *dst, size_t * dst_size)
{
  ...
  */
  ...
  if(datalen > 0 && (tstatus == TRANSACTION_WRITE)) {
      ...
      RETURN_ON_FAILURE( usb_write(fd, ((const Sane.Byte *) src) + cmdlen, datalen) )
...
static Sane.Status snapscani_usb_shm_init(void)
{
 ...
 * Only define union semun if not already defined in <sys/sem. Fixes
