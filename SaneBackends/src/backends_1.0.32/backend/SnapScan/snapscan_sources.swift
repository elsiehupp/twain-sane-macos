...
{
    SOURCE_GUTS
    Int scsi_buf_pos;    /* current position in scsi buffer */
    Int scsi_buf_max;    /* data limit */
...
{
    TX_SOURCE_GUTS
    Sane.Byte *ch_buf;            /* channel buffer */
    Int   ch_size;            /* channel buffer size = #bytes in a channel */
...
static Int Expander_remaining(Source *pself)
{
    ...
    Int sub_bits_per_channel = TxSource_pixelsPerLine(pself)
...
static Sane.Status Expander_init(Expander *pself,
                  SnapScan_Scanner *pss,
                                  Source *psub)
{
    Sane.Status status = TxSource_init((TxSource *) pself,
                                       pss,
                                       Expander_remaining,
...
typedef struct
{
    TX_SOURCE_GUTS
    Sane.Byte *ch_buf;            /* channel buffer */
    Int   ch_size;           /* channel buffer size */
    Int   ch_line_size;      /* size of one line */
    Int   ch_ndata;          /* actual #bytes in channel buffer */
    Int   ch_pos;            /* position in buffer */
    Int   ch_bytes_per_pixel
    Bool  ch_lineart
    Int   ch_offset;         /* The number of lines to be shifted */
    Bool  ch_past_init;      /* flag indicating if we have enough data to shift pixels down */
        ...
        if(ps.ch_lineart)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    Sane.Byte valid_pixel = ps.ch_buf[ps.ch_pos] & 0xaa
                    *pbuf = valid_pixel | (valid_pixel << 1)
                }
            }
        }
...
{
    TX_SOURCE_GUTS
    Sane.Byte *cbuf;            /* circular line buffer */
    Sane.Byte *xbuf;            /* single line buffer */
    Int pos;                    /* current position in xbuf */
    Int cb_size;            /* size of the circular buffer */
    Int cb_line_size;/* size of a line in the circular buffer */
    Int cb_start;        /* start of valid data in the circular buffer */
    Int cb_finish;        /* finish of valid data, for next read */
    Int ch_offset[3];/* offset in cbuf */
    Int round_req
    Int round_read
} RGBRouter
