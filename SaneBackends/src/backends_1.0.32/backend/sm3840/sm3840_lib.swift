		   ...
		   unsigned char *bytes, Int size, Int timeout)
{
  ...
  my_size = size
  status =
    sanei_usb_write_bulk((Int) dev, (Sane.Byte *) bytes, &my_size)
		  ...
		  unsigned char *bytes, Int size, Int timeout)
{
  ...
  my_size = size
  status =
    sanei_usb_read_bulk((Int) dev, (Sane.Byte *) bytes, &my_size)
  if(status != Sane.STATUS_GOOD)
    my_size = -1
  return my_size
}
		    ...
		    unsigned char *bytes, Int size, Int timeout)
{
  ...
  timeout = timeout
  status = sanei_usb_control_msg((Int) dev, (Int) requesttype,
