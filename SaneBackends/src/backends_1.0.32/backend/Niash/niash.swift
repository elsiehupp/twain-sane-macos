...
typedef struct
{
  ...
  Int aGammaTable[Sane.GAMMA_SIZE];        /* a 12-to-8 bit color lookup table */

  /* fCancelled needed to let sane issue the cancel message
     instead of an error message */
  Bool fCancelled;         /* Sane.TRUE if scanning cancelled */

  Bool fScanning;          /* Sane.TRUE if actively scanning */
...
/* option constraints */
static const Sane.Range rangeGammaTable = { 0, 255, 1 ]

/* available scanner resolutions */
static const Int setResolutions[] = { 4, 75, 150, 300, 600 ]
  ...
  for (i = 0; i < Sane.GAMMA_SIZE; ++i)
    {
      ...
      next = ((i + 1) * HW_GAMMA_SIZE) / Sane.GAMMA_SIZE
...
static const char lineartStr[] = { Sane.VALUE_SCAN_MODE_LINEART ]
...
static const Sane.Range rangeThreshold = {
  0,
  100,
  1
]
...
/* scanning modes */
static Sane.String_Const modeList[] = {
  colorStr,
  grayStr,
  lineartStr,
  NULL
...
static const TModeParam modeParam[] = {
  {DEPTH_COLOR, Sane.FRAME_RGB, _bytesPerLineColor, _rgb2rgb},
  {DEPTH_GRAY, Sane.FRAME_GRAY, _bytesPerLineGray, _rgb2gray},
  {DEPTH_LINEART, Sane.FRAME_GRAY, _bytesPerLineLineart, _rgb2lineart}
]
  ...
  for (i = optCount; i < optLast; i++)
    {
      ...
      switch (i)
...
#ifdef EXPERIMENTAL
        case optGamma:
          pDesc.name = Sane.NAME_ANALOG_GAMMA
          pDesc.title = Sane.TITLE_ANALOG_GAMMA
          pDesc.desc = Sane.DESC_ANALOG_GAMMA
        ...
        case optMode:
          ...
          pDesc.cap =
            Sane.CAP_SOFT_SELECT | Sane.CAP_SOFT_DETECT | Sane.CAP_EMULATED
          pVal.w = MODE_COLOR
          break
    ...
    case Sane.ACTION_SET_VALUE:
      ...
      switch (n)
        ...
        case optBRY:
          ...
          fSame = s.aValues[n].w == *(Sane.Word *) pVal
#endif
