...
#define COLOR_RAW_STR			Sane.I18N("Color RAW")
#define COLOR_RGB_STR			Sane.I18N("Color RGB")
#define COLOR_RGB_TEXT_STR              Sane.I18N("Color RGB TEXT")
/*--------------------------------------------------------------------------*/
...
typedef struct Stv680_Vidcam
{
  ...
  Sane.Byte *buffer;		/* for USB transfer. */

  /* Bayer handling */
  size_t output_size;		/* size of the output */
  Sane.Byte *output;		/* for bayer conversion */
  ...
  Sane.Byte *windoww;		/* for window write     */
  Sane.Byte *windowr;		/* for window read      */

  /* Scanner infos. */
  const struct vidcam_hardware *hw;	/* default options for that vidcam */

  Sane.Word *resolutions_list
  Sane.Word *color_sequence_list

  /* Scanning handling. */
  Bool scanning;		/* TRUE if a scan is running. */
  Bool deliver_eof
