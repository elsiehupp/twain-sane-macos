 ...
 */
static Bool usb_normFileName( char *fname, char* buffer, u_long max_len )
{
	char *src, *dst
...
static Int usb_CheckForPlustekDevice( Int handle, Plustek_Device *dev )
{
	...
	/* get the PCB-ID */
	result = sanei_lm983x_read( handle, 0x59, reg59s, 3, Sane.TRUE )
	...
	result = sanei_lm983x_write( handle, 0x59,  reg59, 3, Sane.TRUE )
	if( Sane.STATUS_GOOD != result ) {
		sanei_usb_close( handle )
		return -1
	}
...
static void usbDev_shutdown( Plustek_Device *dev  )
{
	...
	if( Sane.STATUS_GOOD == sanei_usb_open( dev.sane.name, &handle )) {
		...
		if( 0 != dev.usbDev.bLampOffOnEnd ) {
			...
			usb_LampOn( dev, Sane.FALSE, Sane.FALSE )
		}
		dev.fd = -1
		sanei_usb_close( handle )
	}
 ...
 * @returns; Sane.TRUE if supported, Sane.FALSE if not
 */
static Bool usb_IsDeviceInList( char *usbIdStr )
{
	var i: Int
 ...
 */
static Sane.Status usb_attach( Sane.String_Const dev_name )
{
	Int      len
	DevList *tmp, *last
...
/**
 */
static Int usbDev_setMap( Plustek_Device *dev, Sane.Word *map,
                          Sane.Word length, Sane.Word channel )
{
	Sane.Word i, idx
	...
	if( channel == _MAP_MASTER ) {
		...
		for( i = 0; i < length; i++ ) {
			a_bMap[i]            = (Sane.Byte)map[i]
			a_bMap[length +i]    = (Sane.Byte)map[i]
			a_bMap[(length*2)+i] = (Sane.Byte)map[i]
		}
		...
		for( i = 0; i < length; i++ ) {
			a_bMap[(length * idx)+i] = (Sane.Byte)map[i]
		}
	}
...
usbDev_setScanEnv( Plustek_Device *dev, ScanInfo *si )
{
	 ...
	 */
	usb_AdjustCISLampSettings( dev, Sane.TRUE )
	...
	if( scan.sParam.bSource == SOURCE_ADF ) {
		...
		if( scan.dwFlag & SCANDEF_ContinuousScan )
			dev.usbDev.fLastScanIsAdf = Sane.TRUE
		else
			dev.usbDev.fLastScanIsAdf = Sane.FALSE
	}
...
/* HEINER: Preview currently not working correctly */
	...
	if((scan.dwFlag & SCANDEF_Adf) && (scan.dwFlag & SCANDEF_ContinuousScan)) {
		scan.fCalibrated = Sane.TRUE
	} else {
		scan.fCalibrated = Sane.FALSE
	}
	...
	usb_LampOn( dev, Sane.TRUE, Sane.TRUE )

	m_fStart    = m_fFirst = Sane.TRUE
	m_fAutoPark = (scan.dwFlag&SCANFLAG_StillModule)?Sane.FALSE:Sane.TRUE

	if( usb_IsSheetFedDevice(dev))
		if(usb_InCalibrationMode(dev))
			m_fAutoPark = Sane.FALSE
 ...
 */
static Int
usbDev_Prepare( Plustek_Device *dev, Sane.Byte *buf )
{
	Int       result
	Bool use_alt_cal = Sane.FALSE
	ScanDef  *scan        = &dev.scanning
	DCapsDef *scaps       = &dev.usbDev.Caps
	...
	if( !(scan.dwFlag & SCANFLAG_Scanning)) {
		...
		if( !usb_ScanBegin( dev,
			(scan.dwFlag&SCANFLAG_StillModule) ? Sane.FALSE:Sane.TRUE)) {
