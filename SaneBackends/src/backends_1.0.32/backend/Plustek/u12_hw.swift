...
static void u12hw_PutToIdleMode( U12_Device *dev )
{
	...
	u12io_DataToRegs( dev, (Sane.Byte*)u12CcdStop, _STOP_LEN )
}
 ...
 */
static void u12hw_ButtonSetup( U12_Device *dev, Sane.Byte numButtons )
{
...
/**
 */
static Sane.Status u12hw_InitAsic( U12_Device *dev, Bool shading )
{
	Sane.Byte rb[6]
	Int       c
	...
	dev.MotorID = (Sane.Byte)(dev.DACType & _MOTOR0_MASK)

	dev.regs.RD_MotorDriverType =
	                        (Sane.Byte)((dev.DACType & _MOTOR0_MASK) >> 3)
	dev.regs.RD_MotorDriverType |=
	                        (Sane.Byte)((dev.DACType & _MOTOR1_MASK) >> 1)
...
{
	Sane.Byte lampStatus

	dev.warmupNeeded = Sane.TRUE
...
static void u12hw_SetGeneralRegister( U12_Device *dev )
{
	...
	dev.scan.motorBackward = Sane.FALSE
...
static void u12hw_SetupScanningCondition( U12_Device *dev )
{
	...
	dev.scan.bRefresh          = (Sane.Byte)(dev.scan.dwInterval << 1)
	dev.regs.RD_LineControl    = _LOBYTE(dev.shade.wExposure)
	dev.regs.RD_ExtLineControl = _HIBYTE(dev.shade.wExposure)
	dev.regs.RD_XStepTime      = _LOBYTE(dev.shade.wXStep)
	dev.regs.RD_ExtXStepTime   = _HIBYTE(dev.shade.wXStep)
...
/**
 */
static Sane.Status u12hw_Memtest( U12_Device *dev )
{
	Sane.Byte tmp
	Sane.Byte buf[_TEST_SIZE]
	Int       i
	...
	for( i = 0; i < _TEST_SIZE; i++ ) {
		if((Sane.Byte)((i * 3) & 0xff) != buf[i] ) {
			DBG( _DBG_ERROR, "* Memtest failed at pos %u: %u != %u\n",
					i+1, buf[i], (Sane.Byte)((i * 3) & 0xff) )
 ...
 *  is also accessible...
 */
static Sane.Status u12hw_CheckDevice( U12_Device *dev )
{
#ifndef _FAKE_DEVICE
	Sane.Byte tmp
	Sane.Byte rb[8]
...
static void u12motor_ToHomePosition( U12_Device *, Bool )
...
/**
 */
static Sane.Status u12hw_WarmupLamp( U12_Device *dev )
{
	TimerDef timer
...
static void usb_LampTimerIrq( Int sig )
{
	...
	if( -1 == dev_xxx.fd ) {
		...
		if( Sane.STATUS_GOOD == sanei_usb_open(dev_xxx.sane.name, &handle)) {
			dev_xxx.fd = handle
		}
	}
