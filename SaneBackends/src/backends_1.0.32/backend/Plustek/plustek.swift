...
import plustek-usbio.c"
import plustek-usbdevs.c"
import plustek-usbhw.c"
import plustek-usbmap.c"
import plustek-usbscan.c"
import plustek-usbimg.c"
import plustek-usbcalfile.c"
import plustek-usbshading.c"
import plustek-usbcal.c"
import plustek-usb.c"
...
static const Sane.Range warmup_range   = { -1,   999, 1 ]
static const Sane.Range offtimer_range = {  0,   999, 1 ]
static const Sane.Range gain_range     = { -1,    63, 1 ]
static const Sane.Range loff_range     = { -1, 16363, 1 ]

/* authorization stuff */
static Sane.Auth_Callback auth = NULL

/* prototype... */
static Sane.Status local_Sane.start(Plustek_Scanner *, Int)
...
static void show_cnf( CnfDef *cnf )
{
	...
	DBG( _DBG_Sane.INIT,"model ovr.   : %d\n",  cnf.adj.mov                 )
	DBG( _DBG_Sane.INIT,"warmup       : %ds\n", cnf.adj.warmup              )
	DBG( _DBG_Sane.INIT,"lampOff      : %d\n",  cnf.adj.lampOff             )
	DBG( _DBG_Sane.INIT,"lampOffOnEnd : %s\n",  _YN(cnf.adj.lampOffOnEnd   ))
	DBG( _DBG_Sane.INIT,"cacheCalData : %s\n",  _YN(cnf.adj.cacheCalData   ))
	DBG( _DBG_Sane.INIT,"altCalibrate : %s\n",  _YN(cnf.adj.altCalibrate   ))
	DBG( _DBG_Sane.INIT,"skipCalibr.  : %s\n",  _YN(cnf.adj.skipCalibration))
	DBG( _DBG_Sane.INIT,"skipFine     : %s\n",  _YN(cnf.adj.skipFine       ))
	DBG( _DBG_Sane.INIT,"skipFineWhite: %s\n",  _YN(cnf.adj.skipFineWhite  ))
	DBG( _DBG_Sane.INIT,"skipDarkStrip: %s\n",  _YN(cnf.adj.skipDarkStrip   ))
	DBG( _DBG_Sane.INIT,"incDarkTarget: %s\n",  _YN(cnf.adj.incDarkTgt      ))
	DBG( _DBG_Sane.INIT,"invertNegs.  : %s\n",  _YN(cnf.adj.invertNegatives))
	DBG( _DBG_Sane.INIT,"dis.Speedup  : %s\n",  _YN(cnf.adj.disableSpeedup ))
	DBG( _DBG_Sane.INIT,"pos_x        : %d\n",  cnf.adj.pos.x               )
	DBG( _DBG_Sane.INIT,"pos_y        : %d\n",  cnf.adj.pos.y               )
	DBG( _DBG_Sane.INIT,"pos_shading_y: %d\n",  cnf.adj.posShadingY         )
	DBG( _DBG_Sane.INIT,"neg_x        : %d\n",  cnf.adj.neg.x               )
	DBG( _DBG_Sane.INIT,"neg_y        : %d\n",  cnf.adj.neg.y               )
	DBG( _DBG_Sane.INIT,"neg_shading_y: %d\n",  cnf.adj.negShadingY         )
	DBG( _DBG_Sane.INIT,"tpa_x        : %d\n",  cnf.adj.tpa.x               )
	DBG( _DBG_Sane.INIT,"tpa_y        : %d\n",  cnf.adj.tpa.y               )
	DBG( _DBG_Sane.INIT,"tpa_shading_y: %d\n",  cnf.adj.tpaShadingY         )
	DBG( _DBG_Sane.INIT,"red gain     : %d\n",  cnf.adj.rgain               )
	DBG( _DBG_Sane.INIT,"green gain   : %d\n",  cnf.adj.ggain               )
	DBG( _DBG_Sane.INIT,"blue gain    : %d\n",  cnf.adj.bgain               )
	DBG( _DBG_Sane.INIT,"red offset   : %d\n",  cnf.adj.rofs                )
	DBG( _DBG_Sane.INIT,"green offset : %d\n",  cnf.adj.gofs                )
	DBG( _DBG_Sane.INIT,"blue offset  : %d\n",  cnf.adj.bofs                )
	DBG( _DBG_Sane.INIT,"red lampoff  : %d\n",  cnf.adj.rlampoff            )
	DBG( _DBG_Sane.INIT,"green lampoff: %d\n",  cnf.adj.glampoff            )
	DBG( _DBG_Sane.INIT,"blue lampoff : %d\n",  cnf.adj.blampoff            )
 ...
 */
static Sane.Status
do_cancel( Plustek_Scanner *scanner, Bool closepipe )
{
	struct SIGACTION act
	Sane.Pid         res
	...
	if( sanei_thread_is_valid(scanner.reader_pid) ) {
		...
		cancelRead = Sane.TRUE
		scanner.calibrating = Sane.FALSE
...
init_options( Plustek_Scanner *s )
{
	...
	s.opt[OPT_DEVICE_GROUP].title = Sane.I18N("Device-Settings")
	s.opt[OPT_DEVICE_GROUP].desc  = ""
	s.opt[OPT_DEVICE_GROUP].type  = Sane.TYPE_GROUP
	s.opt[OPT_DEVICE_GROUP].cap   = 0
	s.opt[OPT_DEVICE_GROUP].constraint_type = Sane.CONSTRAINT_NONE

	s.opt[OPT_LAMPSWITCH].name  = "lamp-switch"
	s.opt[OPT_LAMPSWITCH].title = Sane.I18N("Lampswitch")
	s.opt[OPT_LAMPSWITCH].desc  = Sane.I18N("Manually switching the lamp(s).")
	s.opt[OPT_LAMPSWITCH].type  = Sane.TYPE_BOOL
	s.val[OPT_LAMPSWITCH].w     = Sane.FALSE

	s.opt[OPT_LOFF4DARK].name  = "lamp-off-during-dcal"
	s.opt[OPT_LOFF4DARK].title = Sane.I18N("Lamp off during dark calibration")
	s.opt[OPT_LOFF4DARK].desc  = Sane.I18N("Always switches lamp off when doing dark calibration.")
	s.opt[OPT_LOFF4DARK].type  = Sane.TYPE_BOOL
	...
	s.opt[OPT_CACHECAL].title = Sane.I18N("Calibration data cache")
	s.opt[OPT_CACHECAL].desc  = Sane.I18N("Enables or disables calibration data cache.")
	s.opt[OPT_CACHECAL].type  = Sane.TYPE_BOOL
	s.val[OPT_CACHECAL].w     = adj.cacheCalData

	s.opt[OPT_CALIBRATE].name  = "calibrate"
	s.opt[OPT_CALIBRATE].title = Sane.I18N("Calibrate")
	s.opt[OPT_CALIBRATE].desc  = Sane.I18N("Performs calibration")
	s.opt[OPT_CALIBRATE].type  = Sane.TYPE_BUTTON
	s.opt[OPT_CALIBRATE].size = sizeof(Sane.Word)
	s.opt[OPT_CALIBRATE].constraint_type = Sane.CONSTRAINT_NONE
	s.opt[OPT_CALIBRATE].constraint.range = 0
	s.opt[OPT_CALIBRATE].cap = Sane.CAP_SOFT_DETECT | Sane.CAP_SOFT_SELECT |
	                            Sane.CAP_AUTOMATIC
	...
	s.opt[OPT_SPEEDUP].title = Sane.I18N("Speedup sensor")
	s.opt[OPT_SPEEDUP].desc  = Sane.I18N("Enables or disables speeding up sensor movement.")
	s.opt[OPT_SPEEDUP].type  = Sane.TYPE_BOOL
	s.val[OPT_SPEEDUP].w     = !(adj.disableSpeedup)

	if( s.hw.usbDev.HwSetting.dHighSpeed == 0.0 )
		_DISABLE(OPT_SPEEDUP)

	s.opt[OPT_LAMPOFF_ONEND].name  = Sane.NAME_LAMP_OFF_AT_EXIT
	s.opt[OPT_LAMPOFF_ONEND].title = Sane.TITLE_LAMP_OFF_AT_EXIT
	s.opt[OPT_LAMPOFF_ONEND].desc  = Sane.DESC_LAMP_OFF_AT_EXIT
	s.opt[OPT_LAMPOFF_ONEND].type  = Sane.TYPE_BOOL
	s.val[OPT_LAMPOFF_ONEND].w     = adj.lampOffOnEnd

	s.opt[OPT_WARMUPTIME].name  = "warmup-time"
	s.opt[OPT_WARMUPTIME].title = Sane.I18N("Warmup-time")
	s.opt[OPT_WARMUPTIME].desc  = Sane.I18N("Warmup-time in seconds.")
	s.opt[OPT_WARMUPTIME].type  = Sane.TYPE_INT
	s.opt[OPT_WARMUPTIME].constraint_type = Sane.CONSTRAINT_RANGE
	...
	s.opt[OPT_LAMPOFF_TIMER].title = Sane.I18N("Lampoff-time")
	s.opt[OPT_LAMPOFF_TIMER].desc  = Sane.I18N("Lampoff-time in seconds.")
	s.opt[OPT_LAMPOFF_TIMER].type  = Sane.TYPE_INT
	s.opt[OPT_LAMPOFF_TIMER].constraint_type = Sane.CONSTRAINT_RANGE
	s.opt[OPT_LAMPOFF_TIMER].constraint.range = &offtimer_range
	s.val[OPT_LAMPOFF_TIMER].w     = adj.lampOff

	/* "Analog Frontend" group*/
	s.opt[OPT_AFE_GROUP].title = Sane.I18N("Analog frontend")
	s.opt[OPT_AFE_GROUP].desc  = ""
	s.opt[OPT_AFE_GROUP].type  = Sane.TYPE_GROUP
	s.opt[OPT_AFE_GROUP].cap   = Sane.CAP_ADVANCED

	s.opt[OPT_OVR_REDGAIN].name  = "red-gain"
	s.opt[OPT_OVR_REDGAIN].title = Sane.I18N("Red gain")
	s.opt[OPT_OVR_REDGAIN].desc  = Sane.I18N("Red gain value of the AFE")
	s.opt[OPT_OVR_REDGAIN].type  = Sane.TYPE_INT
	s.opt[OPT_OVR_REDGAIN].constraint_type = Sane.CONSTRAINT_RANGE
	s.opt[OPT_OVR_REDGAIN].constraint.range = &gain_range
	s.val[OPT_OVR_REDGAIN].w     = adj.rgain

	s.opt[OPT_OVR_REDOFS].name  = "red-offset"
	s.opt[OPT_OVR_REDOFS].title = Sane.I18N("Red offset")
	s.opt[OPT_OVR_REDOFS].desc  = Sane.I18N("Red offset value of the AFE")
	s.opt[OPT_OVR_REDOFS].type  = Sane.TYPE_INT
	s.opt[OPT_OVR_REDOFS].constraint_type = Sane.CONSTRAINT_RANGE
	s.opt[OPT_OVR_REDOFS].constraint.range = &gain_range
	s.val[OPT_OVR_REDOFS].w     = adj.rofs

	s.opt[OPT_OVR_GREENGAIN].name  = "green-gain"
	s.opt[OPT_OVR_GREENGAIN].title = Sane.I18N("Green gain")
	s.opt[OPT_OVR_GREENGAIN].desc  = Sane.I18N("Green gain value of the AFE")
	s.opt[OPT_OVR_GREENGAIN].type  = Sane.TYPE_INT
	s.opt[OPT_OVR_GREENGAIN].constraint_type = Sane.CONSTRAINT_RANGE
	...
	s.opt[OPT_OVR_GREENOFS].title = Sane.I18N("Green offset")
	s.opt[OPT_OVR_GREENOFS].desc  = Sane.I18N("Green offset value of the AFE")
	s.opt[OPT_OVR_GREENOFS].type  = Sane.TYPE_INT
	s.opt[OPT_OVR_GREENOFS].constraint_type = Sane.CONSTRAINT_RANGE
	s.opt[OPT_OVR_GREENOFS].constraint.range = &gain_range
	s.val[OPT_OVR_GREENOFS].w     = adj.gofs

	s.opt[OPT_OVR_BLUEGAIN].name  = "blue-gain"
	s.opt[OPT_OVR_BLUEGAIN].title = Sane.I18N("Blue gain")
	s.opt[OPT_OVR_BLUEGAIN].desc  = Sane.I18N("Blue gain value of the AFE")
	s.opt[OPT_OVR_BLUEGAIN].type  = Sane.TYPE_INT
	s.opt[OPT_OVR_BLUEGAIN].constraint_type = Sane.CONSTRAINT_RANGE
	...
	s.opt[OPT_OVR_BLUEOFS].title = Sane.I18N("Blue offset")
	s.opt[OPT_OVR_BLUEOFS].desc  = Sane.I18N("Blue offset value of the AFE")
	s.opt[OPT_OVR_BLUEOFS].type  = Sane.TYPE_INT
	s.opt[OPT_OVR_BLUEOFS].constraint_type = Sane.CONSTRAINT_RANGE
	s.opt[OPT_OVR_BLUEOFS].constraint.range = &gain_range
	s.val[OPT_OVR_BLUEOFS].w     = adj.bofs

	s.opt[OPT_OVR_RED_LOFF].name  = "redlamp-off"
	s.opt[OPT_OVR_RED_LOFF].title = Sane.I18N("Red lamp off")
	s.opt[OPT_OVR_RED_LOFF].desc  = Sane.I18N("Defines red lamp off parameter")
	s.opt[OPT_OVR_RED_LOFF].type  = Sane.TYPE_INT
	s.opt[OPT_OVR_RED_LOFF].constraint_type = Sane.CONSTRAINT_RANGE
	...
	s.opt[OPT_OVR_GREEN_LOFF].title = Sane.I18N("Green lamp off")
	s.opt[OPT_OVR_GREEN_LOFF].desc  = Sane.I18N("Defines green lamp off parameter")
	s.opt[OPT_OVR_GREEN_LOFF].type  = Sane.TYPE_INT
	s.opt[OPT_OVR_GREEN_LOFF].constraint_type = Sane.CONSTRAINT_RANGE
	...
	s.opt[OPT_OVR_BLUE_LOFF].title = Sane.I18N("Blue lamp off")
	s.opt[OPT_OVR_BLUE_LOFF].desc  = Sane.I18N("Defines blue lamp off parameter")
	s.opt[OPT_OVR_BLUE_LOFF].type  = Sane.TYPE_INT
	s.opt[OPT_OVR_BLUE_LOFF].constraint_type = Sane.CONSTRAINT_RANGE
	...
	for( i = OPT_BUTTON_0; i <= OPT_BUTTON_LAST; i++ ) {
		...
		s.opt[i].desc  = Sane.I18N("This option reflects the status "
		                            "of the scanner buttons.")
		s.opt[i].type = Sane.TYPE_BOOL
		s.opt[i].cap  = Sane.CAP_SOFT_DETECT |
		                 Sane.CAP_HARD_SELECT | Sane.CAP_ADVANCED
		if(i - OPT_BUTTON_0 >= dev.usbDev.Caps.bButtons )
			_DISABLE(i)

		s.opt[i].unit = Sane.UNIT_NONE
		s.opt[i].size = sizeof(Sane.Word)
		s.opt[i].constraint_type = Sane.CONSTRAINT_NONE
		s.opt[i].constraint.range = 0
		s.val[i].w = Sane.FALSE
	}
...
attach( const char *dev_name, CnfDef *cnf, Plustek_Device **devp )
{
	...
	handle = usbDev_open( dev, usbDevs, Sane.FALSE )
	if( handle < 0 ) {
...
do_calibration( void *args )
{
	...
	for( ; scanmode < 5; scanmode++ ) {
		...
		if(Sane.STATUS_GOOD == local_Sane.start(s, modes[scanmode])) {
			...
			DBG(_DBG_ERROR, "local_Sane.start() failed!\n")
			break
		}
	}
	...
	if(s.calibrating) {
		if(action == Sane.ACTION_SET_VALUE) {
			...
			switch(option) {
				...
				case OPT_BR_X:
				case OPT_BR_Y: break
				default:       return Sane.STATUS_DEVICE_BUSY
			}
		}
	}
	...
	switch( action ) {
		...
		case Sane.ACTION_SET_VALUE:
			...
			switch(option) {
				...
				case OPT_LAMPOFF_TIMER:
					s.val[option].w = (*(Sane.Word *)value)
					adj.lampOff     = (*(Sane.Word *)value)
					usb_CheckAndCopyAdjs( dev )
					break

				case OPT_WARMUPTIME:
					s.val[option].w = (*(Sane.Word *)value)
					adj.warmup      = (*(Sane.Word *)value)
					usb_CheckAndCopyAdjs( dev )
					break

				case OPT_OVR_REDGAIN:
					s.val[option].w = (*(Sane.Word *)value)
					adj.rgain       = (*(Sane.Word *)value)
					break
				case OPT_OVR_GREENGAIN:
					s.val[option].w = (*(Sane.Word *)value)
					adj.ggain       = (*(Sane.Word *)value)
					break
				case OPT_OVR_BLUEGAIN:
					s.val[option].w = (*(Sane.Word *)value)
					adj.bgain       = (*(Sane.Word *)value)
					break
				case OPT_OVR_REDOFS:
					s.val[option].w = (*(Sane.Word *)value)
					adj.rofs        = (*(Sane.Word *)value)
					break
				case OPT_OVR_GREENOFS:
					s.val[option].w = (*(Sane.Word *)value)
					adj.gofs       = (*(Sane.Word *)value)
					break
				case OPT_OVR_BLUEOFS:
					s.val[option].w = (*(Sane.Word *)value)
					adj.bofs        = (*(Sane.Word *)value)
					break
				case OPT_OVR_RED_LOFF:
					s.val[option].w = (*(Sane.Word *)value)
					adj.rlampoff    = (*(Sane.Word *)value)
					break
				case OPT_OVR_GREEN_LOFF:
					s.val[option].w = (*(Sane.Word *)value)
					adj.glampoff    = (*(Sane.Word *)value)
					break
				case OPT_OVR_BLUE_LOFF:
					s.val[option].w = (*(Sane.Word *)value)
					adj.blampoff    = (*(Sane.Word *)value)
					break
					...
					if( s.val[option].w == 0 ) {
						...
						if( s.val[option].w == 1 ) {
							...
							if( dev.usbDev.Caps.wFlags & DEVCAPSFLAG_LargeTPA ) {
								dev.x_range.max = Sane.FIX(_SCALE(_TPALargePageWidth))
								dev.y_range.max = Sane.FIX(_SCALE(_TPALargePageHeight))
							} else {
								dev.x_range.max = Sane.FIX(_SCALE(_TPAPageWidth))
								dev.y_range.max = Sane.FIX(_SCALE(_TPAPageHeight))
							}
							...
							if( dev.usbDev.Caps.wFlags & DEVCAPSFLAG_LargeTPA ) {
								dev.x_range.max = Sane.FIX(_SCALE(_NegLargePageWidth))
								dev.y_range.max = Sane.FIX(_SCALE(_NegLargePageHeight))
							} else {
								dev.x_range.max = Sane.FIX(_SCALE(_NegPageWidth))
								dev.y_range.max = Sane.FIX(_SCALE(_NegPageHeight))
							}
 ...
 */
static Sane.Status
local_Sane.start(Plustek_Scanner *s, Int scanmode )
{
	Plustek_Device *dev
	 ...
	 */
	dev.fd = usbDev_open( dev, NULL, Sane.TRUE )
	if( dev.fd < 0 ) {
	...
	DBG( _DBG_Sane.INIT, "local_Sane.start done\n" )
	return Sane.STATUS_GOOD
}
...
Sane.start( Sane.Handle handle )
{
	...
	status = local_Sane.start(s, getScanMode(s))
	if(status != Sane.STATUS_GOOD) {
		return status
	}
