                                     ...
                                     void *coeff_buffer, u_short wCount )
{
	...
	if( usbio_WriteReg( dev.fd, 0x03, regs[0x03] )) {
		...
		res = sanei_lm983x_write( dev.fd, 0x04, &regs[0x04], 2, Sane.TRUE )

		/* Download offset coefficients */
		if( Sane.STATUS_GOOD == res ) {

			res = sanei_lm983x_write( dev.fd, 0x06,
			                          (u_char*)coeff_buffer, wCount, Sane.FALSE )
			if( Sane.STATUS_GOOD == res )
				return Sane.TRUE
		}
	}
                                      ...
                                      void *data_buffer, u_short wCount )
{
	...
	if( usbio_WriteReg( dev.fd, 0x03, regs[0x03] )) {
		...
		if( Sane.STATUS_GOOD == res ) {
			res = sanei_lm983x_write(dev.fd, 0x06,
			                        (u_char*)data_buffer, wCount, Sane.FALSE )
			if( Sane.STATUS_GOOD == res )
				return Sane.TRUE
		}
	}
 ...
 * @return
 */
static Bool adjLampSetting( Plustek_Device *dev, Int channel, u_long max,
 ...
 *
 */
static Bool usb_AdjustGain( Plustek_Device *dev, Int fNegative )
{
	char      tmp[40]
	Int       i
	double    min_mclk
...
TOGAIN:
	...
	if( !usb_ScanBegin( dev, Sane.FALSE) ||
 ...
 */
static Bool usb_AdjustOffset( Plustek_Device *dev )
{
	char          tmp[40]
	...
	if( usb_IsCISDevice(dev)) {
		...
		if( dev.usbDev.pSource.DarkShadOrgY >= 0 ) {
			...
			usb_ModuleToHome( dev, Sane.TRUE )
	...
	while( cAdjust ) {
		...
		_UIO(sanei_lm983x_write(dev.fd, 0x38, &regs[0x38], 3, Sane.TRUE))
		cAdjust >>= 1
	}
 ...
 */
static Bool usb_AdjustWhiteShading( Plustek_Device *dev )
{
	char          tmp[40]
	ScanDef      *scan  = &dev.scanning
	DCapsDef     *scaps = &dev.usbDev.Caps
...
usb_PrepareCalibration( Plustek_Device *dev )
{
	...
	scan.skipCoarseCalib = Sane.FALSE

	if( dev.adj.cacheCalData )
		if( usb_ReadAndSetCalData( dev ))
			scan.skipCoarseCalib = Sane.TRUE
...
usb_DoCalibration( Plustek_Device *dev )
{
	...
	if( Sane.TRUE == scanning.fCalibrated )
		return Sane.TRUE
...
/*	}*/
		...
		switch( scanning.sParam.bSource ) {
			...
			case SOURCE_Negative:
				...
				usb_line_statistics( "White", a_wWhiteShading,
				                     m_ScanParam.Size.dwPhyPixels, Sane.TRUE)
				break
	...
	scanning.fCalibrated = Sane.TRUE
	...
	switch( what ) {
		...
		case PARAM_WhiteShading:
			...
			_UIO(sanei_lm983x_write( dev.fd, 0x40,
			                        &regs[0x40], 0x42-0x40+1, Sane.TRUE ))
			break
