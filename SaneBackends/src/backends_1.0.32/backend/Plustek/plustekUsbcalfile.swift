 ...
 * @return Sane.TRUE on success, Sane.FALSE on any error
 */
static Bool
 ...
 */
static void
usb_CreatePrefix( Plustek_Device *dev, char *pfx, Bool add_bitdepth )
{
	char       bd[5]
...
usb_SaveCalData( Plustek_Device *dev )
{
	...
	if( Sane.TRUE == scanning.skipCoarseCalib ) {
                     ...
                     u_short *dark, u_short *white, u_long vals )
{
	...
	usb_CreatePrefix( dev, pfx, Sane.FALSE )
 ...
 * @returns Sane.FALSE when the reading fails or Sane.TRUE on success
 */
static Bool
