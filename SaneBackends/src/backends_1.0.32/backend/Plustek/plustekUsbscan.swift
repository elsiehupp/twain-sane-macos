...
static Bool  m_fStart = Sane.FALSE

/* Prototype... */
static Bool usb_DownloadShadingData( Plustek_Device*, u_char )
...
*/
	...
	_UIO(sanei_lm983x_write( dev.fd, 0x03, &regs[0x03], 3, Sane.TRUE))
	_UIO(sanei_lm983x_write( dev.fd, 0x08, &regs[0x08], 0x7f - 0x08+1, Sane.TRUE))
 ...
 */
static Bool
usb_ScanBegin( Plustek_Device *dev, Bool auto_park )
{
	u_char  value
	u_short inches
	HWDef       *hw   = &dev.usbDev.HwSetting
	...
	m_fStart = m_fFirst = Sane.TRUE;        /* Prepare to read */
	...
	if( inches > sc.Normal.DataOrigin.y )
		usb_WaitPos( dev, 150, Sane.FALSE )
...
usb_IsDataAvailableInDRAM( Plustek_Device *dev )
{
	...
	for(;;)	{
		...
		_UIO( sanei_lm983x_read( dev.fd, 0x01, a_bBand, 3, Sane.FALSE ))
...
usb_ScanReadImage( Plustek_Device *dev, void *pBuf, u_long dwSize )
{
	...
	if( m_fFirst ) {
		...
		sanei_lm983x_write(dev.fd, 0x48, &regs[0x48], 2, Sane.TRUE)
	}
	res = sanei_lm983x_read(dev.fd, 0x00, (u_char *)pBuf, dwSize, Sane.FALSE)
