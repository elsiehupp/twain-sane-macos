...
/**
 */
static Sane.Status u12motor_PositionYProc( U12_Device *dev, u_long steps )
{
	TimerDef timer
 ...
 */
static void u12motor_ToHomePosition( U12_Device *dev, Bool wait )
{
	TimerDef  timer
...
/**
 */
static Sane.Status u12motor_BackToHomeSensor( U12_Device *dev )
{
	Sane.Byte rb[20]
	Int       c
	TimerDef  timer
...
/**
 */
static Sane.Status u12motor_ModuleToHome( U12_Device *dev )
{
	Sane.Status res
	...
	if(!(u12io_DataFromRegister( dev, REG_STATUS ) & _FLAG_PAPER)) {
		...
		u12io_DataToRegister( dev, REG_MOTOR0CONTROL,
		             (Sane.Byte)(dev.regs.RD_Motor0Control|_MotorDirForward))
...
/**
 */
static Sane.Status u12motor_WaitForPositionY( U12_Device *dev )
{
	Sane.Byte   rb[20]
	Sane.Status res
	Sane.Byte   bXStep
	...
	bXStep = (Sane.Byte)((dev.DataInf.wPhyDataType <= COLOR_256GRAY) ?
	...
	if( dwBeginY > (_RFT_SCANNING_ORG + _YOFFSET) &&
		...
		u12io_DataToRegister( dev, REG_SCANCONTROL1,
		         (Sane.Byte)(dev.regs.RD_ScanControl1 & ~_MFRC_RUNSCANSTATE))
	...
	if(!dev.f2003 || (dev.shade.intermediate & _ScanMode_AverageOut) ||
		...
		u12io_DataToRegister( dev, REG_MOTORDRVTYPE,
	                (Sane.Byte)(dev.MotorPower & (_MOTORR_MASK | _MOTORR_STRONG)))
	} else {
		u12io_DataToRegister( dev, REG_MOTORDRVTYPE,
 ...
 */
static Sane.Status u12motor_GotoShadingPosition( U12_Device *dev )
{
	Sane.Byte   rb[20]
	Sane.Status res
	Int         c
