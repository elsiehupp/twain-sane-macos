 ...
 */
static void
u12shading_DownloadShadingTable( U12_Device *dev, Sane.Byte *buf, u_long len )
{
	Sane.Byte *val, *rb
	Sane.Byte  reg, regs[20]
	Int        c
	...
	u12io_DataToRegister( dev, REG_SCANCONTROL,
	                  (Sane.Byte)(dev.regs.RD_ScanControl | _SCAN_12BITMODE))
	...
	val = (Sane.Byte*)&dev.regs.RD_RedDarkOff
	rb  = &regs[2]
	c   = 1
...
/**
 */
static Sane.Status u12shadingAdjustShadingWaveform( U12_Device *dev )
{
	Sane.Byte     b
	...
	dev.scan.refreshState = Sane.FALSE
...
/*	_DODELAY( 100 ); */
	...
	if( dev.shade.pHilight ) {
		...
		memset((Sane.Byte*)dev.shade.pHilight +
	...
	for( count = 32; count--; ) {
		...
		u12io_ReadOneShadingLine( dev, ((Sane.Byte*)dev.bufs.b1.pShadingRam)+
...
/**
 */
static void u12shading_GainOffsetToDAC( U12_Device *dev, Sane.Byte ch,
                                        Sane.Byte reg, Sane.Byte d )
{
...
/**
 */
static Sane.Byte u12shading_SumGains( Sane.Byte *pb, u_long pixelsLine )
{
	Sane.Byte hilight, tmp
 ...
 */
static void
u12shading_AdjustGain( U12_Device *dev, u_long color, Sane.Byte hilight )
{
	if( hilight < dev.shade.bGainLow ) {
		...
		if( dev.shade.Hilight.bColors[color] < dev.shade.bGainHigh ) {
			...
			if( hilight <= (Sane.Byte)(dev.shade.bGainLow - hilight))
...
/**
 */
static Sane.Status u12shading_AdjustRGBGain( U12_Device *dev )
{
	Int       i
	Sane.Byte hi[3]
...
/*		_DODELAY( 100 ); */
		...
		if( u12io_ReadOneShadingLine( dev,
		                         (Sane.Byte*)dev.bufs.b1.pShadingRam, 2560)) {

			if( dev.DataInf.wPhyDataType <= COLOR_256GRAY ) {

				hi[1] = u12shading_SumGains(
				           (Sane.Byte*)dev.bufs.b1.pShadingRam + 2560, 2560)
				if( hi[1] ) {
...
/**
 */
static Sane.Status u12shadingAdjustDark( U12_Device *dev )
{
	u_long  i
	u_short wDarks[3]
 ...
 */
static void u12shading_DownloadMapTable( U12_Device *dev, Sane.Byte *buf )
{
	Sane.Byte addr, regs[6]
	Int       i

	u12io_DataToRegister( dev, REG_SCANCONTROL,
	          (Sane.Byte)((dev.regs.RD_ScanControl & 0xfc) | _SCAN_BYTEMODE))
...
/**
 */
static Sane.Status u12shading_DoCalibration( U12_Device *dev )
{
	Sane.Byte   tb[4096*3]
	u_long      i, tmp
	Sane.Byte   bScanControl, rb[20]
	Sane.Status res
	Int         c
