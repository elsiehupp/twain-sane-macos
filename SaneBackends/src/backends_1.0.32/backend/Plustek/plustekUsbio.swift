...
/**
 */
static void dumpPic( char* name, Sane.Byte *buffer, u_long len, Int is_gray )
{
	u_short type
	FILE   *fp
 ...
 */
static void dumpregs( Int fd, Sane.Byte *cmp )
{
	char      buf[256], b2[10]
	Sane.Byte regs[0x80]
	Int       i
	...
	if( fd >= 0 ) {
		...
		sanei_lm983x_read( fd, 0x08, &regs[0x8], 0x80-0x8, Sane.TRUE )
 ...
 * @return
 */
static Bool usbio_WriteReg( Int handle,
 ...
 */
static Sane.Status usbio_DetectLM983x( Int fd, Sane.Byte *version )
{
	char        buf[256]
	Sane.Byte   value
	Sane.Status res
 ...
 */
static Sane.Status usbio_ResetLM983x( Plustek_Device *dev )
{
	Sane.Byte value
