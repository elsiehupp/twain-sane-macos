 ...
 *
 * @param - none
 * @return the function returns Sane.TRUE if a cancel condition has been
 *  detected, if not, it returns Sane.FALSE
 */
static Bool
 ...
 * @param fOn    - Sane.TRUE means motor on, Sane.FALSE means motor off
 * @return always Sane.TRUE
 */
static Bool
usb_MotorOn( Plustek_Device *dev, Bool fOn )
{
	if( fOn )
	...
	for(;;) {
		...
		if( min_ffs != 0xffff ) {
			...
			if( ffs >= min_ffs ) {
				...
				r[0] = (u_char)(ffs >> 8)
				r[1] = (u_char)(ffs & 0xFF)
				sanei_lm983x_write(dev.fd, 0x48, r, 2, Sane.TRUE)
				if(ffs == min_ffs )
					ffs = 0
			} else {
...
usb_ModuleMove( Plustek_Device *dev, u_char action, u_long dwStep )
{
	...
	_UIO(sanei_lm983x_write(dev.fd, 0x48, &regs[0x48], 2, Sane.TRUE))
	_UIO(sanei_lm983x_write(dev.fd, 0x4A, &regs[0x4A], 2, Sane.TRUE))
 ...
 */
static Bool
usb_ModuleToHome( Plustek_Device *dev, Bool fWait )
{
	u_char    mclk_div
	u_char    value
	...
	if( value & 1 ) {
		dev.usbDev.fModFirstHome = Sane.FALSE
		return Sane.TRUE
	}
	...
	if( value != 2 ) {
		...
		_UIO(sanei_lm983x_write(dev.fd, 0x48, &regs[0x48], 4, Sane.TRUE))
		_UIO(sanei_lm983x_write(dev.fd, 0x56, &regs[0x56], 3, Sane.TRUE))
 ...
 */
static Bool
usb_MotorSelect( Plustek_Device *dev, Bool fADF )
{
	DCapsDef *sCaps = &dev.usbDev.Caps
	HWDef    *hw    = &dev.usbDev.HwSetting
	u_char   *regs  = dev.usbDev.a_bRegs
 ...
 */
static Bool
usb_AdjustLamps( Plustek_Device *dev, Bool on )
{
	HWDef  *hw   = &dev.usbDev.HwSetting
	u_char *regs = dev.usbDev.a_bRegs
	...
	return sanei_lm983x_write( dev.fd, 0x2c,
	                           &regs[0x2c], 0x37-0x2c+1, Sane.TRUE )
}

/**
 */
static void
usb_AdjustCISLampSettings( Plustek_Device *dev, Bool on )
{
	HWDef *hw = &dev.usbDev.HwSetting
 ...
 */
static void
usb_GetLampRegAndMask( u_long flag, Sane.Byte *reg, Sane.Byte *msk )
{
	if( _MIO6 == ( _MIO6 & flag )) {
		*reg = 0x5b
		*msk = 0x80
	...
	if( _WAF_MISC_IO_LAMPS & sc.workaroundFlag ) {
		...
		if( 0 == reg ) {
			...
			if((dev.usbDev.vendor == 0x04A9) && (dev.usbDev.product==0x2208)) {
				sanei_lm983x_read( dev.fd, 0x29, &regs[0x29], 3, Sane.TRUE )
				DBG( _DBG_INFO, "[29]=0x%02x, [2A]=0x%02x, [2B]=0x%02x\n",
				                regs[0x29], regs[0x2a], regs[0x2b] )
			}
		}
	} else {
		sanei_lm983x_read(dev.fd, 0x29,&regs[0x29],0x37-0x29+1,Sane.TRUE)
 ...
 */
static Bool
usb_switchLampX( Plustek_Device *dev, Bool on, Bool tpa )
{
	Sane.Byte reg, msk
	DCapsDef *sc   = &dev.usbDev.Caps
	u_char   *regs = dev.usbDev.a_bRegs
	...
	if( 0 == reg )
		return Sane.FALSE; /* no need to switch something */
 ...
 */
static Bool
usb_switchLamp( Plustek_Device *dev, Bool on )
{
	Bool result

	if((dev.scanning.sParam.bSource == SOURCE_Negative) ||
	   (dev.scanning.sParam.bSource == SOURCE_Transparency)) {
		result = usb_switchLampX( dev, on, Sane.TRUE )
	} else {
		result = usb_switchLampX( dev, on, Sane.FALSE )
	}
 ...
 */
static void
usb_LedOn( Plustek_Device *dev, Bool fOn )
{
	u_char value
 ...
 */
static Bool
usb_LampOn( Plustek_Device *dev, Bool fOn, Bool fResetTimer )
{
	DCapsDef      *sc          = &dev.usbDev.Caps
 ...
 */
			if( lampId != dev.usbDev.currentLamp ) {
				if( dev.usbDev.currentLamp == DEV_LampReflection )
					usb_switchLampX( dev, Sane.FALSE, Sane.FALSE )
				else
					usb_switchLampX( dev, Sane.FALSE, Sane.TRUE )
			}
			...
			if( !usb_switchLamp(dev, Sane.TRUE )) {
			...
			sanei_lm983x_write( dev.fd, 0x29,
			                    &regs[0x29], 0x37-0x29+1, Sane.TRUE )
			if( lampId != dev.usbDev.currentLamp ) {
		...
		if( iStatusChange != iLampStatus ) {
			...
			memset( &regs[0x29], 0, 0x37-0x29+1 )
			if( !usb_switchLamp(dev, Sane.FALSE )) {
	...
	if( usb_GetLampStatus(dev))
		usb_LedOn( dev, Sane.TRUE )
	else
		usb_LedOn( dev, Sane.FALSE )
	return Sane.TRUE
}
...
/* HEINER: CHECK WHY THIS has been disabled*/
	...
	if( dev.initialized >= 0 ) {
		...
		sanei_lm983x_read( dev.fd, 0x59, &regs[0x59], 3, Sane.TRUE )
		...
		if( usb_IsCISDevice( dev )) {
			...
			sanei_lm983x_write( dev.fd, 0x59, &regs[0x59], 3, Sane.TRUE )
		}
	}
...
/* HEINER: Maybe needed to avoid recalibration!!! */
	...
	if( value & 1 ) {
		...
		if( value ) {
			...
			sanei_lm983x_write( dev.fd, 0x58,
								&hw.bReg_0x58, 0x5b-0x58+1, Sane.TRUE )
			usbio_ReadReg( dev.fd, 2, &value )
			usbio_ReadReg( dev.fd, 2, &value )
		}

		usb_MotorOn( dev, Sane.FALSE )
		return Sane.TRUE
	}
	...
	if( !(value & 2)) {
		usb_ModuleToHome( dev, Sane.FALSE )
	}
 ...
 */
static void
usb_LampSwitch( Plustek_Device *dev, Bool sw )
{
	Int handle = -1
	...
	dev.scanning.fCalibrated = Sane.FALSE

	if( -1 != dev.fd )
		usb_LampOn( dev, sw, Sane.FALSE )
...
usb_LampTimerIrq( Int sig )
{
	...
	usb_LampSwitch( dev_xxx, Sane.FALSE )
}
