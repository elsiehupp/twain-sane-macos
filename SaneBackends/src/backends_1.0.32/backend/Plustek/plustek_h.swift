...
#define _ENABLE(option)  s.opt[option].cap &= ~Sane.CAP_INACTIVE
#define _DISABLE(option) s.opt[option].cap |=  Sane.CAP_INACTIVE
...
{
	Int               initialized;      /* device already initialized?  */
...
typedef struct Plustek_Scanner
{
	...
	Bool               ipc_read_done;  /* TRUE after ipc has been red   */
