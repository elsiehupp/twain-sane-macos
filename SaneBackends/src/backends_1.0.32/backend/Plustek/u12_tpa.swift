...
{
	Sane.Byte   bHi[3], bHiLeft[3], bHiRight[3]
	...
	u12io_DataToRegister( dev, REG_XSTEPTIME,
	                               (Sane.Byte)(dev.regs.RD_LineControl >> 4))
	_DODELAY( 12 )
	...
	dev.regs.RD_XStepTime      = (Sane.Byte)(dev.shade.wExposure)
	...
	if( dwR > dwG && dwR > dwB )
		dev.shade.bGainHigh = (Sane.Byte)dwR;     /* >> 4 for average, >> 4 to 8-bit */
	else {
		if( dwG > dwR && dwG > dwB )
			dev.shade.bGainHigh = (Sane.Byte)dwG
		else
			dev.shade.bGainHigh = (Sane.Byte)dwB
	}

	dev.shade.bGainHigh = (Sane.Byte)(dev.shade.bGainHigh - 0x18)
	dev.shade.bGainLow  = (Sane.Byte)(dev.shade.bGainHigh - 0x10)
