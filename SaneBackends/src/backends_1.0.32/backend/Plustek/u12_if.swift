...
/** for autodetection */
static Sane.Char USB_devname[1024]

/********************** the USB scanner interface ****************************/

/**
 */
static Sane.Status u12_initDev( U12_Device *dev, Int handle, Int vendor )
{
	Int         i
	Sane.Status res
	TimerDef    timer
	...
	if( !(u12io_DataFromRegister( dev, REG_STATUS ) & _FLAG_PAPER)) {
		...
		res = u12hw_InitAsic( dev, Sane.FALSE )
	}
 ...
 */
static Bool u12if_IsDeviceSupported( U12_Device *dev )
{
	var i: Int
 ...
 * @return Sane.TRUE if a matching device has been found or
 *         Sane.FALSE if nothing supported found...
 */
static Bool usbDev_autodetect( Sane.Word *vendor, Sane.Word *product )
{
	Int       i
	Sane.Word p, v
...
static Int u12if_open( U12_Device *dev )
{
	...
	if( !strcmp( dev.name, "auto" )) {
		...
		if( Sane.STATUS_GOOD != sanei_usb_open( USB_devname, &handle )) {
			return -1
		}
		...
		if( Sane.STATUS_GOOD != sanei_usb_open( dev.name, &handle ))
    		return -1
	}
	was_empty = Sane.FALSE
	...
	/* is it accessible ? */
	if( Sane.STATUS_GOOD != u12hw_CheckDevice( dev )) {
		dev.fd = -1
		sanei_usb_close( handle )
		return -1
	}
...
/**
 */
static Sane.Status u12if_getCaps( U12_Device *dev )
{
	Int cntr
...
/**
 */
static Sane.Status u12if_startScan( U12_Device *dev )
{
...
/**
 */
static Sane.Status u12if_stopScan( U12_Device *dev )
{
	DBG( _DBG_INFO, "u12if_stopScan()\n" )

#if 0
	u12motor_ToHomePosition( dev, Sane.FALSE )
#else
#if 0
	u12motor_ToHomePosition( dev, Sane.TRUE )
...
/**
 */
static Sane.Status u12if_prepare( U12_Device *dev )
{
	Sane.Status res
...
/**
 */
static Sane.Status u12if_readLine( U12_Device *dev, Sane.Byte *line_buffer )
{
	Sane.Status res
...
/**
 */
static Sane.Status u12if_SetupBuffer( U12_Device *dev )
{
	void *buffer
	...
	dev.bufs.TpaBuf.pb   = &((Sane.Byte*)dev.bufs.b2.pSumBuf)[_SIZE_SHADING_SUM_BUF]
