...
struct scanner
{
  ...
  /*mode group*/
  Sane.String_Const o_source_list[4]
  Sane.String_Const o_mode_list[5]
  Int o_res_list[4][6]

  /*geometry group*/
  Sane.Range o_tl_x_range
  Sane.Range o_tl_y_range
  Sane.Range o_br_x_range
  Sane.Range o_br_y_range
  Sane.Range o_page_x_range
  Sane.Range o_page_y_range

  /*enhancement group*/
  Sane.Range o_brightness_range
  Sane.Range o_contrast_range
  Sane.Range o_threshold_range
...
static Sane.Status read_from_scanner(struct scanner *s)
...
/* values for Sane.DEBUG_KODAK env var:
 - errors           5
 - function trace  10
 - function detail 15
 - get/setopt cmds 20
 ...
 */
/* Sane.Parameters is defined as a struct containing:
	Sane.Frame format
	Bool last_frame
	Int lines
...
Sane.get_parameters(Sane.Handle handle, Sane.Parameters * params)
{
    ...
    if(s.started){
	    ...
	    DBG(5,"Sane.get_parameters: unsupported depth %d\n", s.i_bpp)
	    return Sane.STATUS_INVAL
        }
    }
...
static Sane.Status
read_from_buffer(struct scanner *s, Sane.Byte * buf,
  Int max_len, Int * len)
{
    Sane.Status ret=Sane.STATUS_GOOD
    Int bytes = max_len
 ...
 * decides the best Sane.Status for the problem
