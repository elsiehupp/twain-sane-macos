...
{				/* HS2P_REQUEST_SENSE_DATA  */
  /* bit7:valid is 1 if information byte is valid,
     bits6:0 error_code */
  Sane.Byte error_code

  /* not used, set to 0 */
  Sane.Byte segment_number
  ...
  Sane.Byte information[4]

  /* fixed at 6 */
  Sane.Byte sense_length

  /* not used and set to 0 */
  Sane.Byte command_specific_information[4]
  Sane.Byte sense_code
  Sane.Byte sense_code_qualifier
} SENSE_DATA
...
struct inquiry_standard_data
{
   ...
   */
  Sane.Byte devtype;		/* must be 0x06 */
  ...
  Sane.Byte vendor[8];		/* vendor_id="RICOH   " */
  Sane.Byte product[16];	/* product_id="IS450           " */
  Sane.Byte revision[4];	/* product_revision_level="xRxx" where x indicate firmware version number */
]

/* VPD Information [EVPD=0x01, PageCode=0xC0] */
struct inquiry_vpd_data
{
  Sane.Byte devtype;		/* bits7-5: Peripheral Qualifier
				 * bits4-0: Peripheral Device Type */
  Sane.Byte pagecode;		/* Page Code  => 0xC0 */
  Sane.Byte byte2;		/* Reserved */
  Sane.Byte pagelength;		/* Page Length => 12 (0x0C) */
  Sane.Byte adf_id;		/* ADF Identification
				 * 0: No ADF is mounted
				 ...
				 */
  Sane.Byte end_id;		/* Endorser Identification
				 * 0: No endorser
				 * 1: Endorser mounted
				 * 2: Reserved
				 * It should be 0 or 1 with this scanner
				 */
  Sane.Byte ipu_id;		/* Image Processing Unit Identification
				 * bits 7:2   Reserved
				 ...
				 */
  Sane.Byte imagecomposition;	/* indicates supported image data type.
				 * This is set to 0x37
				 ...
				 * bit7 => Reserved
				 */
  Sane.Byte imagedataprocessing[2];	/* Image Data Processing Method
					 ...
					 * Byte15 => Reserved
					 */
  Sane.Byte compression;	/* Compression Method is set to 0x00
				 ...
				 * bit4 => Reserved
				 */
  Sane.Byte markerrecognition;	/* Marker Recognition Method is set to 0x00
				 * bit0    => Marker Recognition supported ? 1:0
				 * bits1-7 => Reserved
				 */
  Sane.Byte sizerecognition;	/* Size Detection
				 * bit0    => Size Detection Supported ? 1:0
				 * bits1-7 => Reserved
				 */
  Sane.Byte byte13;		/* Reserved */
  Sane.Byte xmaxoutputpixels[2];	/* X Maximum Output Pixel is set to 4960 (0x1360)
...
{				/* JIS INFORMATION  VPD_IDENTIFIER_F0H */
  Sane.Byte devtype;		/* 7-5: peripheral qualifier, 4-0: peripheral device type */
  Sane.Byte pagecode
  Sane.Byte jisversion
  Sane.Byte reserved1
  Sane.Byte alloclen;		/* page length: Set to 25 (19H)  */
  struct
  {
    Sane.Byte x[2];		/* Basic X Resolution: Set to 400 (01H,90H) */
    Sane.Byte y[2];		/* Basic Y Resolution: Set to 400 (01H,90H) */
  } BasicRes
  Sane.Byte resolutionstep;	/* 7-4: xstep, 3-0 ystep: Both set to 1 (11H) */
  struct
  {
    Sane.Byte x[2];		/* Maximum X resolution: Set to 800 (03H,20H) */
    Sane.Byte y[2];		/* Maximum Y resolution: Set to 800 (03H,20H) */
  } MaxRes
  struct
  {
    Sane.Byte x[2];		/* Minimum X resolution: Set to 100 (00H,64H) */
    Sane.Byte y[2];		/* Minimum Y resolution */
  } MinRes
  Sane.Byte standardres[2];	/* Standard Resolution: bits 7-0:
				 * byte18:  60, 75,100,120,150,160,180, 200
				 * byte19: 240,300,320,400,480,600,800,1200
				 */
  struct
  {
    Sane.Byte width[4];		/* in pixels based on basic resolution. Set to 4787 (12B3H) */
    Sane.Byte length[4];	/* maximum number of scan lines based on basic resolution. Set to 6803 (1A93H) */
  } Window
  Sane.Byte functions;		/* This is set to 0EH: 0001110
...
{
  Sane.Byte opcode;		/* 15H                                                 */
  Sane.Byte byte1;		/* 7-5:LUN; 4:PF; 2:Reserved; 1:SP
				 * Save Page Bit must be 0 since pages cannot be saved
				 * Page Format Bit must be 1                           */
  Sane.Byte reserved[2]
  Sane.Byte len;		/* Parameter List Length                               */
...
{
  Sane.Byte data_len;		/* Mode Data Length          NOT USED so must be 0 */
  Sane.Byte medium_type;	/* Medium Type               NOT USED so must be 0 */
  Sane.Byte dev_spec;		/* Device Specific Parameter NOT USED so must be 0 */
  Sane.Byte blk_desc_len;	/* Block Descriptor Length             is set to 0 */
} MPHdr
...
typedef struct mode_page_connect
{
  ...
  Sane.Byte code;		/* 7-6:Reserved; 5-0: 02H  */
  Sane.Byte len;		/* Parameter Length 0EH    */
  Sane.Byte buffer_full_ratio;	/* Ignored                 */
  Sane.Byte buffer_empty_ratio;	/* Ignored                 */
  Sane.Byte bus_inactive_limit[2];	/* Ignored                 */
  Sane.Byte disconnect_time_limit[2];	/* indicates minimum time to disconnect SCSI bus until reconnection.
					 * It is expressed in 100msec increments; i.e. "1" for 100msec, "2" for 200msec
					 * The maximum time is 2sec */
  Sane.Byte connect_time_limit[2];	/* Ignored                  */
  Sane.Byte maximum_burst_size[2];	/* expressed in 512 increments, i.e. "1" for 512 bytes, "2" for 1024 bytes
					 * "0" indicates unlimited amount of data */
  Sane.Byte dtdc;		/* 7-2:Reserved; 1-0:DTDC indicates limitations of disconnection (bit1,bit0):
...
{
  MPHdr hdr;			/* Mode Page Header: 4 bytes         */
  Sane.Byte code;		/* 7-6:Reserved; 5-0:Page Code (03H) */
  Sane.Byte len;		/* Parameter Length (06H)            */
  Sane.Byte bmu;		/* Basic Measurement Unit            */
  Sane.Byte reserved0
  Sane.Byte mud[2];		/* Measurement Unit Divisor
...
typedef struct mode_page_white_balance
{
  ...
  Sane.Byte code;		/* 7-6:Reserved; 5-0:Page Code (03H) */
  Sane.Byte len;		/* Parameter Length (06H)            */
  Sane.Byte white_balance;	/* "0" selects relative white mode (DEFAULT when power on)
				 * "1" selects absolute white mode */
  Sane.Byte reserved[5]
} MP_WhiteBal;			/* White Balance */

typedef struct mode_page_lamp_timer_set
{
  MPHdr hdr;			/* Mode Page Header: 4 bytes            */
  Sane.Byte code;		/* 7-6:Reserved; 5-0:Page Code (22H)    */
  Sane.Byte len;		/* Parameter Length (06H)               */
  Sane.Byte time_on;		/* indicates the time of lamp turned on */
  Sane.Byte ignored[5]
...
{
  MPHdr hdr;			/* Mode Page Header: 4 bytes            */
  Sane.Byte code;		/* 7-6:Reserved; 5-0:Page Code (26H)    */
  Sane.Byte len;		/* Parameter Length (06H)               */
  Sane.Byte adf_control;	/* 7-2:Reserved; 1-0:ADF selection:
				 * 00H Book Mode (DEFAULT when power on)
				 * 01H Simplex ADF
				 * 02H Duplex ADF
				 * 03H-FFH Reserved                     */
  Sane.Byte adf_mode_control;	/* 7-3:Reserved; 2:Prefeed Mode Validity 1-0:Ignored
				 * Prefeed Mode "0" means invalid, "1" means valid */
  Sane.Byte medium_wait_timer;	/* indicates time for scanner to wait for media. Scanner
...
{
  MPHdr hdr;			/* Mode Page Header: 4 bytes            */
  Sane.Byte code;		/* 7-6:Reserved; 5-0:Page Code (27H)    */
  Sane.Byte len;		/* Parameter Length (06H)               */
  Sane.Byte endorser_control;	/* 7-3:Reserved; 2-0:Endorser Control:
...
{
  MPHdr hdr;			/* Mode Page Header: 4 bytes            */
  Sane.Byte code;		/* 7-6:Reserved; 5-0:Page Code (2BH)    */
  Sane.Byte len;		/* Parameter Length (06H)               */
  Sane.Byte swm;		/* 7-1:Reserved; 0:Scan Wait Mode
...
{				/* Selectable when Send Diagnostic command is performed */
  MPHdr hdr;			/* Mode Page Header: 4 bytes            */
  Sane.Byte code;		/* 7-6:Reserved; 5-0:Page Code (3EH)    */
  Sane.Byte len;		/* Parameter Length (06H)               */
  Sane.Byte service;		/* 7-1:Reserved; 0:Service Mode
...
{
  Sane.Byte opcode;		/* 1AH */
  Sane.Byte dbd;		/* 7-5:LUN; 4:Reserved; 3:DBD (Disable Block Description) set to "0"; 2-0:Reserved */
  Sane.Byte pc;			/* 7-6:PC; 5-0:Page Code
  ...
  Sane.Byte reserved
  Sane.Byte len;		/* Allocation length */
  Sane.Byte control;		/* 7-6:Vendor Unique; 5-2:Reserved; 1:Flag; 0:Link */
} SENSE
...
{
  Sane.Byte opcode;		/* 1BH                   */
  Sane.Byte byte1;		/* 7-5:LUN; 4-0:Reserved */
  Sane.Byte page_code
  Sane.Byte reserved
  Sane.Byte len;		/* Transfer Length
...
static Sane.String_Const noisematrix_list[] = {
...
static Sane.String_Const grayfilter_list[] = {
...
static Sane.String_Const auto_separation_list[] = {
  "Off", "On", "User", NULL
]
...
static Sane.String_Const auto_binarization_list[] = {
  "Off",
  "On",
...
static Sane.String_Const halftone_code[] = {
...
{				/* 32 bytes */
  ...
  Sane.Byte width[4]
  Sane.Byte length[4]
  Sane.Byte binary_filtering
  Sane.Byte ignored1
  Sane.Byte threshold
  Sane.Byte ignored2
  Sane.Byte image_composition
  Sane.Byte halftone_id
  Sane.Byte halftone_code
  Sane.Byte ignored3[7]
]
/* 1-3-11 SET WINDOW COMMAND
 * Byte0: 24H
...
{				/* HS2P_WINDOW_DATA_FORMAT       */
  Sane.Byte window_id;		/*     0: Window Identifier      */
  Sane.Byte auto_bit;		/*     1: 1-1:Reserved; 0:Auto   */
  Sane.Byte xres[2];		/*   2-3: X-Axis Resolution      100-800dpi in 1dpi steps */
  Sane.Byte yres[2];		/*   4-5: Y-Axis Resolution      100-800dpi in 1dpi steps */
  Sane.Byte ulx[4];		/*   6-9: X-Axis Upper Left      */
  Sane.Byte uly[4];		/* 10-13: Y-Axis Upper Left      */
  Sane.Byte width[4];		/* 14-17: Window Width           */
  Sane.Byte length[4];		/* 18-21: Window Length          */
  Sane.Byte brightness;		/*    22: Brightness  [0-255] dark-light 0 means default value of 128 */
  Sane.Byte threshold;		/*    23: Threshold   [0-255] 0 means default value of 128            */
  Sane.Byte contrast;		/*    24: Contrast    [0-255] low-high   0 means default value of 128 */
  Sane.Byte image_composition;	/*    25: Image Composition
				 *        00H Lineart
				 *        01H Dithered Halftone
				 *        02H Gray scale
				 */
  Sane.Byte bpp;		/*    26: Bits Per Pixel         */
  Sane.Byte halftone_code;	/*    27: Halftone Code
				 *        00H-01H Reserved
				 *        02H Dither (partial Dot)
				 *        03H Error Diffusion
				 *        04H-07H Reserved
				 */
  Sane.Byte halftone_id;	/*    28: Halftone ID
				 *        00H Reserved
				 *        01H 8x4, 45 degree
				 *        02H 6x6, 90 degree
				 *        03H 4x4, Spiral
				 ...
				 *        82H-FFH Reserved
				 */
  Sane.Byte byte29;		/*    29:   7: RIF (Reverse Image Format) bit inversion
				 ...
				 *             04H-FFH Reserved
				 */
  Sane.Byte bit_ordering[2];	/* 30-31: Bit Ordering: Default 0xF8
				 ...
				 *     8-15: reserved
				 */
  Sane.Byte compression_type;	/*    32: Compression Type:     Unsupported in IS450   */
  Sane.Byte compression_arg;	/*    33: Compression Argument: Unsupported in IS450   */
  Sane.Byte reserved2[6];	/* 34-39: Reserved               */
  Sane.Byte ignored1;		/*    40: Ignored                */
  Sane.Byte ignored2;		/*    41: Ignored                */
  Sane.Byte byte42;		/*    42:   7: MRIF: Grayscale Reverse Image Format
				 ...
				 *             09H-0FH Reserved
				 */
  Sane.Byte ignored3;		/*    43: Ignored                */
  Sane.Byte ignored4;		/*    44: Ignored                */
  Sane.Byte binary_filtering;	/*    45: Binary Filtering
   ...
   */
  Sane.Byte ignored5;		/*    46: Ignored                       */
  Sane.Byte ignored6;		/*    47: Ignored                       */
  Sane.Byte automatic_separation;	/*    48: Automatic Separation
					 *            00H OFF
					 *            01H Default
					 *        02H-7FH Reserved
					 *            80H Download table
					 *        91H-FFH Reserved
					 */
  Sane.Byte ignored7;		/*    49: Ignored                       */
  Sane.Byte automatic_binarization;	/*    50: Automatic Binarization
					 *            00H OFF
					 *            01H Default
					 ...
					 *        81H-FFH Reserved
					 */
  Sane.Byte ignored8[13];	/* 51-63: Ignored                       */
...
{
  Sane.Byte opcode;		/* 24H */
  Sane.Byte byte2;		/* 7-5:LUN 4-0:Reserve */
  Sane.Byte reserved[4];	/* Reserved */
  Sane.Byte len[3];		/* Transfer Length */
  Sane.Byte control;		/* 76543210
				 * XX       Vendor Unique
				 *   XXXX   Reserved
				 *       X  Flag
				 *        X Link
...
{
  Sane.Byte opcode
  Sane.Byte byte1;		/* 7-5: LUN; * 4-1:Reserved; *   0:Single bit is 0 */
  Sane.Byte reserved[3]
  Sane.Byte win_id;		/* Window ID is either 0 or 1 */
  Sane.Byte len[3];		/* Transfer Length */
  Sane.Byte control;		/* 7-6:Vendor Unique; 5-2:Reserved; 1:Flag; 0:Link */
]
...
{
  Sane.Byte data_len[2];	/* Window Data Length indicates byte len of data which follows less its own 2 bytes */
  Sane.Byte reserved[4]
  Sane.Byte desc_len[2];	/* Window Descriptor Length indicates byte length of one Window Descriptor which is 640 */
]
...
{
  Sane.Byte opcode;		/* READ=28H  SEND=2AH                    */
  Sane.Byte byte1;		/* 7-5:LUN; 4-0:Reserved                 */
  Sane.Byte dtc;		/* Data Type Code: See DTC DEFINES above */
  Sane.Byte reserved
  Sane.Byte dtq[2];		/* Data Type Qualifier valid only for DTC 02H,03H,93H */
  Sane.Byte len[3];		/* Transfer Length */
  Sane.Byte control;		/* 7-6:Vendor Unique; 5-2:Reserved; 1:Flag; 0:Link */
]
/*
...
{
  Sane.Byte nregx_adf;		/* number of registers of main-scanning in ADF mode */
  Sane.Byte nregy_adf;		/* number of registers of sub-scanning  in ADF mode */
  Sane.Byte nregx_book;		/* number of registers of main-scanning in Book mode */
  Sane.Byte nregy_book;		/* number of registers of sub-scanning  in Book mode */
  Sane.Byte nscans_adf[4];	/* Number of scanned pages in ADF mode */
  Sane.Byte nscans_book[4];	/* Number of scanned pages in Book mode */
  Sane.Byte lamp_time[4];	/* Lamp Time */
  Sane.Byte eo_odd;		/* Adjustment data of E/O balance in black level (ODD) */
  Sane.Byte eo_even;		/* Adjustment data of E/O balance in black level (EVEN) */
  Sane.Byte black_level_odd;	/* The adjustment data in black level (ODD) */
  Sane.Byte black_level_even;	/* The adjustment data in black level (EVEN) */
  Sane.Byte white_level_odd[2];	/* The adjustment data in white level (ODD) */
  Sane.Byte white_level_even[2];	/* The adjustment data in white level (EVEN) */
  Sane.Byte first_adj_white_odd[2];	/* First adjustment data in white level (ODD) */
  Sane.Byte first_adj_white_even[2];	/* First adjustment data in white level (EVEN) */
  Sane.Byte density_adj;	/* Density adjustment */
  Sane.Byte nregx_reverse;	/* The number of registers of main-scanning of the reverse-side ADF */
  Sane.Byte nregy_reverse;	/* The number of registers of sub-scanning of the reverse-side ADF */
  Sane.Byte nscans_reverse_adf[4];	/* Number of scanned pages of the reverse side ADF */
  Sane.Byte reverse_time[4];	/* The period of lamp turn on of the reverse side */
  Sane.Byte nchars[4];		/* The number of endorser characters */
  Sane.Byte reserved0
  Sane.Byte reserved1
  Sane.Byte reserved2
  Sane.Byte zero[2];		/* All set as 0 */
} MAINTENANCE_DATA
/* ADF status 1byte:
 * 7-3:Reserved
 *   2:Reserved
...
struct IPU
{
  Sane.Byte byte0;		/* 7-4:Reserved; 3:White mode; 2:Reserved; 1-0: Gamma Table Select */
  Sane.Byte byte1;		/* 7-2:Reserved; 1-0: MTF Filter Select */
]
...
{
  Sane.Byte opcode;		/* 31H */
  Sane.Byte position_func;	/* 7-5:LUN; 4-3:Reserved; 2-0:Position Function (bit2,bit1,bit0):
...
typedef struct scsi_get_data_buffer_status_cmd
{
  ...
  Sane.Byte wait;		/* 7-5:LUN; 4-1:Reserved; 0: Wait bit is "0" */
  Sane.Byte reserved[5]
  Sane.Byte len[2];		/* Allocation Length */
  Sane.Byte control;		/* 7-6:Vendor Unique; 5-2:Reserved; 1:Flag; 0:Link */
} GET_DBS_CMD
typedef struct scsi_status_hdr
{
  Sane.Byte len[3];		/* Data Buffer Status Length */
  Sane.Byte block;		/* 7-1:Reserved; 0:Block bit is 0 */
} STATUS_HDR
typedef struct scsi_status_data
{
  Sane.Byte wid;		/* window identifier is 0 or 1 */
  Sane.Byte reserved
...
{
  Sane.Status status
  DBG (DBG_Sane.proc, ">> get_sense_data\n")
...
print_sense_data (Int dbg_level, SENSE_DATA * data)
{
  ...
  DBG (DBG_Sane.proc, ">> print_sense_data\n")

  bp = (Sane.Byte *) data
  end = bp + (Sane.Byte) sizeof (SENSE_DATA)
  for (i = 0; bp < end; bp++, i++)
    {
...
*/
static Sane.Status
sense_handler (Int __Sane.unused__ scsi_fd, u_char * sense_buffer, void *sd)
{
   ...
   * Sane.STATUS_GOOD, _ACCESS_DEINIED, _NO_MEM, _INVAL, _IO_ERROR, _DEVICE_BUSY,
...
*/
static Sane.Status
inquiry (Int fd, void *buf, size_t * buf_size, Sane.Byte evpd,
	 Sane.Byte page_code)
{
  static Sane.Byte cmd[6]
  Sane.Status status
...
static Sane.Status
mode_sense (Int fd, MP * buf, Sane.Byte page_code)
{
  Sane.Status status
  SENSE cmd;			/* 6byte cmd */
...
static Sane.Status
read_data (Int fd, void *buf, size_t * buf_size, Sane.Byte dtc, u_long dtq)
{
...
is_valid_endorser_character (char c)
{
  ...
  if (i >= 0x41 && i <= 0x5A)
    return Sane.TRUE;		/* ABCDEFGHIJKLMNOPQRSTUVWXYZ <spaces> */
  if (i >= 0x61 && i <= 0x7A)
    return Sane.TRUE;		/* abcdefghijklmnopqrstuvwxyz <spaces> */
...
static Sane.Status
set_endorser_string (Int fd, String s)
{
  struct
  {
    struct scsi_rs_scanner_cmd cmd
    Sane.Byte endorser[19]
  } out
  char *t
  var i: Int, len
...
hs2p_send_gamma (HS2P_Scanner * s)
{
  ...
  {
    struct scsi_rs_scanner_cmd cmd
    Sane.Byte gamma[2 + GAMMA_LENGTH]
  } out
  var i: Int
...
#if 0
static Sane.Status
read_halftone_mask (Int fd, Sane.Byte halftone_id, void *buf,
		    size_t * buf_size)
{
  static struct scsi_rs_scanner_cmd cmd
  Sane.Status status
  ...
  switch (halftone_id)
    ...
    case 0x0A:
      ...
      break;			/* 8x8, Bayer */
    default:
      return Sane.STATUS_INVAL;	/* Reserved */
    }
...
#if 0
static Sane.Status
set_halftone_mask (Int fd, Sane.Byte halftone_id, void *buf,
		   size_t * buf_size)
{
  static struct scsi_rs_scanner_cmd cmd
  Sane.Status status
...
#if 0
static Sane.Status
read_adf_status (Int fd, Sane.Byte * adf_status_byte)
{
  Sane.Status status = Sane.STATUS_GOOD
  struct scsi_rs_scanner_cmd cmd
  static size_t len = 1
...
*/
static Sane.Status
unit_cmd (Int fd, Sane.Byte opcode)
{
  static struct
  {
    Sane.Byte opcode;		/* 16H: Reserve Unit    17H: Release Unit                     */
    Sane.Byte byte1;		/* 7-5: LUN; 4: 3rd Party; 3-1: 3rd Party Device; 0: Reserved */
    Sane.Byte reserved[3]
    Sane.Byte control;		/* 7-6: Vendor Unique; 5-2: Reserved; 1: Flag; 0: Link        */
  } cmd

  Sane.Byte LUN = (0x00 & 0x07) << 5
  Sane.Status status
...
trigger_scan (HS2P_Scanner * s)
{
  ...
  {
    START_SCAN cmd
    Sane.Byte wid[2];		/* scanner supports up to 2 windows */
  } scan
  Sane.Status status
   ...
   */
  scan.cmd.len = (s.val[OPT_DUPLEX].w == Sane.TRUE) ? 2 : 1
...
/* MODE PAGES GET/SET */
static Sane.Status
connection_parameters (Int fd, MP_CXN * settings, Bool flag)
{
  Sane.Status status
  MP_CXN buf
  size_t nbytes
  ...
  if (flag)
    {				/* GET */
      ...
      status =
	mode_sense (fd, (MP *) & buf, (Sane.Byte) PAGE_CODE_CONNECTION)
      if (status != Sane.STATUS_GOOD)
	{
	  DBG (DBG_error,
...
static Sane.Status
get_basic_measurement_unit (Int fd, Int * bmu, Int * mud)
{
  Sane.Status status
  MP_SMU buf
  ...
  status =
    mode_sense (fd, (MP *) & buf,
		(Sane.Byte) PAGE_CODE_SCANNING_MEASUREMENTS)
  if (status != Sane.STATUS_GOOD)
    {
      DBG (DBG_error,
...
static Sane.Status
set_basic_measurement_unit (Int fd, Sane.Byte bmu)
{
...
static Sane.Status
adf_control (Int fd, Bool flag, Sane.Byte * adf_control,
	     Sane.Byte * adf_mode, Sane.Byte * mwt)
{
  Sane.Status status
  MP_ADF buf
  size_t bufsize = sizeof (buf)
  ...
  if (flag)
    {				/* GET */
      ...
      status =
	mode_sense (fd, (MP *) & buf, (Sane.Byte) PAGE_CODE_ADF_CONTROL)
      if (status != Sane.STATUS_GOOD)
	{
...
static Sane.Status
white_balance (Int fd, Int *val, Bool flag)
{
  Sane.Status status
  ...
  if (flag)
    {				/* GET */
      ...
      status =
	mode_sense (fd, (MP *) & buf, (Sane.Byte) PAGE_CODE_WHITE_BALANCE)
      if (status != Sane.STATUS_GOOD)
	{
	  DBG (DBG_error,
...
#if 0
static Int
lamp_timer (Int fd, Int val, Bool flag)
{
  Sane.Status status
  ...
  if (flag)
    {				/* GET */
      ...
      if ((status =
	   mode_sense (fd, (MP *) & buf,
		       (Sane.Byte) PAGE_CODE_LAMP_TIMER_SET)) !=
	  Sane.STATUS_GOOD)
	{
    ...
    {				/* SET */
      ...
      if ((status = mode_select (fd, (MP *) & buf)) != Sane.STATUS_GOOD)
	{
	  DBG (DBG_error,
...
static Sane.Status
endorser_control (Int fd, Int *val, Bool flag)
{
  Sane.Status status
  MP_EndCtrl buf;		/* MPHdr (4bytes) + MPP (8bytes)       */
  Sane.Byte mask = 0x7;		/* 7-3:reserved; 2-0: Endorser Control */
  size_t bufsize = sizeof (buf)
  ...
  if (flag)
    {				/* GET */
      ...
      status =
	mode_sense (fd, (MP *) & buf, (Sane.Byte) PAGE_CODE_ENDORSER_CONTROL)
      if (status != Sane.STATUS_GOOD)
	{
	  DBG (DBG_error,
...
static Sane.Status
scan_wait_mode (Int fd, Int val, Bool flag)
{
  Sane.Status status
  ...
  if (flag)
    {				/* GET */
      ...
      status =
	mode_sense (fd, (MP *) & buf, (Sane.Byte) PAGE_CODE_SCAN_WAIT_MODE)
      if (status != Sane.STATUS_GOOD)
	{
	  DBG (DBG_error,
...
static Int
service_mode (Int fd, Int val, Bool flag)
{
  Sane.Status status
  ...
  if (flag)
    {				/* GET */
      ...
      status =
	mode_sense (fd, (MP *) & buf,
		    (Sane.Byte) PAGE_CODE_SERVICE_MODE_SELECT)
      if (status != Sane.STATUS_GOOD)
	{
	  DBG (DBG_error,
