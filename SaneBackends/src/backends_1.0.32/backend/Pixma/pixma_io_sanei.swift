...
static Sane.Status
attach_bjnp(Sane.String_Const devname,
...
static Sane.Status
get_string_descriptor(Int dn, Int index, Int lang,
		       Int length, Sane.Byte * data)
{
...
read_serial_number(scanner_info_t * si)
{
  ...
  if(Sane.STATUS_GOOD != sanei_usb_open(si.devname, &usb))
    return
  ...
  if(iSerialNumber != 0)
    {
      ...
      if(get_string_descriptor(usb, 0, 0, 4, unicode) != Sane.STATUS_GOOD)
        goto done
...
map_error(Sane.Status ss)
{
  switch(ss)
    ...
    case Sane.STATUS_HW_LOCKED:       /* unused by pixma */
#endif
#ifdef Sane.STATUS_WARMING_UP
    case Sane.STATUS_WARMING_UP:      /* unused by pixma */
#endif
      break
    }
...
pixma_collect_devices(const char **conf_devices,
                       const struct pixma_config_t *const pixma_devices[], Bool local_only)
{
  unsigned i, j
...
func Int pixma_connect(unsigned devnr, pixma_io_t ** handle)
{
  pixma_io_t *io
  Int dev
...
func Int pixma_reset_device(pixma_io_t * io)
{
  UNUSED(io)
  return PIXMA_ENOTSUP
}

func Int pixma_write(pixma_io_t * io, const void *cmd, unsigned len)
{
  size_t count = len
  Int error
...
func Int pixma_read(pixma_io_t * io, void *buf, unsigned size)
{
  size_t count = size
  Int error
...
func Int pixma_wait_interrupt(pixma_io_t * io, void *buf, unsigned size, Int timeout)
{
  size_t count = size
  Int error
...
func Int pixma_set_interrupt_mode(pixma_io_t * s, Int background)
{
  UNUSED(s)
