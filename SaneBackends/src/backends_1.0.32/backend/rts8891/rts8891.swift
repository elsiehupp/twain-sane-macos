/* sane - Scanner Access Now Easy.
   ...
   - interface is released at the end of Sane.open() to allow
   	multiple usage by fronteds
   - if free, interface is claimed at Sane.start(), and held until
   	Sane.cancel() is called
...
import rts8891
import rts88xx_lib
import rts8891_low.c"
import rts8891_devices.c"
 ...
 * opened device. Sessions are inserted here on Sane.open() and
 * removed on Sane.close().
 */
...
static const Sane.Range sensor_range = {
  0,				/* minimum */
...
compute_parameters(Rts8891_Session * session)
{
  ...
  dev.lines += (dev.lds_max + dev.ripple) / dev.bytesPerLine
  ...
  dev.ystart -= (dev.lds_max + dev.ripple) / dev.bytesPerLine
  ...
  dev.read = 0
  dev.to_read = dev.lines * dev.bytesPerLine
  ...
  if(dev.xdpi < dev.model.max_ydpi)
    {
      dev.data_size =
	(PREFERED_BUFFER_SIZE / dev.bytesPerLine) * (dev.bytesPerLine)
    }
  else
    {
      dev.data_size =
	(((PREFERED_BUFFER_SIZE / 2) - dev.lds_max -
	  dev.ripple) / dev.bytesPerLine) * dev.bytesPerLine
    }

  /* 32 lines minimum in buffer */
  if(dev.data_size < 32 * dev.bytesPerLine)
    dev.data_size = 32 * dev.bytesPerLine
  ...
  DBG(DBG_data, "compute_parameters: dev extra lines   =%d\n",
       (dev.lds_max + dev.ripple) / dev.bytesPerLine)
	   ...
	   Int max_len, Int * len)
{
  ...
  *len = 0
  ...
  length = (session.params.bytesPerLine * 8) / session.params.depth
	     ...
	     && session.sent < session.to_send)
	{
	  ...
	  if(dev.xdpi == dev.model.max_xdpi)
	      ...
	      if(session.emulated_gray == Sane.TRUE)
		{
		  ...
		  /* at a time                                                        */
		  switch(((session.sent * 3) % dev.bytesPerLine) % 6)
		    {
		    case 0:
		    case 1:
		    case 2:
		...
		{
		  switch((session.sent % dev.bytesPerLine) % 6)
		    {
		    case 0:
		  ...
		  && (session.sent % length == 0))
		{
		  DBG(DBG_io,
		       ...
		       dev.bytesPerLine, session.sent)
		  dev.current -= dev.bytesPerLine
		}
	    }
	      ...
	      else
		{
	      ...
	      if((dev.current - dev.start) % dev.bytesPerLine == 0)
		dev.current += dev.bytesPerLine
	    }
	}
    }
...
attach_rts8891 (const char *devicename)
{
  ...
  device.needs_warming = Sane.TRUE
...
init_options(struct Rts8891_Session *session)
{
  ...
  session.opt[OPT_NUM_OPTS].name = Sane.NAME_NUM_OPTIONS
  session.opt[OPT_NUM_OPTS].title = Sane.TITLE_NUM_OPTIONS
  session.opt[OPT_NUM_OPTS].desc = Sane.DESC_NUM_OPTIONS
  session.opt[OPT_NUM_OPTS].type = Sane.TYPE_INT
  session.opt[OPT_NUM_OPTS].cap = Sane.CAP_SOFT_DETECT
  session.val[OPT_NUM_OPTS].w = NUM_OPTIONS

  /* "Standard" group: */
  session.opt[OPT_STANDARD_GROUP].title = Sane.TITLE_STANDARD
  session.opt[OPT_STANDARD_GROUP].name = Sane.NAME_STANDARD
  session.opt[OPT_STANDARD_GROUP].desc = Sane.DESC_STANDARD
  session.opt[OPT_STANDARD_GROUP].type = Sane.TYPE_GROUP
  session.opt[OPT_STANDARD_GROUP].size = 0
  session.opt[OPT_STANDARD_GROUP].cap = 0
  session.opt[OPT_STANDARD_GROUP].constraint_type = Sane.CONSTRAINT_NONE

  /* scan mode */
  session.opt[OPT_MODE].name = Sane.NAME_SCAN_MODE
  session.opt[OPT_MODE].title = Sane.TITLE_SCAN_MODE
  session.opt[OPT_MODE].desc = Sane.DESC_SCAN_MODE
  ...
  session.opt[OPT_PREVIEW].title = Sane.TITLE_PREVIEW
  session.opt[OPT_PREVIEW].desc = Sane.DESC_PREVIEW
  session.opt[OPT_PREVIEW].type = Sane.TYPE_BOOL
  session.opt[OPT_PREVIEW].cap |= Sane.CAP_AUTOMATIC
  session.opt[OPT_PREVIEW].unit = Sane.UNIT_NONE
  session.opt[OPT_PREVIEW].constraint_type = Sane.CONSTRAINT_NONE
  session.val[OPT_PREVIEW].w = Sane.FALSE
  ...
  session.opt[OPT_RESOLUTION].title = Sane.TITLE_SCAN_RESOLUTION
  session.opt[OPT_RESOLUTION].desc = Sane.DESC_SCAN_RESOLUTION
  session.opt[OPT_RESOLUTION].type = Sane.TYPE_INT
  session.opt[OPT_RESOLUTION].cap |= Sane.CAP_AUTOMATIC
  session.opt[OPT_RESOLUTION].unit = Sane.UNIT_DPI
  session.opt[OPT_RESOLUTION].constraint_type = Sane.CONSTRAINT_WORD_LIST
  ...
  /* "Geometry" group: */
  session.opt[OPT_GEOMETRY_GROUP].title = Sane.TITLE_GEOMETRY
  session.opt[OPT_GEOMETRY_GROUP].name = Sane.NAME_GEOMETRY
  session.opt[OPT_GEOMETRY_GROUP].desc = Sane.DESC_GEOMETRY
  session.opt[OPT_GEOMETRY_GROUP].type = Sane.TYPE_GROUP
  session.opt[OPT_GEOMETRY_GROUP].cap = Sane.CAP_ADVANCED
  session.opt[OPT_GEOMETRY_GROUP].size = 0
  session.opt[OPT_GEOMETRY_GROUP].constraint_type = Sane.CONSTRAINT_NONE
  ...
  /* top-left x */
  session.opt[OPT_TL_X].name = Sane.NAME_SCAN_TL_X
  session.opt[OPT_TL_X].title = Sane.TITLE_SCAN_TL_X
  session.opt[OPT_TL_X].desc = Sane.DESC_SCAN_TL_X
  session.opt[OPT_TL_X].type = Sane.TYPE_FIXED
  session.opt[OPT_TL_X].cap |= Sane.CAP_AUTOMATIC
  session.opt[OPT_TL_X].unit = Sane.UNIT_MM
  session.opt[OPT_TL_X].constraint_type = Sane.CONSTRAINT_RANGE
  session.opt[OPT_TL_X].constraint.range = &x_range
  session.val[OPT_TL_X].w = 0

  /* top-left y */
  session.opt[OPT_TL_Y].name = Sane.NAME_SCAN_TL_Y
  session.opt[OPT_TL_Y].title = Sane.TITLE_SCAN_TL_Y
  session.opt[OPT_TL_Y].desc = Sane.DESC_SCAN_TL_Y
  session.opt[OPT_TL_Y].type = Sane.TYPE_FIXED
  session.opt[OPT_TL_Y].cap |= Sane.CAP_AUTOMATIC
  session.opt[OPT_TL_Y].unit = Sane.UNIT_MM
  session.opt[OPT_TL_Y].constraint_type = Sane.CONSTRAINT_RANGE
  session.opt[OPT_TL_Y].constraint.range = &y_range
  session.val[OPT_TL_Y].w = 0

  /* bottom-right x */
  session.opt[OPT_BR_X].name = Sane.NAME_SCAN_BR_X
  session.opt[OPT_BR_X].title = Sane.TITLE_SCAN_BR_X
  session.opt[OPT_BR_X].desc = Sane.DESC_SCAN_BR_X
  session.opt[OPT_BR_X].type = Sane.TYPE_FIXED
  session.opt[OPT_BR_X].cap |= Sane.CAP_AUTOMATIC
  session.opt[OPT_BR_X].unit = Sane.UNIT_MM
  session.opt[OPT_BR_X].constraint_type = Sane.CONSTRAINT_RANGE
  session.opt[OPT_BR_X].constraint.range = &x_range
  session.val[OPT_BR_X].w = x_range.max

  /* bottom-right y */
  session.opt[OPT_BR_Y].name = Sane.NAME_SCAN_BR_Y
  session.opt[OPT_BR_Y].title = Sane.TITLE_SCAN_BR_Y
  session.opt[OPT_BR_Y].desc = Sane.DESC_SCAN_BR_Y
  session.opt[OPT_BR_Y].type = Sane.TYPE_FIXED
  session.opt[OPT_BR_Y].cap |= Sane.CAP_AUTOMATIC
  session.opt[OPT_BR_Y].unit = Sane.UNIT_MM
  session.opt[OPT_BR_Y].constraint_type = Sane.CONSTRAINT_RANGE
  session.opt[OPT_BR_Y].constraint.range = &y_range
  session.val[OPT_BR_Y].w = y_range.max

  /* "Enhancement" group: */
  session.opt[OPT_ENHANCEMENT_GROUP].title = Sane.TITLE_ENHANCEMENT
  session.opt[OPT_ENHANCEMENT_GROUP].name = Sane.NAME_ENHANCEMENT
  session.opt[OPT_ENHANCEMENT_GROUP].desc = Sane.DESC_ENHANCEMENT
  session.opt[OPT_ENHANCEMENT_GROUP].type = Sane.TYPE_GROUP
  session.opt[OPT_ENHANCEMENT_GROUP].cap = Sane.CAP_ADVANCED
  session.opt[OPT_ENHANCEMENT_GROUP].size = 0
  session.opt[OPT_ENHANCEMENT_GROUP].constraint_type = Sane.CONSTRAINT_NONE

  /* BW threshold */
  session.opt[OPT_THRESHOLD].name = Sane.NAME_THRESHOLD
  session.opt[OPT_THRESHOLD].title = Sane.TITLE_THRESHOLD
  session.opt[OPT_THRESHOLD].desc = Sane.DESC_THRESHOLD
  session.opt[OPT_THRESHOLD].type = Sane.TYPE_FIXED
  session.opt[OPT_THRESHOLD].unit = Sane.UNIT_PERCENT
  ...
  session.opt[OPT_CUSTOM_GAMMA].title = Sane.TITLE_CUSTOM_GAMMA
  session.opt[OPT_CUSTOM_GAMMA].desc = Sane.DESC_CUSTOM_GAMMA
  session.opt[OPT_CUSTOM_GAMMA].type = Sane.TYPE_BOOL
  session.opt[OPT_CUSTOM_GAMMA].cap |= Sane.CAP_ADVANCED
  session.val[OPT_CUSTOM_GAMMA].b = Sane.FALSE

  /* grayscale gamma vector */
  session.opt[OPT_GAMMA_VECTOR].name = Sane.NAME_GAMMA_VECTOR
  session.opt[OPT_GAMMA_VECTOR].title = Sane.TITLE_GAMMA_VECTOR
  session.opt[OPT_GAMMA_VECTOR].desc = Sane.DESC_GAMMA_VECTOR
  session.opt[OPT_GAMMA_VECTOR].type = Sane.TYPE_INT
  ...
  /* red gamma vector */
  session.opt[OPT_GAMMA_VECTOR_R].name = Sane.NAME_GAMMA_VECTOR_R
  session.opt[OPT_GAMMA_VECTOR_R].title = Sane.TITLE_GAMMA_VECTOR_R
  session.opt[OPT_GAMMA_VECTOR_R].desc = Sane.DESC_GAMMA_VECTOR_R
  session.opt[OPT_GAMMA_VECTOR_R].type = Sane.TYPE_INT
  ...
  /* green gamma vector */
  session.opt[OPT_GAMMA_VECTOR_G].name = Sane.NAME_GAMMA_VECTOR_G
  session.opt[OPT_GAMMA_VECTOR_G].title = Sane.TITLE_GAMMA_VECTOR_G
  session.opt[OPT_GAMMA_VECTOR_G].desc = Sane.DESC_GAMMA_VECTOR_G
  session.opt[OPT_GAMMA_VECTOR_G].type = Sane.TYPE_INT
  ...
  /* blue gamma vector */
  session.opt[OPT_GAMMA_VECTOR_B].name = Sane.NAME_GAMMA_VECTOR_B
  session.opt[OPT_GAMMA_VECTOR_B].title = Sane.TITLE_GAMMA_VECTOR_B
  session.opt[OPT_GAMMA_VECTOR_B].desc = Sane.DESC_GAMMA_VECTOR_B
  session.opt[OPT_GAMMA_VECTOR_B].type = Sane.TYPE_INT
  ...
  /* "Sensors" group */
  session.opt[OPT_SENSOR_GROUP].title = Sane.TITLE_SENSORS
  session.opt[OPT_SENSOR_GROUP].name = Sane.NAME_SENSORS
  session.opt[OPT_SENSOR_GROUP].desc = Sane.DESC_SENSORS
  session.opt[OPT_SENSOR_GROUP].type = Sane.TYPE_GROUP
  session.opt[OPT_SENSOR_GROUP].cap = Sane.CAP_ADVANCED
  ...
  session.opt[OPT_ADVANCED_GROUP].constraint_type = Sane.CONSTRAINT_NONE
  ...
  session.opt[OPT_LAMP_ON].constraint_type = Sane.CONSTRAINT_NONE
...
shading_calibration(struct Rts8891_Device *dev, Bool color, Int mode, Int light)
{
  ...
  if(dev.shading_data != NULL)
    free(dev.shading_data)
  dev.shading_data = (unsigned char *) malloc(dev.bytesPerLine)
  if(dev.shading_data == NULL)
    {
      free(image)
      DBG(DBG_error,
...
send_calibration_data(struct Rts8891_Session *session)
{
  ...
  Sane.Byte *calibration = NULL, format, val
  struct Rts8891_Device *dev = session.dev
  var i: Int, idx
...
move_to_scan_area(struct Rts8891_Session *session)
{
  Sane.Status status = Sane.STATUS_GOOD
  Sane.Byte control
  Sane.Byte regs[RTS8891_MAX_REGISTERS]
  struct Rts8891_Device *dev = session.dev
  Int distance
