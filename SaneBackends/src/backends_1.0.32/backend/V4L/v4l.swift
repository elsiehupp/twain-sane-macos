...
static const Sane.String_Const mode_list[] = {
  Sane.VALUE_SCAN_MODE_GRAY, Sane.VALUE_SCAN_MODE_COLOR,
  0
]
...
static Sane.Range x_range = { 0, 338, 2 ]

static Sane.Range odd_x_range = { 1, 339, 2 ]

static Sane.Range y_range = { 0, 249, 1 ]

static Sane.Range odd_y_range = { 1, 250, 1 ]
...
/* top-left x *//* ??? first check if window is settable at all */
  ...
  s.opt[OPT_TL_X].cap |= Sane.CAP_INACTIVE
  ...
  s.opt[OPT_TL_Y].cap |= Sane.CAP_INACTIVE
  ...
  s.opt[OPT_BR_X].cap |= Sane.CAP_INACTIVE
  ...
  s.opt[OPT_BR_Y].cap |= Sane.CAP_INACTIVE
		     ...
		     Sane.Action action, void *val, Int * info)
{
  ...
  if(action == Sane.ACTION_GET_VALUE)
	...
	case OPT_BRIGHTNESS:
	  ...
	  break
	case OPT_HUE:
	  s.pict.hue = *(Sane.Word *) val *256
	  s.val[option].w = *(Sane.Word *) val
	  break
	case OPT_COLOR:
	  s.pict.colour = *(Sane.Word *) val *256
	  s.val[option].w = *(Sane.Word *) val
	  break
	case OPT_CONTRAST:
	  s.pict.contrast = *(Sane.Word *) val *256
	  s.val[option].w = *(Sane.Word *) val
	  break
	case OPT_WHITE_LEVEL:
	  s.pict.whiteness = *(Sane.Word *) val *256
	  s.val[option].w = *(Sane.Word *) val
	  break
	case OPT_CHANNEL:
	  {
