 ...
 *
 * 2-byte short ints are represented by 4-byte Int types
 *
 * ========================================================================= */

struct Pieusb_Scanner_Properties {
    Sane.Byte deviceType; /* 0x06 = scanner */
    Sane.Byte additionalLength; /* including this byte: 0xb4 = 180, so total structure 184 bytes */
    Sane.Char vendor[9]; /* actually 8 bytes, not null-terminated ...PIE     ... */
    Sane.Char product[17]; /* actually 16 bytes, null-terminated ...SF Scanner... */
    Sane.Char productRevision[5]; /* actually 4 bytes, not null-terminated ...1.70... */
    ...
    Sane.Byte colorFormat; /* 0x07 = 00000111 0-0-0-0-0-Index-Line-Pixel */
    Sane.Byte imageFormat; /* 0x09 = 00001001 0-0-0-0-OKLine-BlkOne-Motorola-Intel */
    Sane.Byte scanCapability
    ...
    Sane.Byte optionalDevices
       ...
       * TransModule: no
       * AutoDocFeeder: yes */
    Sane.Byte enhancements; /* 0x02 = no info in pie.c */
    ...
    Int maximumExposure; /* 0xc409 = 2500 (units?) */
    Int minimumExposure; /* 0x6400 = 100 (units?) */
    Int x0; /* 0xd002 = 720 transparency top left x */
    Int y0; /* 0x8604 = 1158 transparency top left y */
    Int x1; /* 0xbc10 = 4284 transparency bottom right x */
    Int y1; /* 0xc015 = 5568 transparency bottom right y */
    Int model; /* 0x3000 => model number */
    /* Int div_118; 0x0000 meaning? */
    Sane.Char production[4]; /* null terminated */
    Sane.Char timestamp[20]; /* null terminated */
    Sane.Byte signature[40]; /* null terminated */
]

struct Pieusb_Sense {
    /* 14 bytes according to SCSI-2 p158, table 67 (p469 ASC/Q alphabetically) */
    Sane.Byte errorCode; /* 0x70 or 0x71 */
    Sane.Byte segment
    Sane.Byte senseKey; /* sense key is actually this value & 0x0F - table 69 */
    Sane.Byte info[4]
    Sane.Byte addLength; /* should be 0x07 (remaining struct length including this byte) */
    Sane.Byte cmdInfo[4]; /* command specific information */
    Sane.Byte senseCode; /* abbreviated ASC - table 71 */
    Sane.Byte senseQualifier; /* abbreviated ASCQ - table 71 */
]
...
struct Pieusb_Scan_Parameters {
    Int width; /* Number of pixels on a scan line */
    Int lines; /* Number of lines in the scan. Value depends on color format, see Pieusb_Mode. */
    Int bytes; /* Number of bytes on a scan line. Value depends on color format. */
...
struct Pieusb_Mode {
    /* Sane.Byte size; of remaining data, not useful */
    ...
    Sane.Byte colorFormat
       ...
       *                 valid data. */
    Sane.Byte byteOrder; /* 0x01 = Intel; only bit 0 used */
    Bool sharpen; /* byte 9 bit 1 */
    Bool skipShadingAnalysis; /* byte 9 bit 3 */
...
struct Pieusb_Settings_Condensed {
    ...
    Sane.Byte gain[4]; /* => Pieusb_Settings.gain */
    Sane.Byte light; /* => Pieusb_Settings.light */
    Sane.Byte extraEntries; /* => Pieusb_Settings.extraEntries */
    Sane.Byte doubleTimes; /* => Pieusb_Settings.doubleTimes */
]
...
struct Pieusb_Exposure_Time_Color {
    Int filter; /* color mask 0x02, 0x04 or 0x08 for R, G, B */
    Int value; /* relative exposure time 0 - 100 */
]

struct Pieusb_Exposure_Time {
    Int code; /* 0x93 */
    Int size; /* number of bytes in rest of structure */
...
struct Pieusb_Shading_Parameters_Info {
    Sane.Byte type; /* 0x00, 0x08, 0x10, 0x20; RGBI(?) */
    Sane.Byte sendBits; /* 0x10 = 16 */
    Sane.Byte recieveBits; /* 0x10 = 16 */
    Sane.Byte nLines; /* 0x2D = 45 */
    Int pixelsPerLine; /* 0x14dc = 5340 */
]
...
struct Pieusb_Command_Status {
    ...
    Sane.Byte senseKey; /* sense key: see Pieusb_Sense */
    Sane.Byte senseCode; /* sense code */
    Sane.Byte senseQualifier; /* sense code qualifier */
]
...
void sanei_pieusb_cmd_slide(Int device_number, slide_action action, struct Pieusb_Command_Status *status)

/* Scanner commands */

void sanei_pieusb_cmd_test_unit_ready(Int device_number, struct Pieusb_Command_Status *status)

void sanei_pieusb_cmd_get_sense(Int device_number, struct Pieusb_Sense* sense, struct Pieusb_Command_Status *status, PIEUSB_Status *ret)

void sanei_pieusb_cmd_get_halftone_pattern(Int device_number, Int index, struct Pieusb_Halftone_Pattern* pattern, struct Pieusb_Command_Status *status)
void sanei_pieusb_cmd_set_halftone_pattern(Int device_number, Int index, struct Pieusb_Halftone_Pattern* pattern, struct Pieusb_Command_Status *status)

void sanei_pieusb_cmd_get_scan_frame(Int device_number, Int index, struct Pieusb_Scan_Frame* frame, struct Pieusb_Command_Status *status)
void sanei_pieusb_cmd_set_scan_frame(Int device_number, Int index, struct Pieusb_Scan_Frame* frame, struct Pieusb_Command_Status *status)

void sanei_pieusb_cmd_17(Int device_number, Int value, struct Pieusb_Command_Status *status)

void sanei_pieusb_cmd_get_exposure_time(Int device_number, Int colorbits, struct Pieusb_Exposure_Time* time, struct Pieusb_Command_Status *status)
void sanei_pieusb_cmd_set_exposure_time(Int device_number, struct Pieusb_Exposure_Time* time, struct Pieusb_Command_Status *status)

void sanei_pieusb_cmd_get_highlight_shadow(Int device_number, Int colorbits, struct Pieusb_Highlight_Shadow* hgltshdw, struct Pieusb_Command_Status *status)
void sanei_pieusb_cmd_set_highlight_shadow(Int device_number, struct Pieusb_Highlight_Shadow* hgltshdw, struct Pieusb_Command_Status *status)

void sanei_pieusb_cmd_get_shading_parms(Int device_number, struct Pieusb_Shading_Parameters_Info* shading, struct Pieusb_Command_Status *status)
void sanei_pieusb_cmd_get_scanned_lines(Int device_number, Sane.Byte* data, Int lines, Int size, struct Pieusb_Command_Status *status)

void sanei_pieusb_cmd_get_ccd_mask(Int device_number, Sane.Byte* mask, Int mask_size, struct Pieusb_Command_Status *status)
void sanei_pieusb_cmd_set_ccd_mask(Int device_number, Sane.Byte colorbits, Sane.Byte* mask, Int mask_size, struct Pieusb_Command_Status *status)

/*
void cmdPrepareHalftonePattern(Int device_number, Int index, struct Pieusb_Command_Status *status)
void cmdPrepareScanFrame(Int device_number, Int index, struct Pieusb_Command_Status *status)
void cmdPrepareRelativeExposureTime(Int device_number, Int colorbits, struct Pieusb_Command_Status *status)
void cmdPrepareHighlightShadow(Int device_number, Int colorbits, struct Pieusb_Command_Status *status)
void cmdPrepareShadingParameters(Int device_number, struct Pieusb_Command_Status *status)
*/

void sanei_pieusb_cmd_get_parameters(Int device_number, struct Pieusb_Scan_Parameters* parameters, struct Pieusb_Command_Status *status)

void sanei_pieusb_cmd_inquiry(Int device_number, struct Pieusb_Scanner_Properties* inq, Sane.Byte size, struct Pieusb_Command_Status *status)

void sanei_pieusb_cmd_get_mode(Int device_number, struct Pieusb_Mode* mode, struct Pieusb_Command_Status *status)
void sanei_pieusb_cmd_set_mode(Int device_number, struct Pieusb_Mode* mode, struct Pieusb_Command_Status *status)

void sanei_pieusb_cmd_start_scan(Int device_number, struct Pieusb_Command_Status *status)
void sanei_pieusb_cmd_stop_scan(Int device_number, struct Pieusb_Command_Status *status)

void sanei_pieusb_cmd_set_scan_head(Int device_number, Int mode, Int steps, struct Pieusb_Command_Status *status)

void sanei_pieusb_cmd_get_gain_offset(Int device_number, struct Pieusb_Settings* settings, struct Pieusb_Command_Status *status)

void sanei_pieusb_cmd_set_gain_offset(Int device_number, struct Pieusb_Settings* settings, struct Pieusb_Command_Status *status)

void sanei_pieusb_cmd_read_state(Int device_number, struct Pieusb_Scanner_State* state, struct Pieusb_Command_Status *status)
