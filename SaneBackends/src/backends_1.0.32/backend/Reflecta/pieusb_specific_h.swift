...
#define Sane.VALUE_SCAN_MODE_RGBI    "RGBI"
...
#define POST_SW_CROP            (1 << 4)        /* trim whole image in Sane.start
                                                   before Sane.get_parameters() is answered */
...
struct Pieusb_Device_Definition
{
    ...
    Sane.Range x_range
    Sane.Range y_range
    Sane.Range exposure_range; /* Unit is a 8051 machine cycle, which is approximately 1 us. (Exactly: 12 cycles at 11.059 Mhz = 1.085 us.) */
    Sane.Range dust_range
    ...
    String halftone_list[17]; /* names of the halftone patterns from the scanner */
    String speed_list[9];	   /* names of available speeds */
    String ir_sw_list[4]
    String crop_sw_list[4]
    Sane.Word grain_sw_list[6]
...
struct Pieusb_Scanner
{
    ...
    Int scanning; /* true if busy scanning */
    Int cancel_request; /* if true, scanner should terminate a scan */

    /* Scan settings */
    struct Pieusb_Mode mode
    struct Pieusb_Settings settings
    struct Pieusb_Scan_Frame frame
    Sane.Parameters scan_parameters
...
#define PIEUSB_CCD_MASK_SIZE 0x1a1d  /* pieusb 5340; */ /* cyberview: 6685 0x1a1d */
    ...
    Bool preview_done
    Int preview_exposure[4];    /* exposure values used in preview */
    Int preview_gain[4];        /* gain values used in preview */
    Int preview_offset[4];      /* offset values used in preview */
    Int preview_lower_bound[4]; /* lowest RGBI values in preview */
    Int preview_upper_bound[4]; /* highest RGBI values in preview */
...
Sane.Status sanei_pieusb_parse_config_line(const char* config_line,
                                           Sane.Word* vendor_id,
...
Sane.Status sanei_pieusb_get_ccd_mask(Pieusb_Scanner * scanner)
Sane.Status sanei_pieusb_get_shading_data(Pieusb_Scanner * scanner)
Sane.Status sanei_pieusb_set_mode_from_options(Pieusb_Scanner * scanner)
Sane.Status sanei_pieusb_set_gain_offset(Pieusb_Scanner * scanner, const char* calibration_mode)
Sane.Status sanei_pieusb_set_frame_from_options(Pieusb_Scanner * scanner)
void sanei_pieusb_print_options(struct Pieusb_Scanner *scanner)
/* sub to Sane.control_option() and Sane.start() */
