...
static Sane.Status _ctrl_out_byte(Int device_number, Int port, Sane.Byte b)
static Sane.Status _bulk_size(Int device_number, unsigned Int size)
static Sane.Status _ctrl_in_byte(Int device_number, Sane.Byte* b)
static Sane.Status _bulk_in(Int device_number, Sane.Byte* data, size_t *size)
static Sane.Status _ieee_command(Int device_number, Sane.Byte command)
...
static PIEUSB_USB_Status _pieusb_scsi_command(Int device_number, Sane.Byte command[], Sane.Byte data[], Int size)
...
/**
 * Convert PIEUSB_Status to Sane.Status
 */
Sane.Status
sanei_pieusb_convert_status(PIEUSB_Status status)
{
  return (Sane.Status)status
}
...
PIEUSB_Status
sanei_pieusb_command(Int device_number, Sane.Byte command[], Sane.Byte data[], Int size)
{
...
#define SCSI_REQUEST_SENSE      0x03
    ...
    if (usb_status == USB_STATUS_BUSY) {
      /* wait on usb */
      Sane.status = _ctrl_in_byte (device_number, &usbstat)
...
Sane.Status
sanei_pieusb_usb_reset(Int device_number)
{
...
static Sane.Status
_ieee_command(Int device_number, Sane.Byte command)
{
    Sane.Status st
...
_pieusb_scsi_command(Int device_number, Sane.Byte command[], Sane.Byte data[], Int size)
{
  Sane.Status st
  Sane.Byte usbstat
  var i: Int
 ...
 */
static Sane.Status _bulk_size(Int device_number, unsigned Int size) {
    Sane.Byte bulksize[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]
 ...
 */
static Sane.Status
_bulk_in(Int device_number, Sane.Byte *data, size_t *size) {
    size_t remaining = 0
    Sane.Status r = Sane.STATUS_GOOD
    size_t part
