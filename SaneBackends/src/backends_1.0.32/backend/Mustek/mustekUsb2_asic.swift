...
Mustek_ClearFIFO(PAsic chip)
{
  ...
  buf[2] = 0
  buf[3] = 0
  status = WriteIOControl(chip, 0x05, 0, 4, (Sane.Byte *) (buf))
  if(status != STATUS_GOOD)
    return status

  status = WriteIOControl(chip, 0xc0, 0, 4, (Sane.Byte *) (buf))
  if(status != STATUS_GOOD)
    return status
...
SetRWSize(PAsic chip, Sane.Byte ReadWrite, unsigned Int size)
{
  ...
  if(ReadWrite == 0)
    {				/*write */
      status = Mustek_SendData(chip, 0x7C, (Sane.Byte) (size))
      if(status != STATUS_GOOD)
	return status
      status = Mustek_SendData(chip, 0x7D, (Sane.Byte) (size >> 8))
      if(status != STATUS_GOOD)
	return status
      status = Mustek_SendData(chip, 0x7E, (Sane.Byte) (size >> 16))
      if(status != STATUS_GOOD)
	return status
      status = Mustek_SendData(chip, 0x7F, (Sane.Byte) (size >> 24))
      if(status != STATUS_GOOD)
	return status
    }
  else
    {				/* read */
      status = Mustek_SendData(chip, 0x7C, (Sane.Byte) (size >> 1))
      if(status != STATUS_GOOD)
	return status
      status = Mustek_SendData(chip, 0x7D, (Sane.Byte) (size >> 9))
      if(status != STATUS_GOOD)
	return status
      status = Mustek_SendData(chip, 0x7E, (Sane.Byte) (size >> 17))
      if(status != STATUS_GOOD)
	return status
      status = Mustek_SendData(chip, 0x7F, (Sane.Byte) (size >> 25))
      if(status != STATUS_GOOD)
	return status
    }
  ...
  for(i = 0; i < size / (read_size); i++)
    {
      ...
      status = WriteIOControl(chip, 0x03, 0, 4, (Sane.Byte *) (buf))
  ...
  for(i = 0; i < size / (write_size); i++)
    {
      ...
      WriteIOControl(chip, 0x02, 0, 4, (Sane.Byte *) buf)
...
Mustek_SendData2Byte(PAsic chip, unsigned short reg, Sane.Byte data)
{
  ...
  static Sane.Byte BankBuf[4]
  static Sane.Byte DataBuf[4]
...
LLFRamAccess(PAsic chip, LLF_RAMACCESS * RamAccess)
{
  ...
  if(RamAccess.ReadWrite == WRITE_RAM)
    {				/*Write RAM */
      ...
      usleep(20000)
      RamAccess.RwSize = 2
      RamAccess.BufferPtr = (Sane.Byte *) a
			    ...
			    MotorCurrentAndPhase)
{
  ...
  if(MotorCurrentAndPhase.FillPhase == 0)
	...
	{			/* 1/4 step */
	  ...
	  /*1 */
	  Mustek_SendData2Byte(chip, ES02_52_MOTOR_CURRENT_TABLE_A,
				(Sane.Byte) (MotorCurrentAndPhase->
  ...
  if(LLF_SetMotorTable.MotorTablePtr != NULL)
    {
      ...
      RamAccess.BufferPtr = (Sane.Byte *) LLF_SetMotorTable.MotorTablePtr
		   ...
		   unsigned Int dwScanImageSteps, unsigned short wYResolution)
{
  ...
  MotorStepsTable.MotorSyncUnit = (Sane.Byte) wMotorSycnPixelNumber
  ...
  Mustek_SendData(chip, ES01_F2_ScanImageStep16_19,
		   (Sane.Byte) ((dwTotalMotorSteps & 0x00ff0000) >> 16))
...
DRAM_Test(PAsic chip)
{
  ...
  status = Mustek_DMAWrite(chip, 64, (Sane.Byte *) (temps))
  if(status != STATUS_GOOD)
    {
  ...
  status = Mustek_DMARead(chip, 64, (Sane.Byte *) (temps))
  if(status != STATUS_GOOD)
    {
      free(temps)
      return status
...
CCDTiming(PAsic chip)
{
  ...
  Mustek_SendData(chip, ES01_1D0_DUMMY_CYCLE_TIMING_B0,
		   (Sane.Byte) (chip.Timing.CCD_DummyCycleTiming))
  Mustek_SendData(chip, ES01_1D1_DUMMY_CYCLE_TIMING_B1,
		   (Sane.Byte) (chip.Timing.CCD_DummyCycleTiming >> 8))
  Mustek_SendData(chip, ES01_1D2_DUMMY_CYCLE_TIMING_B2,
		   (Sane.Byte) (chip.Timing.CCD_DummyCycleTiming >> 16))
  Mustek_SendData(chip, ES01_1D3_DUMMY_CYCLE_TIMING_B3,
		   (Sane.Byte) (chip.Timing.CCD_DummyCycleTiming >> 24))
  ...
  Mustek_SendData(chip, ES01_1D4_PH1_TIMING_ADJ_B0, (Sane.Byte) (dwPH1))
  Mustek_SendData(chip, ES01_1D5_PH1_TIMING_ADJ_B1, (Sane.Byte) (dwPH1 >> 8))
  Mustek_SendData(chip, ES01_1D6_PH1_TIMING_ADJ_B2, (Sane.Byte) (dwPH1 >> 16))
  Mustek_SendData(chip, ES01_1D7_PH1_TIMING_ADJ_B3, (Sane.Byte) (dwPH1 >> 24))
  ...
  Mustek_SendData(chip, ES01_1D8_PH2_TIMING_ADJ_B0, (Sane.Byte) (dwPH2))
  Mustek_SendData(chip, ES01_1D9_PH2_TIMING_ADJ_B1, (Sane.Byte) (dwPH2 >> 8))
  Mustek_SendData(chip, ES01_1DA_PH2_TIMING_ADJ_B2, (Sane.Byte) (dwPH2 >> 16))
  Mustek_SendData(chip, ES01_1DB_PH2_TIMING_ADJ_B3, (Sane.Byte) (dwPH2 >> 24))

  Mustek_SendData(chip, ES01_1E4_PHRS_TIMING_ADJ_B0, (Sane.Byte) (dwPHRS))
  Mustek_SendData(chip, ES01_1E5_PHRS_TIMING_ADJ_B1, (Sane.Byte) (dwPHRS >> 8))
  Mustek_SendData(chip, ES01_1E6_PHRS_TIMING_ADJ_B2, (Sane.Byte) (dwPHRS >> 16))
  Mustek_SendData(chip, ES01_1E7_PHRS_TIMING_ADJ_B3, (Sane.Byte) (dwPHRS >> 24))

  Mustek_SendData(chip, ES01_1E8_PHCP_TIMING_ADJ_B0, (Sane.Byte) (dwPHCP))
  Mustek_SendData(chip, ES01_1E9_PHCP_TIMING_ADJ_B1, (Sane.Byte) (dwPHCP >> 8))
  Mustek_SendData(chip, ES01_1EA_PHCP_TIMING_ADJ_B2, (Sane.Byte) (dwPHCP >> 16))
  Mustek_SendData(chip, ES01_1EB_PHCP_TIMING_ADJ_B3, (Sane.Byte) (dwPHCP >> 24))
...
SetAFEGainOffset(PAsic chip)
{
  ...
  for(i = 0; i < 4; i++)
      ...
      if(chip.AD.DirectionR == 0)
	{
	  Mustek_SendData(chip, ES01_2A1_AFE_AUTO_CONFIG_GAIN,
			   (Sane.Byte) (chip.AD.GainR << 1))
	  Mustek_SendData(chip, ES01_2A2_AFE_AUTO_CONFIG_OFFSET,
			   (Sane.Byte) (chip.AD.OffsetR))
	}
      else
	{
	  Mustek_SendData(chip, ES01_2A1_AFE_AUTO_CONFIG_GAIN,
			   (Sane.Byte) (chip.AD.GainR << 1) | 0x01)
	  Mustek_SendData(chip, ES01_2A2_AFE_AUTO_CONFIG_OFFSET,
			   (Sane.Byte) (chip.AD.OffsetR))
	}
    }
  ...
  for(i = 0; i < 4; i++)
	...
	{
	  Mustek_SendData(chip, ES01_2A1_AFE_AUTO_CONFIG_GAIN,
			   (Sane.Byte) (chip.AD.GainG << 1))
	  Mustek_SendData(chip, ES01_2A2_AFE_AUTO_CONFIG_OFFSET,
			   (Sane.Byte) (chip.AD.OffsetG))
	}
      else
	{
	  Mustek_SendData(chip, ES01_2A1_AFE_AUTO_CONFIG_GAIN,
			   (Sane.Byte) (chip.AD.GainG << 1) | 0x01)
	  Mustek_SendData(chip, ES01_2A2_AFE_AUTO_CONFIG_OFFSET,
			   (Sane.Byte) (chip.AD.OffsetG))
	}
    }
  ...
  for(i = 0; i < 4; i++)
	...
	{
	  Mustek_SendData(chip, ES01_2A1_AFE_AUTO_CONFIG_GAIN,
			   (Sane.Byte) (chip.AD.GainB << 1))
	  Mustek_SendData(chip, ES01_2A2_AFE_AUTO_CONFIG_OFFSET,
			   (Sane.Byte) (chip.AD.OffsetB))
	}
      else
	{
	  Mustek_SendData(chip, ES01_2A1_AFE_AUTO_CONFIG_GAIN,
			   (Sane.Byte) (chip.AD.GainB << 1) | 0x01)
	  Mustek_SendData(chip, ES01_2A2_AFE_AUTO_CONFIG_OFFSET,
			   (Sane.Byte) (chip.AD.OffsetB))
	}
    }
		...
		Sane.Byte byClear_Pulse_Width, unsigned short * PValidPixelNumber)
{
  ...
  Sane.Byte InValidPixelNumber
  unsigned Int CISPackAreaStartAddress
  Sane.Byte PackAreaUseLine
  ...
  if((FinalLinePixelPerSegment % 8) > 0)
    {
      InValidPixelNumber = (Sane.Byte) (8 - (FinalLinePixelPerSegment % 8))
    }
  else
    {
  ...
  Mustek_SendData(chip, 0x270,
		   (Sane.Byte) ((SegmentTotalPixel * (PackAreaUseLine) * 1)))
  Mustek_SendData(chip, 0x271,
		   (Sane.Byte) ((SegmentTotalPixel * (PackAreaUseLine) * 1) >> 8))
  Mustek_SendData(chip, 0x272,
		   (Sane.Byte) ((SegmentTotalPixel * (PackAreaUseLine) *
			    1) >> 16))

  Mustek_SendData(chip, 0x27C,
		   (Sane.Byte) ((SegmentTotalPixel * (PackAreaUseLine) * 2)))
  Mustek_SendData(chip, 0x27D,
		   (Sane.Byte) ((SegmentTotalPixel * (PackAreaUseLine) * 2) >> 8))
  Mustek_SendData(chip, 0x27E,
		   (Sane.Byte) ((SegmentTotalPixel * (PackAreaUseLine) *
			    2) >> 16))

  Mustek_SendData(chip, 0x288,
		   (Sane.Byte) ((SegmentTotalPixel * (PackAreaUseLine) * 3)))
  Mustek_SendData(chip, 0x289,
		   (Sane.Byte) ((SegmentTotalPixel * (PackAreaUseLine) * 3) >> 8))
  ...
  Mustek_SendData(chip, ES01_1FC_PACK_CHANNEL_SIZE_B1,
		   (Sane.Byte) ((SegmentTotalPixel * PackAreaUseLine) >> 8))
  Mustek_SendData(chip, ES01_1FD_PACK_CHANNEL_SIZE_B2,
		   (Sane.Byte) ((SegmentTotalPixel * PackAreaUseLine) >> 16))
  ...
  Mustek_SendData(chip, ES01_19E_PACK_AREA_R_START_ADDR_BYTE0,
		   (Sane.Byte) ((CISPackAreaStartAddress +
...
static Sane.String_Const device_name

static Sane.Status
attach_one_scanner(Sane.String_Const devname)
{
...
Asic_Open(PAsic chip, Sane.Byte *pDeviceName)
{
  ...
  if(Sane.status != Sane.STATUS_GOOD)
    {
      DBG(DBG_ERR, "Asic_Open: sanei_usb_find_devices failed: %s\n",
	   Sane.strstatus(Sane.status))
      return STATUS_INVAL
    }
  /* open usb */
  if(device_name == NULL)
  ...
  Sane.status = sanei_usb_open(device_name, &chip.fd)
  if(Sane.status != Sane.STATUS_GOOD)
    {
      DBG(DBG_ERR, "Asic_Open: sanei_usb_open of %s failed: %s\n",
	   device_name, Sane.strstatus(Sane.status))
      return STATUS_INVAL
    }
  ...
  pDeviceName = (Sane.Byte *) strdup(device_name)
  if(!pDeviceName)
    {
		...
		unsigned short wX, unsigned short wY, unsigned short wWidth, unsigned short wLength)
{
  ...
  Sane.Byte byDummyCycleNum = 0
  unsigned short Total_MotorDPI

  unsigned short wMultiMotorStep = 1
  Sane.Byte bMotorMoveType = _MOTOR_MOVE_TYPE

  Sane.Byte byClear_Pulse_Width = 0
  ...
  Sane.Byte isMotorMoveToFirstLine = chip.isMotorGoToFirstLine
  Sane.Byte isUniformSpeedToScan = chip.isUniformSpeedToScan
  Sane.Byte isScanBackTracking = SCAN_BACK_TRACKING_ENABLE
  unsigned short * lpMotorTable
  ...
  RamAccess.BufferPtr = (Sane.Byte *) lpMotorTable
  LLFRamAccess(chip, &RamAccess)
  ...
  Mustek_SendData(chip, ES01_9D_MotorTableAddrA14_A21,
		   (Sane.Byte) (dwTableBaseAddr >> TABLE_OFFSET_BASE))
  ...
  RamAccess.BufferPtr = (Sane.Byte *) chip.lpShadingTable
  LLFRamAccess(chip, &RamAccess)
  ...
  Mustek_SendData(chip, ES01_9B_ShadingTableAddrA14_A21,
		   (Sane.Byte) (dwShadingTableAddr >> TABLE_OFFSET_BASE))
			  ...
			  unsigned Int dwXferBytes, Sane.Byte bScanBits)
{
  ...
  if(bScanBits == 24)
    {
      unsigned var i: Int
      pCalBuffer = (Sane.Byte *) malloc(dwXferBytes)
      if(pCalBuffer == NULL)
	{
	  DBG(DBG_ERR,
      ...
      for(dwTotalReadData = 0; dwTotalReadData < dwXferBytes;)
	{
	  ...
	  Mustek_DMARead(chip, dwReadImageData,
			  (Sane.Byte *) (pCalBuffer + dwTotalReadData))
	...
	{
	  *((Sane.Byte *) pBuffer + i) = *(pCalBuffer + i * 3)
	  *((Sane.Byte *) pBuffer + dwXferBytes + i) = *(pCalBuffer + i * 3 + 1)
	  *((Sane.Byte *) pBuffer + dwXferBytes * 2 + i) =
	    *(pCalBuffer + i * 3 + 2)
	}
      free(pCalBuffer)
    }
      ...
      for(dwTotalReadData = 0; dwTotalReadData < dwXferBytes;)
	{
	  ...
	  Mustek_DMARead(chip, dwReadImageData,
			  (Sane.Byte *) pBuffer + dwTotalReadData)
	  dwTotalReadData += dwReadImageData
	}
    }
		      ...
		      unsigned short wXResolution, unsigned short wWidth, unsigned short wX)
{
  ...
  chip.lpShadingTable = (Sane.Byte *) malloc(wShadingTableSize)
  if(chip.lpShadingTable == NULL)
    {
		   ...
		   unsigned short wWidth, unsigned short wLength, Bool isShading)
{
  ...
  Sane.Byte byDummyCycleNum = 1
  unsigned short Total_MotorDPI
  unsigned short BeforeScanFixSpeedStep = 0
  unsigned short BackTrackFixSpeedStep = 20
  unsigned short wMultiMotorStep = 1
  Sane.Byte bMotorMoveType = _MOTOR_MOVE_TYPE
  Sane.Byte isMotorMoveToFirstLine = MOTOR_MOVE_TO_FIRST_LINE_DISABLE
  Sane.Byte isUniformSpeedToScan = UNIFORM_MOTOR_AND_SCAN_SPEED_ENABLE
  Sane.Byte isScanBackTracking = SCAN_BACK_TRACKING_DISABLE
  ...
  Mustek_SendData(chip, ES01_F0_ScanImageStep0_7, (Sane.Byte) (TotalStep))
  Mustek_SendData(chip, ES01_F1_ScanImageStep8_15, (Sane.Byte) (TotalStep >> 8))
  Mustek_SendData(chip, ES01_F2_ScanImageStep16_19,
		   (Sane.Byte) (TotalStep >> 16))
