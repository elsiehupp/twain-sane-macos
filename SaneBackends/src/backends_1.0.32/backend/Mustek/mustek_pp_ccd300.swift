...
static void return_home_1015 (Mustek_pp_Handle * dev, Bool nowait)
...
{
  return(signed Int) (*(const Sane.Byte *) a) -
    (signed Int) (*(const Sane.Byte *) b)
}
...
static void
read_line_101x(Mustek_pp_Handle * dev, Sane.Byte * buf, Int pixel,
		Int RefBlack, Sane.Byte * calib, Int * gamma)
{
...
static void
read_average_line_101x(Mustek_pp_Handle * dev, Sane.Byte * buf, Int pixel,
			Int RefBlack)
{

  Sane.Byte lbuf[4][CCD300_MAXHSIZE * 2]
  Int ctr, sum
  mustek_pp_ccd300_priv *priv = dev.priv
...
{
  Sane.Byte buf[CCD300_MAXHSIZE * 2]
  Sane.Byte blackposition[5]
...
max_color_levels_101x(Mustek_pp_Handle * dev)
{
  ...
  Int ctr, line, sum
  Sane.Byte rbuf[32][CCD300_MAXHSIZE * 2]
  Sane.Byte gbuf[32][CCD300_MAXHSIZE * 2]
  Sane.Byte bbuf[32][CCD300_MAXHSIZE * 2]
  mustek_pp_ccd300_priv *priv = dev.priv
...
static void
get_grayscale_line_101x(Mustek_pp_Handle * dev, Sane.Byte * buf)
{

  Int skips
  mustek_pp_ccd300_priv *priv = dev.priv

  priv.line_diff += Sane.FIX(300.0 / (float) dev.res)

  skips = (priv.line_diff >> Sane.FIXED_SCALE_SHIFT)
...
static void
get_lineart_line_101x(Mustek_pp_Handle * dev, Sane.Byte * buf)
{

  Int ctr
  Sane.Byte gbuf[CCD300_MAXHSIZE * 2]
  mustek_pp_ccd300_priv *priv = dev.priv
...
static void
get_color_line_101x(Mustek_pp_Handle * dev, Sane.Byte * buf)
{

  Sane.Byte *red, *blue, *src, *dest
  Int gotline = 0, ctr
  ...
  do
    {
      ...
      if((priv.rdiff >> Sane.FIXED_SCALE_SHIFT) == priv.ccd_line)
	{
	  gored = 1
	  priv.rdiff += step
	}
	...
	gored = 0
      ...
      if((priv.bdiff >> Sane.FIXED_SCALE_SHIFT) == priv.ccd_line)
	{
	  goblue = 1
	  priv.bdiff += step
	}
	...
	goblue = 0
      ...
      if((priv.gdiff >> Sane.FIXED_SCALE_SHIFT) == priv.ccd_line)
	{
	  gogreen = 1
	  priv.gdiff += step
	}
      ...
      if(!gored && !goblue && !gogreen)
	{
	  ...
	  wait_bank_change(dev, priv.bank_count, 1)
	  reset_bank_count(dev)
	  if(priv.ccd_line >= (priv.line_step >> Sane.FIXED_SCALE_SHIFT))
	    priv.redline = (priv.redline + 1) % priv.green_offs
	  if(priv.ccd_line >=
	      priv.blue_offs + (priv.line_step >> Sane.FIXED_SCALE_SHIFT))
...
config_ccd_1013 (Mustek_pp_Handle * dev)
{
  ...
  if(dev.res != 0)
    priv.res_step = Sane.FIX((float) priv.hwres / (float) dev.res)
  ...
  sanei_pa4s2_writebyte(dev.fd, 6,
			 (dev.invert == Sane.TRUE ? 0x04 : 0x14))
...
static void
return_home_1015 (Mustek_pp_Handle * dev, Bool nowait)
{
...
static void
return_home(Mustek_pp_Handle * dev, Bool nowait)
{
  mustek_pp_ccd300_priv *priv = dev.priv
...
move_motor(Mustek_pp_Handle * dev, Int count, Int forward)
{
  ...
  DBG(5, "move_motor: %u steps(%s)\n", count,
       (forward == Sane.TRUE ? "forward" : "backward"))
...
static void
get_lineart_line(Mustek_pp_Handle * dev, Sane.Byte * buf)
{
  get_lineart_line_101x(dev, buf)
}

static void
get_grayscale_line(Mustek_pp_Handle * dev, Sane.Byte * buf)
{
...
static void
get_color_line(Mustek_pp_Handle * dev, Sane.Byte * buf)
{
...
static Sane.Status
ccd300_init(Int options, Sane.String_Const port,
...
static void
ccd300_capabilities(Int info, String * model,
...
static Sane.Status
ccd300_open(String port, Int caps, Int * fd)
{
  Sane.Status status
...
static void
ccd300_setup(Sane.Handle handle)
{
...
static void
ccd300_close(Sane.Handle handle)
{
...
static Sane.Status
ccd300_config(Sane.Handle handle, Sane.String_Const optname,
...
static void
ccd300_stop(Sane.Handle handle)
{
  Mustek_pp_Handle *dev = handle
  mustek_pp_ccd300_priv *priv = dev.priv
  Int cnt
...
static Sane.Status
ccd300_start(Sane.Handle handle)
{
  Mustek_pp_Handle *dev = handle
  mustek_pp_ccd300_priv *priv = dev.priv
  ...
  move_motor(dev, priv.top + dev.topY -
	      (dev.mode == MODE_COLOR ? priv.green_offs : 0), Sane.TRUE)
  ...
  if(dev.mode == MODE_COLOR)
    {
      Int failed = Sane.FALSE, cnt

      priv.line_step = Sane.FIX(300.0 / (float) dev.res)
      priv.rdiff = priv.line_step
      priv.bdiff = priv.rdiff + (priv.blue_offs << Sane.FIXED_SCALE_SHIFT)
      priv.gdiff =
	priv.rdiff + (priv.green_offs << Sane.FIXED_SCALE_SHIFT)

      priv.red = malloc(sizeof(Sane.Byte *) * priv.green_offs)
      priv.blue = malloc(sizeof(Sane.Byte *) * priv.blue_offs)
...
static void
ccd300_read(Sane.Handle handle, Sane.Byte * buffer)
{
  Mustek_pp_Handle *dev = handle
  mustek_pp_ccd300_priv *priv = dev.priv
