...
static Sane.String_Const mode_list_paragon[] = {
...
static Sane.String_Const mode_list_se[] = {
  Sane.VALUE_SCAN_MODE_LINEART,
  Sane.VALUE_SCAN_MODE_GRAY,
  Sane.VALUE_SCAN_MODE_COLOR,
  0
...
static Sane.String_Const bit_depth_list_pro[] = {
  "8", "12",
  0
]
...
static const Sane.Byte scsi_inquiry[] = {
  MUSTEK_SCSI_INQUIRY, 0x00, 0x00, 0x00, INQ_LEN, 0x00
]
static const Sane.Byte scsi_test_unit_ready[] = {
...
static const Sane.Byte scsi_lookup_table[] = {
...
attach(Sane.String_Const devname, Mustek_Device ** devp, Bool may_wait)
{
  ...
  Sane.Byte result[INQ_LEN]
  Sane.Byte inquiry_byte_list[50], inquiry_text_list[17]
  Sane.Byte inquiry_byte[5], inquiry_text[5]
  Sane.Byte *model_name = result + 44
  Mustek_Scanner s
  Mustek_Device *dev, new_dev
  Sane.Status status
  size_t size
  String scsi_device_type[] = {
    "Direct-Access", "Sequential-Access", "Printer", "Processor",
    "Write-Once", "CD-ROM", "Scanner", "Optical Memory", "Medium Changer",
    "Communications"
  ]
  Sane.Byte scsi_vendor[9]
  Sane.Byte scsi_product[17]
  Sane.Byte scsi_revision[5]
  Sane.Byte *pp
  Bool warning = Sane.FALSE
  ...
  if(warning == Sane.TRUE)
    {
      DBG(0,
	   ...
	   "(export Sane.DEBUG_MUSTEK=255) and send the output of\n  "
  ...
  if(option == OPT_RESOLUTION)
      ...
      if(s.hw.flags & MUSTEK_FLAG_THREE_PASS)
	{
	  /* The three pass scanners use a 0.5% of the maximum resolution
	     ...
	     if necessary.  */
	  Sane.Fixed max_dpi, quant, half_res
...
do_stop(Mustek_Scanner * s)
{
  ...
  if(sanei_thread_is_valid(s.reader_pid))
	...
	scan_time = 1
      ...
      if(s.total_bytes == s.params.lines * s.params.bytesPerLine)
	DBG(3, "Scanned %d bytes as expected\n", s.total_bytes)
      else if(s.total_bytes < s.params.lines * s.params.bytesPerLine)
	DBG(3, "Scanned %d bytes, expected %d bytes\n", s.total_bytes,
	     s.params.lines * s.params.bytesPerLine)
      else
	DBG(1, "Warning: Scanned %d bytes, but expected only %d bytes\n",
	     s.total_bytes, s.params.lines * s.params.bytesPerLine)
...
get_image_status(Mustek_Scanner * s, Int * bpl, Int * lines)
{
  ...
  *lines = s.hw.lines - offset
  ...
  DBG(3, "get_image_status: bytesPerLine=%d, lines=%d(offset = %d)\n",
       *bpl, *lines, offset)
  return Sane.STATUS_GOOD
}
      ...
      if((s.hw.flags & MUSTEK_FLAG_ENLARGE_X) && (res > half_res))
	{
	  ...
	  if(little_endian())
		...
		for(x_out = 0; x_out < s.params.pixels_per_line; x_out++)
		  {
		    x_in = x_out * bpl / s.params.bytesPerLine / 2
		    x_in *= 2
		    out_addr = out + y * s.params.bytesPerLine + x_out * 6
	  ...
	  for(y = 0; y < num_lines; y++)
	    {
	      for(x_out = 0; x_out < s.params.pixels_per_line; x_out++)
		{
		  x_in = x_out * bpl / s.params.bytesPerLine
		  out_addr = out + y * s.params.bytesPerLine + x_out * 3
...
init_options(Mustek_Scanner * s)
{
  ...
  s.opt[OPT_BRIGHTNESS_R].unit = Sane.UNIT_PERCENT
  s.opt[OPT_BRIGHTNESS_R].constraint_type = Sane.CONSTRAINT_RANGE
  ...
  s.opt[OPT_BRIGHTNESS_G].unit = Sane.UNIT_PERCENT
  s.opt[OPT_BRIGHTNESS_G].constraint_type = Sane.CONSTRAINT_RANGE
  ...
  s.opt[OPT_BRIGHTNESS_B].unit = Sane.UNIT_PERCENT
  s.opt[OPT_BRIGHTNESS_B].constraint_type = Sane.CONSTRAINT_RANGE
  s.opt[OPT_BRIGHTNESS_B].constraint.range = &percentage_range
  s.opt[OPT_BRIGHTNESS_B].cap |= Sane.CAP_INACTIVE
  ...
  s.opt[OPT_CONTRAST_R].unit = Sane.UNIT_PERCENT
  s.opt[OPT_CONTRAST_R].constraint_type = Sane.CONSTRAINT_RANGE
  s.opt[OPT_CONTRAST_R].constraint.range = &percentage_range
  s.opt[OPT_CONTRAST_R].cap |= Sane.CAP_INACTIVE
  ...
  s.opt[OPT_CONTRAST_G].unit = Sane.UNIT_PERCENT
  s.opt[OPT_CONTRAST_G].constraint_type = Sane.CONSTRAINT_RANGE
  s.opt[OPT_CONTRAST_G].constraint.range = &percentage_range
  s.opt[OPT_CONTRAST_G].cap |= Sane.CAP_INACTIVE
  ...
  s.opt[OPT_CONTRAST_B].unit = Sane.UNIT_PERCENT
  s.opt[OPT_CONTRAST_B].constraint_type = Sane.CONSTRAINT_RANGE
  s.opt[OPT_CONTRAST_B].constraint.range = &percentage_range
  s.opt[OPT_CONTRAST_B].cap |= Sane.CAP_INACTIVE
  ...
  /* halftone dimension */
  s.opt[OPT_HALFTONE_DIMENSION].name = Sane.NAME_HALFTONE_DIMENSION
  ...
  s.opt[OPT_HALFTONE_PATTERN].type = Sane.TYPE_INT
  s.opt[OPT_HALFTONE_PATTERN].cap |= Sane.CAP_INACTIVE
  s.opt[OPT_HALFTONE_PATTERN].constraint_type = Sane.CONSTRAINT_RANGE
	     ...
	     Sane.Byte * extra)
{
  ...
  if((s.mode & MUSTEK_MODE_COLOR)
	...
	fwrite(extra, num_lines, s.params.bytesPerLine, fp)
    }
  else
    {
	  ...
	  (s.val[OPT_RESOLUTION].w > (s.hw.dpi_range.max / 2)))
	{
	  ...
	  DBG(5, "output_data: enlarge lines from %d bpl to %d bpl\n",
	       s.hw.bpl, s.params.bytesPerLine)
      ...
      while(1)
	{
		  ...
		  if(status == Sane.STATUS_NO_MEM)
		    {
		      DBG(1,
			   ...
			   "or\n         use Sane.SG_BUFFERSIZE variable. "
	  ...
	  if((s.line < s.hw.lines) && (buffer_count < max_buffers))
		...
		bstat[buffernumber].finished = Sane.TRUE
	      ...
	      bstat[buffernumber].ready = Sane.TRUE
...
Sane.init(Int * version_code, Sane.Auth_Callback authorize)
{
  ...
  num_devices = 0
  force_wait = Sane.FALSE
  disable_double_buffering = Sane.FALSE
  first_dev = 0
  first_handle = 0
  devlist = 0
  new_dev = 0
...
Sane.start(Sane.Handle handle)
{
  ...
  if(s.hw.flags & MUSTEK_FLAG_SE)
      ...
      if(status != Sane.STATUS_GOOD)
	{
      ...
      status = get_window(s, &s.params.bytesPerLine,
	...
	goto stop_scanner_and_return
      ...
      status = get_image_status(s, &s.params.bytesPerLine,
				 &s.params.lines)
      if(status != Sane.STATUS_GOOD)
	goto stop_scanner_and_return
