...
#ifndef LOBYTE
#define LOBYTE(w) (Sane.Byte)((unsigned short)(w) & 0x00ff)
#endif

#ifndef HIBYTE
#define HIBYTE(w) (Sane.Byte)((unsigned short)(w)>>8 & 0x00ff)
#endif
...
typedef struct
{
  ...
  Sane.Byte LineGap
  Sane.Byte TG_Pulse_Width_Pixel
  Sane.Byte TG_Wait_Width_Pixel
  unsigned short Multi_TG_Dummy_Pixel
  unsigned short CCD_Dummy_Pixel
  Sane.Byte Dummy_Cycle
  Sane.Byte TG_Times
...
typedef struct
{
  ...
  Sane.Byte AFE_Secondary_FF_LatchPos
  /* Sensor */
  unsigned Int CCD_DummyCycleTiming
  Sane.Byte PHTG_PluseWidth
  Sane.Byte PHTG_WaitWidth
  ...
  Sane.Byte PHTG_TimingAdj
  Sane.Byte PHTG_TimingSetup
  ...
  Sane.Byte DE_CCD_SETUP_REGISTER_1200
  ...
  Sane.Byte DE_CCD_SETUP_REGISTER_600
...
typedef struct
{
  ...
  Sane.Byte ImageEndAddr_0
  Sane.Byte ImageEndAddr_1
  Sane.Byte ImageEndAddr_2

  Sane.Byte ImageFullBank_0
  Sane.Byte ImageFullBank_1
}
RamPosition
...
typedef struct
{
  ...
  Sane.Byte isMotorGoToFirstLine;	/*Roy add */
  Sane.Byte * lpShadingTable;	/*Roy add */
  Sane.Byte isUniformSpeedToScan
}
Asic, *PAsic
...
#define DBG_CRIT 	0	/* Critical errors thatshould be printed even
				   ...
				   with care and only after Sane.open has been
				   called */
...
typedef struct tagMOTOR_CURRENT_AND_PHASE
{
  Sane.Byte MoveType
  Sane.Byte FillPhase
  Sane.Byte MotorDriverIs3967
  Sane.Byte MotorCurrentTableA[32]
  Sane.Byte MotorCurrentTableB[32]
  Sane.Byte MotorPhaseTable[32]
} MOTOR_CURRENT_AND_PHASE, LPMOTOR_CURRENT_AND_PHASE

typedef struct tagLLF_RAMACCESS
{
  Sane.Byte ReadWrite
  Sane.Byte IsOnChipGamma
  unsigned short LoStartAddress
  unsigned short HiStartAddress
  Int RwSize
  Sane.Byte DramDelayTime
  Sane.Byte *BufferPtr
} LLF_RAMACCESS
...
typedef struct tagLLF_SETMOTORTABLE
{
  ...
  Sane.Byte MotorTableAddress
  unsigned short *MotorTablePtr
} LLF_SETMOTORTABLE

typedef struct tagLLF_MOTORMOVE
{
  Sane.Byte ActionMode;		/* 0: AccDec Mode, 1: Uniform Speed Mode, 2: Test Mode */
  Sane.Byte ActionType;		/* 0: Forward, 1: Backward, 2:Back To Home */
  Sane.Byte MotorSelect;		/* 0: Motor 0 only, 1: Motor 1 only, 2: Motor 0 & 1 */
  Sane.Byte HomeSensorSelect;	/* 0: Sensor 0, 1: Sensor 1, 2: Sensor 0 & 1, 3:Invert Sensor 1 */
  unsigned short FixMoveSpeed
  unsigned Int FixMoveSteps;		/* 3 bytes */
  Sane.Byte MotorSpeedUnit
  Sane.Byte MotorSyncUnit
  unsigned short AccStep;			/*Max = 511 */
  Sane.Byte DecStep;			/* Max = 255 */
  Sane.Byte MotorMoveUnit
  Sane.Byte WaitOrNoWait;		/* 0: no wait, 1: wait */
  Sane.Byte Lamp0PwmFreq;		/* Lamp0 PWM freq */
  Sane.Byte Lamp1PwmFreq;		/* Lamp1 PWM freq */
