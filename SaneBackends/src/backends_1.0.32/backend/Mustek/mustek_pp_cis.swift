...
Mustek_PP_1015_wait_bit(Mustek_PP_CIS_dev * dev, Mustek_PP_1015R_reg reg,
                        Mustek_PP_1015R_bit bit, Bool on, unsigned period)
{
   Sane.Byte tmp
   Sane.Byte mask, val
   Int tries = 0
...
static void
Mustek_PP_1015_write_reg(Mustek_PP_CIS_dev * dev, Mustek_PP_1015W_reg reg, Sane.Byte val)
{

   Sane.Byte regBank = (reg & 0xF0) >> 4
   Sane.Byte regNo   = (reg & 0x0F)
...
Mustek_PP_1015_write_reg3(Mustek_PP_CIS_dev * dev, Mustek_PP_1015W_reg reg,
               Sane.Byte val1, Sane.Byte val2, Sane.Byte val3)
{
   Sane.Byte regBank = (reg & 0xF0) >> 4
   Sane.Byte regNo   = (reg & 0x0F)
 ...
 *****************************************************************************/
static void
Mustek_PP_1015_write_reg_val(Mustek_PP_CIS_dev * dev, Sane.Byte val)
{
#ifdef M1015_TRACE_REGS
   Sane.Byte regBank = (dev.CIS.regs.current_write_reg & 0xF0) >> 4
   Sane.Byte regNo   = (dev.CIS.regs.current_write_reg & 0x0F)
 ...
 *****************************************************************************/
static void
Mustek_PP_1015_send_command(Mustek_PP_CIS_dev * dev, Sane.Byte command)
{
   assert (command & 0x04)
...
cis_set_sti(Mustek_PP_CIS_dev *dev)
{
   ...
   dev.bank_count++
   dev.bank_count &= (dev.CIS.use8KBank == Sane.TRUE) ? 3 : 7
}
...
cis_wait_bank_change (Mustek_PP_CIS_dev * dev, Int bankcount)
{
  ...
  if (dev.bank_count != bankcount && dev.desc.state != STATE_CANCELLED)
  {
   ...
   return dev.bank_count == bankcount ? Sane.TRUE : Sane.FALSE
}
...
cis_set_ccd_channel (Mustek_PP_CIS_dev * dev)
{
   ...
   Sane.Byte codes[] = { 0x84, 0x44, 0xC4 ]
   Sane.Byte chancode
...
{
   Int skipCount, byteCount

   if (dev.CIS.res != 0)
     dev.CIS.hres_step =
       Sane.FIX ((float) dev.CIS.hw_hres / (float) dev.CIS.res)
   ...
   */
   ...
   if (dev.CIS.setParameters == Sane.TRUE)
   {
      /*
   ...
   if (dev.CIS.setParameters == Sane.TRUE)
   {
      ...
      dev.CIS.setParameters = Sane.FALSE
   }
   else
   {
...
static void
cis_move_motor (Mustek_PP_CIS_dev * dev, Int steps) /* steps @ maxres */
{
   /* Note: steps is expressed at maximum resolution */
   Sane.Byte fullStep = 0x13, biStep = 0x73, quadStep = 0x7B
   Int fullSteps, biSteps, quadSteps
   /*
    * During a multi-step feed, the expose time is fixed. The value depends
    * on the type of the motor (600/1200 CP)
    */
   Sane.Byte savedExposeTime = dev.CIS.exposeTime
...
cis_wait_read_ready (Mustek_PP_CIS_dev * dev)
{
   ...
   {
      if (!cis_wait_next_channel(dev)) return Sane.FALSE
   }
   return Sane.TRUE
}
...
*/
static void
cis_read_line_low_level (Mustek_PP_CIS_dev * dev, Sane.Byte * buf,
                         Int pixel, Sane.Byte * calib_low,
                         Sane.Byte * calib_hi, Int * gamma)
{
   u_char color
   Int ctr, skips = dev.CIS.adjustskip, cval
   Int bpos = 0
   Sane.Byte low_val = 0, hi_val = 255
   ...
   else if (dev.CIS.hw_hres > dev.CIS.res)
   {
      ...
      do
      {
         ...
         cval = color
         if (ctr < (pos >> Sane.FIXED_SCALE_SHIFT))
	 {
	    ctr++
	    continue
	 }
   ...
   else
   {
	 ...
	 cval = color
	    ...
	    cval = gamma[cval]
         ...
         if ((pos >> Sane.FIXED_SCALE_SHIFT) >= nextPos)
         {
            nextPos++
...
static Bool
cis_read_line (Mustek_PP_CIS_dev * dev, Sane.Byte* buf, Int pixel,
               Bool raw)
{
   if (!dev.CIS.dontIncRead)
      CIS_INC_READ(dev)
   else
      dev.CIS.dontIncRead = Sane.FALSE
...
static void
cis_get_next_line (Mustek_PP_CIS_dev * dev, Sane.Byte * buf)
{
   Sane.Byte *dest, *tmpbuf = dev.tmpbuf
   ...
   {
      dev.ccd_line++
      if ((dev.line_diff >> Sane.FIXED_SCALE_SHIFT) != dev.ccd_line)
      {
         cis_motor_forward (dev)
         continue
      }
...
static void
cis_get_grayscale_line (Mustek_PP_CIS_dev * dev, Sane.Byte * buf)
{
   cis_get_next_line(dev, buf)
}

static void
cis_get_lineart_line (Mustek_PP_CIS_dev * dev, Sane.Byte * buf)
{
   Int ctr
...
static void
cis_get_color_line (Mustek_PP_CIS_dev * dev, Sane.Byte * buf)
{
   cis_get_next_line(dev, buf)
}
...
static Int
cis_check_result(Sane.Byte* buffer, Int pixel)
{
   var i: Int, maxVal = 0
...
cis_maximize_dynamic_range(Mustek_PP_CIS_dev * dev)
{
   ...
   var i: Int, j, pixel, channel, minExposeTime, first, last
   Sane.Byte powerOnDelayLower[3], powerOnDelayUpper[3], exposeTime[3]
   Sane.Byte buf[3][MUSTEK_PP_CIS_MAX_H_PIXEL]
   Int pixels = dev.calib_pixels
   ...
   dev.CIS.setParameters  = Sane.TRUE
...
static Bool
cis_measure_extremes(Mustek_PP_CIS_dev * dev, Sane.Byte* calib[3],
		     Int pixels, Int first, Int last)
{
   Sane.Byte buf[3][MUSTEK_PP_CIS_MAX_H_PIXEL]
   Sane.Byte min[3][MUSTEK_PP_CIS_MAX_H_PIXEL]
   Sane.Byte max[3][MUSTEK_PP_CIS_MAX_H_PIXEL]
   Int  sum[3][MUSTEK_PP_CIS_MAX_H_PIXEL]
   Int channel, cnt, p

   memset((void*)&min, 255, 3*MUSTEK_PP_CIS_MAX_H_PIXEL*sizeof(Sane.Byte))
   memset((void*)&max,   0, 3*MUSTEK_PP_CIS_MAX_H_PIXEL*sizeof(Sane.Byte))
   memset((void*)&sum,   0, 3*MUSTEK_PP_CIS_MAX_H_PIXEL*sizeof(Int))
...
{
   Sane.Byte cal_low, cal_hi 
   Sane.Byte powerOnDelay[3] 
   Int pixels = dev.calib_pixels
   Int channel, p, first, last
...
{
   Sane.Byte buf[2][2048]
   unsigned i, j, d
   Int saved_res
   Bool error = Sane.FALSE
   ...
   for (d = 0; d < 75 /* 255 */  && dev.desc.state != STATE_CANCELLED; d += 5)
   {
      ...
      for (i=0; i<5; ++i)
      {
         ...
         if (dev.desc.state == STATE_CANCELLED) return Sane.FALSE
      }
...
static void
cis_return_home (Mustek_PP_CIS_dev * dev, Bool nowait)
{
   Sane.Byte savedExposeTime = dev.CIS.exposeTime
...
{
   var i: Int, saved_res = dev.CIS.res, saved_vres = dev.CIS.hw_vres
...
static Sane.Status cis_attach(Sane.String_Const port,
...
Sane.Status cis600_drv_init(Int options, Sane.String_Const port,
...
Sane.Status cis1200_drv_init(Int options, Sane.String_Const port,
		Sane.String_Const name, Sane.Attach_Callback attach)
{
   if (options != CAP_NOTHING)
      return Sane.STATUS_INVAL
...
Sane.Status cis1200p_drv_init(Int options, Sane.String_Const port,
		Sane.String_Const name, Sane.Attach_Callback attach)
{
   if (options != CAP_NOTHING)
      return Sane.STATUS_INVAL
...
* Capabilities                                                                *
******************************************************************************/
void cis_drv_capabilities(Int info, String *model,
...
* Open                                                                        *
******************************************************************************/
Sane.Status cis_drv_open (String port, Int caps, Int *fd)
{
   Sane.Status status
...
* Setup                                                                       *
******************************************************************************/
void cis_drv_setup (Sane.Handle hndl)
{
   ...
   /* Default values for configurable parameters; configuration file
      may override them. */
   cisdev.fast_skip = Sane.TRUE
   cisdev.bw_limit = 127
   cisdev.calib_mode = Sane.FALSE
...
* Config                                                                      *
******************************************************************************/
Sane.Status cis_drv_config(Sane.Handle hndl, Sane.String_Const optname,
...
* Close                                                                       *
******************************************************************************/
void cis_drv_close (Sane.Handle hndl)
{
   Mustek_pp_Handle *dev = hndl
   Mustek_PP_CIS_dev *cisdev = dev.priv
...
* Start                                                                       *
******************************************************************************/
Sane.Status cis_drv_start (Sane.Handle hndl)
{
   Mustek_pp_Handle *dev = hndl
   Mustek_PP_CIS_dev *cisdev = dev.priv
   Int pixels = dev.params.pixels_per_line
   ...
   cisdev.CIS.setParameters = Sane.FALSE
   cisdev.CIS.use8KBank = Sane.TRUE
   ...
   cis_reset_device (cisdev)
   cis_return_home (cisdev, Sane.TRUE); /* no wait */
   ...
   if (!cis_calibrate (cisdev))
   {
      ...
      free(cisdev.tmpbuf); cisdev.tmpbuf = NULL
      free(cisdev); dev.priv = NULL
      return Sane.STATUS_CANCELLED; /* Most likely cause */
   }
...
/*   M1015_DISPLAY_REGS(dev, "after calibration"); */
   ...
   cisdev.CIS.setParameters = Sane.TRUE
   ...
   cisdev.CIS.line_step =
     Sane.FIX ((float) cisdev.CIS.hw_vres / (float) cisdev.CIS.res)
...
* Read                                                                        *
******************************************************************************/
void cis_drv_read (Sane.Handle hndl, Sane.Byte *buffer)
{
   Mustek_pp_Handle *dev = hndl
   Mustek_PP_CIS_dev *cisdev = dev.priv
...
* Stop                                                                        *
******************************************************************************/
void cis_drv_stop (Sane.Handle hndl)
{
   Mustek_pp_Handle *dev = hndl
   Mustek_PP_CIS_dev *cisdev = dev.priv
