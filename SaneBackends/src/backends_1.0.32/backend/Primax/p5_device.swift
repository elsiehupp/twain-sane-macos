 ...
 * @return Sane.TRUE on success, Sane.FALSE otherwise
 */
static Int
 ...
 * @return Sane.TRUE in case of success, Sane.FALSE otherwise
 */
static Int
connect(Int fd)
{
 ...
 * @return Sane.STATUS_NO_DOCS if no document or Sane.STATUS_GOOD
...
{
  unsigned var i: Int, j, shift, step
...
read_line(P5_Device * dev, uint8_t * data, size_t length, Int ltr,
	   Bool retry, Bool x2, Int mode, Bool correction)
{
  uint8_t counter, read, cnt
  uint8_t inbuffer[MAX_SENSOR_PIXELS * 2 * 3 + 2]
  unsigned var i: Int, factor
  float val
...
move(P5_Device * dev)
{
  ...
  do
	...
	{
	  read =
	    read_line(dev, buffer, 256, 1, Sane.FALSE, Sane.FALSE,
		       MODE_GRAY, Sane.FALSE)
	  if(read == -1)
	    {
...
is_black_line(uint8_t * buffer, unsigned Int pixels, Int mode)
{
  ...
  if(count > (3 * width) / 100)
    {
      DBG(DBG_io, "is_black_line=Sane.FALSE\n")
      return Sane.FALSE
    }

  DBG(DBG_io, "is_black_line=Sane.TRUE\n")
  return Sane.TRUE
}
...
is_white_line(uint8_t * buffer, unsigned Int pixels, Int mode)
{
  ...
  if(count > (3 * width) / 100)
    {
      DBG(DBG_io, "is_white_line=Sane.FALSE\n")
      return Sane.FALSE
    }

  DBG(DBG_io, "is_white_line=Sane.TRUE\n")
  return Sane.TRUE
}
 ...
 * @return Sane.STATUS_GOOD on success, otherwise error code
 */
static Sane.Status
...
sheetfed_calibration(P5_Device * dev)
{
  ...
  unsigned var i: Int, j, k, dpi, pixels, read, black, white
  float coeff
  ...
  do
    {
      status = test_document(dev.fd)
      if(status == Sane.STATUS_NO_DOCS && (white < 10 || black < 10))
	{
	  cleanup_calibration(dev)
	  DBG(DBG_error,
      ...
      line =
	read_line(dev, buffer, pixels * 3, 1, Sane.FALSE, Sane.FALSE,
		   MODE_COLOR, Sane.FALSE)
      if(line == -1)
	{
  ...
  while(test_document(dev.fd) != Sane.STATUS_NO_DOCS)
