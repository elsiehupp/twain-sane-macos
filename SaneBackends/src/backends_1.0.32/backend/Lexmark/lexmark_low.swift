...
import lexmark_sensors.c"
import lexmark_models.c"
...
low_poll_data(Int devnum)
{
  ...
  Int loops = 0
  size_t size
  static Sane.Byte command4_block[] = { 0x90, 0x00, 0x00, 0x03 ]
  Sane.Byte result[3]
  Sane.Word count
...
low_write_all_regs(Int devnum, Sane.Byte * regs)
{
  ...
  Sane.Status status
  size_t size
  static Sane.Byte command_block1[0xb7]
  static Sane.Byte command_block2[0x4f]
			    ...
			    Sane.Byte * regs)
{
  ...
  static Sane.Byte pollstopmoving_command_block[] =
    { 0x80, 0xb3, 0x00, 0x01 ]
...
sanei_lexmark_low_search_home_fwd(Lexmark_Device * dev)
{
  ...
  Int devnum
  var i: Int
  Sane.Byte poll_result[3]
  Sane.Byte *buffer
  Sane.Byte temp_byte

  static Sane.Byte command4_block[] = { 0x90, 0x00, 0x00, 0x03 ]

  static Sane.Byte command5_block[] = { 0x91, 0x00, 0x09, 0xc4 ]
...
/* This function must only be called if the scan head is past the home dot.
  ...
  static Sane.Byte command5_block[] = { 0x91, 0x00, 0xff, 0xc0 ]
#ifdef DEEP_DEBUG
  FILE *img = NULL
#endif
  ...
  while(buffer_count < buffer_limit)
	...
	no_of_buffers = 10
      ...
      command5_block[1] = (Sane.Byte) (size_requested >> 16)
      command5_block[2] = (Sane.Byte) (size_requested >> 8)
...
sanei_lexmark_low_find_start_line(Lexmark_Device * dev)
{
  ...
  static Sane.Byte command5_block[] = { 0x91, 0x00, 0x14, 0x48 ]
...
sanei_lexmark_low_start_scan(Lexmark_Device * dev)
{
  ...
  static Sane.Byte command5_block[] = { 0x80, 0xb3, 0x00, 0x01 ]
				  ...
				  Lexmark_Device * dev)
{
  ...
  static Sane.Byte command1_block[] = { 0x91, 0x00, 0xff, 0xc0 ]
  ...
  if(dev.transfer_buffer == NULL)
      ...
      if(dev.bytes_remaining > 0)
	{
	  ...
	  command1_block[2] = (Sane.Byte) (xfer_request >> 8)
...
read_buffer_init(Lexmark_Device * dev, Int bytesperline)
{
  ...
  dev.read_buffer.empty = Sane.TRUE
...
sanei_lexmark_low_offset_calibration(Lexmark_Device * dev)
{
  ...
  Sane.Byte *data = NULL
  Sane.Byte top[OFFSET_RANGES] = { 0, 0x7f, 0x9f, 0xbf, 0xff ]
#ifdef DEEP_DEBUG
  char title[20]
#endif
