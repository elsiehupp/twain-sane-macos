...
static const Sane.Device **st400_device_array = NULL
...
/* prototypes */
static Sane.Status st400_inquiry( Int fd, ST400_Model **modelP )
static Sane.Status st400_sense_handler( Int fd, Sane.Byte *result, void *arg )
static Sane.Status st400_attach( const char *devname, ST400_Device **devP )
static Sane.Status st400_attach_one( const char *device )
static void st400_init_options( ST400_Device *dev )
static Sane.Status st400_set_window( ST400_Device *dev )
static Sane.Status st400_wait_ready( Int fd )
static Sane.Status st400_cmd6( Int fd, Sane.Byte cmd, Sane.Byte ctrl )
static Sane.Status st400_read10( Int fd, Sane.Byte *buf, size_t *lenP )
...
#define set24(m, x)	do {										\
	*((Sane.Byte *)(m)+0) = (Sane.Byte)(((x) >> 16) & 0xff);	\
	*((Sane.Byte *)(m)+1) = (Sane.Byte)(((x) >> 8) & 0xff);		\
	*((Sane.Byte *)(m)+2) = (Sane.Byte)(((x) >> 0) & 0xff);		\
} while(0)
#define set16(m, x)	do {										\
	*((Sane.Byte *)(m)+0) = (Sane.Byte)(((x) >> 8) & 0xff);		\
	*((Sane.Byte *)(m)+1) = (Sane.Byte)(((x) >> 0) & 0xff);		\
} while(0)
...
{
	struct { Sane.Byte cmd, lun, reserved[2], tr_len, ctrl; } scsi_cmd
/*
	struct {
		Sane.Byte devtype, devqual, version
		Sane.Byte reserved1, additionallength
		Sane.Byte reserved2[2], flags
		Sane.Byte vendor[8], product[16], release[4]
		Sane.Byte reserved3[60]
	} inqdata
*/
	struct { Sane.Byte bytes[96]; } inqdata
...
static Sane.Status
st400_cmd6( Int fd, Sane.Byte cmd, Sane.Byte ctrl )
{
	struct { Sane.Byte cmd, lun, reserved[2], tr_len, ctrl; } scsi_cmd
	Sane.Status status
...
st400_set_window( ST400_Device *dev )
{
	...
	struct {
		/* 10byte command */
		Sane.Byte cmd, lun, reserved1[4], tr_len[3], ctrl

		/* 40byte window struct */
		Sane.Byte reserved2[6], wd_len[2], winnr, reserved3
		Sane.Byte x_res[2], y_res[2];			/* resolution: 200, 300, 400 */
		Sane.Byte x_ul[2], y_ul[2];				/* upper left corner */
		Sane.Byte width[2], height[2]
		Sane.Byte reserved4, threshold
		Sane.Byte reserved5, halftone;			/* ht: 0 or 2 */
		Sane.Byte bitsperpixel, reserved6[13];	/* bpp: 1 or 8 */
	} scsi_cmd
	...
	yoff = 6
	th = (double)maxval(dev.model.bits) * Sane.UNFIX(dev.val[OPT_THRESHOLD]) / 100.0
...
static Sane.Status
st400_read10( Int fd, Sane.Byte *buf, size_t *lenP )
{
	struct { Sane.Byte cmd, lun, res[4], tr_len[3], ctrl; } scsi_cmd
	Sane.Status status
...
static Sane.Status
st400_sense_handler( Int fd, Sane.Byte *result, void *arg )
{
...
st400_attach( const char *devname, ST400_Device **devP )
{
	...
	dev.sane.vendor = model.Sane.vendor
	dev.sane.model = model.Sane.model
	dev.sane.type = model.Sane.type
	dev.status.open = 0
...
st400_config_do_option(char *opt, size_t linenum)
{
	...
	if( strncmp(opt, "scanner_resolutions", 19) == 0 && isspace(opt[19]) ) {
		opt += 20
		st400_devices.model.dpi_list = malloc(16 * sizeof(Int))
		i = 0
		do {
			status = st400_config_get_arg(&opt, &arg, linenum)
			if( status == Sane.STATUS_GOOD ) {
				++i
				st400_devices.model.dpi_list[i] = (Int)arg
			}
		} while( status == Sane.STATUS_GOOD && i < 15 )
...
Sane.Status
Sane.init( Int *versionP, Sane.Auth_Callback authorize )
{
	FILE *fp
	Sane.Status status

	DBG_INIT()
	DBG(DCODE, "Sane.init: version %s null, authorize %s null\n", (versionP) ? "!=" : "==", (authorize) ? "!=" : "==")

	if( versionP != NULL )
		*versionP = Sane.VERSION_CODE(Sane.CURRENT_MAJOR, V_MINOR, 0)
	...
	if( (fp = sanei_config_open(ST400_CONFIG_FILE)) != NULL ) {
		...
		linenum = 0
		DBG(DCODE, "Sane.init: reading config file\n")
		while( sanei_config_read(line, sizeof(line), fp) ) {
			...
			if( strncmp(str, "option", 6) == 0 && isspace(str[6]) ) {
				DBG(DCODE, "Sane.init: config line <%s>\n", line)
				status = st400_config_do_option(str+7, linenum)
			}
			else {
				DBG(DCODE, "Sane.init: attaching device <%s>\n", line)
				sanei_config_attach_matching_devices(line, st400_attach_one)
			}
			if( status != Sane.STATUS_GOOD )
				break
		}
		DBG(DCODE, "Sane.init: closing config file\n")
		fclose(fp)
	}

	if( status == Sane.STATUS_GOOD && st400_devices == NULL ) {
		DBG(DCODE, "Sane.init: attaching default device <%s>\n", ST400_DEFAULT_DEVICE)
...
Sane.exit( void )
{
	...
	if( st400_device_array ) {
		DBG(DCODE, "Sane.exit: freeing device array\n")
...
Sane.Status
Sane.get_devices( const Sane.Device ***devarrayP, Bool local_only )
{
	ST400_Device *dev
	unsigned var i: Int

	DBG(DCODE, "Sane.get_devices(%p, %d)\n", (void *) devarrayP, (Int)local_only)

	if( !st400_status.array_valid ) {
		if( st400_device_array ) {
			DBG(DCODE, "Sane.get_devices: freeing old device array\n")
			free(st400_device_array)
		}
		st400_device_array = malloc((st400_num_devices + 1) * sizeof(*st400_device_array))
		if( !st400_device_array )
			return Sane.STATUS_NO_MEM
		DBG(DCODE, "Sane.get_devices: new device array at %p\n", (void *) st400_device_array)
	...
	DBG(DCODE, "Sane.get_devices: %u entries in device array\n", st400_num_devices)
	if( devarrayP )
...
Sane.Status
Sane.open( Sane.String_Const devicename, Sane.Handle *handleP )
{
	ST400_Device *dev
	Sane.Status status

	DBG(DCODE, "Sane.open(%s, %p)\n", devicename, (void *) handleP)
...
st400_reset_options( ST400_Device *dev )
{
    ...
    dev.val[OPT_THRESHOLD]	= Sane.FIX(50.0)
	dev.opt[OPT_THRESHOLD].cap &= ~Sane.CAP_INACTIVE
    dev.val[OPT_TL_X]		= Sane.FIX(0.0)
	dev.val[OPT_TL_Y]		= Sane.FIX(0.0)
	dev.val[OPT_BR_X]		= Sane.FIX(0.0)
	dev.val[OPT_BR_Y]		= Sane.FIX(0.0)
...
{
	static const Int depth_list[] = { 2, 1, 8 ]
	static const Int dpi_list[] = { 3, 200, 300, 400 ]
	static const Sane.Range thres_range = {
		Sane.FIX(0.0), Sane.FIX(100.0), Sane.FIX(0.0)
	]
	static const Sane.Range x_range = {
		Sane.FIX(0.0), Sane.FIX(ST400_MAX_X * MM_PER_INCH), Sane.FIX(0.0)
	]
	static const Sane.Range y_range = {
		Sane.FIX(0.0), Sane.FIX(ST400_MAX_Y * MM_PER_INCH), Sane.FIX(0.0)
	]
	...
	dev.opt[OPT_NUM_OPTS].unit	= Sane.UNIT_NONE
	dev.opt[OPT_NUM_OPTS].size = sizeof(Sane.Word)
	dev.opt[OPT_NUM_OPTS].cap	= Sane.CAP_SOFT_DETECT
	dev.opt[OPT_NUM_OPTS].constraint_type = Sane.CONSTRAINT_NONE
	...
	dev.opt[OPT_RESOLUTION].size = sizeof(Sane.Word)
	...
	dev.opt[OPT_DEPTH].name	= Sane.NAME_BIT_DEPTH
	dev.opt[OPT_DEPTH].title	= Sane.TITLE_BIT_DEPTH
	dev.opt[OPT_DEPTH].desc	= Sane.DESC_BIT_DEPTH
	dev.opt[OPT_DEPTH].type	= Sane.TYPE_INT
	dev.opt[OPT_DEPTH].unit	= Sane.UNIT_BIT
	dev.opt[OPT_DEPTH].size	= sizeof(Sane.Word)
	dev.opt[OPT_DEPTH].cap	= Sane.CAP_SOFT_SELECT|Sane.CAP_SOFT_DETECT
	dev.opt[OPT_DEPTH].constraint_type = Sane.CONSTRAINT_WORD_LIST
	...
	dev.opt[OPT_THRESHOLD].type	= Sane.TYPE_FIXED
	dev.opt[OPT_THRESHOLD].unit	= Sane.UNIT_PERCENT
	dev.opt[OPT_THRESHOLD].size	= sizeof(Sane.Word)
	...
	dev.opt[OPT_TL_X].size	= sizeof(Sane.Word)
	dev.opt[OPT_TL_X].cap	= Sane.CAP_SOFT_SELECT|Sane.CAP_SOFT_DETECT
	...
	dev.opt[OPT_TL_Y].size	= sizeof(Sane.Word)
	dev.opt[OPT_TL_Y].cap	= Sane.CAP_SOFT_SELECT|Sane.CAP_SOFT_DETECT
	...
	dev.opt[OPT_BR_X].size	= sizeof(Sane.Word)
	dev.opt[OPT_BR_X].cap	= Sane.CAP_SOFT_SELECT|Sane.CAP_SOFT_DETECT
	...
	dev.opt[OPT_BR_Y].size	= sizeof(Sane.Word)
	dev.opt[OPT_BR_Y].cap	= Sane.CAP_SOFT_SELECT|Sane.CAP_SOFT_DETECT
...
Sane.get_option_descriptor( Sane.Handle handle, Int optnum )
{
	...
	DBG(DOPT, "Sane.get_option_descriptor(%p, %d)\n", handle, (Int)optnum)
			...
			switch( optnum ) {
				case OPT_DEPTH:
					if( *(Sane.Word *)valP != 1 )
						...
						dev.opt[OPT_THRESHOLD].cap &= ~Sane.CAP_INACTIVE
					if( infoP )
						*infoP |= Sane.INFO_RELOAD_OPTIONS
					/* fall through */
				case OPT_RESOLUTION:
				case OPT_TL_X:
				case OPT_TL_Y:
				...
				case OPT_BR_Y:
					if( infoP )
						*infoP |= Sane.INFO_RELOAD_PARAMS
					/* fall through */
				case OPT_THRESHOLD:
					dev.val[optnum] = *(Sane.Word *)valP
					break
				default:
					return Sane.STATUS_INVAL
			}
...
Sane.Status
Sane.get_parameters( Sane.Handle handle, Sane.Parameters *paramsP )
{
	ST400_Device *dev = handle

	DBG(DCODE, "Sane.get_parameters(%p, %p)\n", handle, (void *) paramsP)
	...
	if( !dev.status.scanning ) {
		...
		width  = Sane.UNFIX(dev.val[OPT_BR_X] - dev.val[OPT_TL_X])
		height = Sane.UNFIX(dev.val[OPT_BR_Y] - dev.val[OPT_TL_Y])
...
Sane.start( Sane.Handle handle )
{
	...
	status = Sane.get_parameters(dev, NULL)
	if( status != Sane.STATUS_GOOD )
		return status
...
Sane.Status
Sane.read( Sane.Handle handle, Sane.Byte *buf, Int maxlen, Int *lenP )
{
	ST400_Device *dev = handle
	Sane.Status status
	size_t r, i
	...
	*lenP = 0
	...
	while( maxlen > 0 ) {
		...
		r = min((Int) dev.bytes_in_buffer, maxlen)
		...
		if( dev.val[OPT_DEPTH] == 1 || dev.model.bits == 8 ) {
			 ...
			 * reasonably fast.
			 */
			mv = (Sane.Byte)maxval(dev.model.bits)
...
Sane.Status
Sane.set_io_mode( Sane.Handle handle, Bool nonblock )
{
	DBG(DCODE, "Sane.set_io_mode(%p, %d)\n", handle, (Int)nonblock)

	if( nonblock == Sane.TRUE )
		return Sane.STATUS_UNSUPPORTED
	return Sane.STATUS_GOOD
}
...
{
	DBG(DCODE, "Sane.get_select_fd(%p, %p)\n", handle, (void *) fdP)
