 ...
 * instead of Sane.CURRENT_MAJOR in Sane.init()
 ...
 * OPT_NUM_OPTS must be of type Sane.TYPE_INT.
 *
 ...
 *  	  backend/st400.c: Moved import Sane.config to the beginning.
...
static const Int x_res_list[] =
{11, 0, 60, 75, 80, 100, 120, 150, 200, 240, 300, 600]

static const Int y_res_list[] =
{11, 0, 60, 75, 80, 100, 120, 150, 200, 240, 300, 600]

static const char lineStr[] = Sane.VALUE_SCAN_MODE_LINEART
static const char halfStr[] = Sane.VALUE_SCAN_MODE_HALFTONE
...
{0, Sane.FIX(295), 1]
static const Sane.Range y_range_adf =
{0, Sane.FIX(356), 1]

/* ################# externally visible routines ################{ */


Sane.Status                     /* looks like frontend ignores results */
...
Sane.open(Sane.String_Const name, Sane.Handle * handle)
{
  ...
  *handle = dev
  ...
  dev.opt[OPT_BRIGHTNESS].cap = Sane.CAP_INACTIVE
  dev.opt[OPT_THRESHOLD].cap = Sane.CAP_SOFT_DETECT
...
Sane.get_option_descriptor(Sane.Handle handle, Int option)
{
  ...
  DBG(10, "Sane.get_option_descriptor: \"%s\"\n",
                     ...
                     Int * info)
{
  ...
  if(scanner.scanning == Sane.TRUE)
    {
      DBG(5, "Sane.control_option: device busy\n")
  ...
  if(action == Sane.ACTION_GET_VALUE)
    {
      DBG(10, "Sane.control_option: get value \"%s\"\n",
           scanner.opt[option].name)
      DBG(11, "\tcap = %d\n", cap)
      ...
      switch(option)
        ...
        case OPT_X_RES:
          *(Sane.Word *) val = scanner.x_res
          return Sane.STATUS_GOOD

        case OPT_Y_RES:
          *(Sane.Word *) val = scanner.y_res
          return Sane.STATUS_GOOD
        ...
        case OPT_TL_X:
          *(Sane.Word *) val = Sane.FIX(iluToMm(scanner.tl_x))
          return Sane.STATUS_GOOD

        case OPT_TL_Y:
          *(Sane.Word *) val = Sane.FIX(iluToMm(scanner.tl_y))
          return Sane.STATUS_GOOD

        case OPT_BR_X:
          *(Sane.Word *) val = Sane.FIX(iluToMm(scanner.br_x))
          return Sane.STATUS_GOOD

        case OPT_BR_Y:
          *(Sane.Word *) val = Sane.FIX(iluToMm(scanner.br_y))
          return Sane.STATUS_GOOD
        ...
        case OPT_THRESHOLD:
          *(Sane.Word *) val = scanner.threshold
          return Sane.STATUS_GOOD
      ...
      switch(option)
        ...
        case OPT_SOURCE:
          if(strcmp(val, "ADF") == 0)
            ...
            {
              if(scanner.use_adf == Sane.FALSE)
                return Sane.STATUS_GOOD
              scanner.use_adf = Sane.FALSE
          ...
          if(strcmp(val, lineStr) == 0)
            {
              ...
              scanner.opt[OPT_THRESHOLD].cap = Sane.CAP_SOFT_DETECT
                | Sane.CAP_SOFT_SELECT
          ...
          else if(strcmp(val, halfStr) == 0)
            {
              ...
              scanner.bitsperpixel = 1
              scanner.opt[OPT_BRIGHTNESS].cap = Sane.CAP_SOFT_DETECT
                | Sane.CAP_SOFT_SELECT
        ...
        case OPT_TL_X:
          scanner.tl_x = mmToIlu(Sane.UNFIX(*(Sane.Word *) val))
          adjust_width(scanner, info)
          *(Sane.Word *) val = Sane.FIX(iluToMm(scanner.tl_x))
          if(info)
            {
              *info |= Sane.INFO_RELOAD_PARAMS | Sane.INFO_INEXACT
            }
          return Sane.STATUS_GOOD

        case OPT_TL_Y:
          scanner.tl_y = mmToIlu(Sane.UNFIX(*(Sane.Word *) val))
          *(Sane.Word *) val = Sane.FIX(iluToMm(scanner.tl_y))
          if(info)
            {
              *info |= Sane.INFO_RELOAD_PARAMS | Sane.INFO_INEXACT
        ...
        case OPT_BR_X:
          scanner.br_x = mmToIlu(Sane.UNFIX(*(Sane.Word *) val))
          adjust_width(scanner, info)
          *(Sane.Word *) val = Sane.FIX(iluToMm(scanner.br_x))
          if(info)
            {
        ...
        case OPT_BR_Y:
          scanner.br_y = mmToIlu(Sane.UNFIX(*(Sane.Word *) val))
          *(Sane.Word *) val = Sane.FIX(iluToMm(scanner.br_y))
          if(info)
            {
              *info |= Sane.INFO_RELOAD_PARAMS | Sane.INFO_INEXACT
        ...
        case OPT_THRESHOLD:
          scanner.threshold = *(Sane.Word *) val
          return Sane.STATUS_GOOD
...
Sane.start(Sane.Handle handle)
{
  ...
  if((ret = sp15c_check_values(scanner)) != 0)
    {                           /* Verify values */
      DBG(1, "Sane.start: ERROR: invalid scan-values\n")
  ...
  if((ret = sp15c_grab_scanner(scanner)))
    {
      DBG(5, "Sane.start: unable to reserve scanner\n")
      sanei_scsi_close(scanner.sfd)
      scanner.scanning = Sane.FALSE
      scanner.sfd = -1
      return ret
  ...
  if(scanner.use_adf == Sane.TRUE
    ...
    {
      DBG(5, "Sane.start: WARNING: ADF empty\n")
      sp15c_free_scanner(scanner)
      sanei_scsi_close(scanner.sfd)
      scanner.scanning = Sane.FALSE
      scanner.sfd = -1
  ...
  if(scanner.composition == WD_comp_MC
    ...
    {
      DBG(5, "Sane.start: start_scan failed\n")
      sp15c_free_scanner(scanner)
      sanei_scsi_close(scanner.sfd)
      scanner.scanning = Sane.FALSE
      scanner.sfd = -1
           ...
           Int max_len, Int * len)
{
  ...
  *len = 0
  ...
  DBG(10, "Sane.read: read %ld bytes of %ld\n",
       (long) nread, (long) max_len)

  if(scanner.scanning == Sane.FALSE)
    {
...
{
  DBG(10, "Sane.close\n")
  if(((struct sp15c *) handle)->scanning == Sane.TRUE)
    do_cancel(handle)
}                               /* Sane.close */
...
/************** "Mode" group: **************/
  ...
  if(scanner.autofeeder)
    {
      scanner.opt[OPT_SOURCE].cap = Sane.CAP_SOFT_SELECT
  ...
  scanner.opt[OPT_MODE].cap = Sane.CAP_SOFT_SELECT | Sane.CAP_SOFT_DETECT
  ...
  /* resolution */
  scanner.opt[OPT_X_RES].name = Sane.NAME_SCAN_RESOLUTION
  scanner.opt[OPT_X_RES].title = Sane.TITLE_SCAN_X_RESOLUTION
  scanner.opt[OPT_X_RES].desc = Sane.DESC_SCAN_X_RESOLUTION
  scanner.opt[OPT_X_RES].type = Sane.TYPE_INT
  scanner.opt[OPT_X_RES].unit = Sane.UNIT_DPI
  scanner.opt[OPT_X_RES].constraint_type = Sane.CONSTRAINT_WORD_LIST
  scanner.opt[OPT_X_RES].constraint.word_list = x_res_list
  scanner.opt[OPT_X_RES].cap = Sane.CAP_SOFT_SELECT | Sane.CAP_SOFT_DETECT

  scanner.opt[OPT_Y_RES].name = Sane.NAME_SCAN_Y_RESOLUTION
  scanner.opt[OPT_Y_RES].title = Sane.TITLE_SCAN_Y_RESOLUTION
  scanner.opt[OPT_Y_RES].desc = Sane.DESC_SCAN_Y_RESOLUTION
  scanner.opt[OPT_Y_RES].type = Sane.TYPE_INT
  scanner.opt[OPT_Y_RES].unit = Sane.UNIT_DPI
  scanner.opt[OPT_Y_RES].constraint_type = Sane.CONSTRAINT_WORD_LIST
  scanner.opt[OPT_Y_RES].constraint.word_list = y_res_list
  scanner.opt[OPT_Y_RES].cap = Sane.CAP_SOFT_SELECT | Sane.CAP_SOFT_DETECT

  scanner.opt[OPT_PREVIEW_RES].name = "preview-resolution"
  scanner.opt[OPT_PREVIEW_RES].title = "Preview resolution"
  scanner.opt[OPT_PREVIEW_RES].desc = Sane.DESC_SCAN_RESOLUTION
  scanner.opt[OPT_PREVIEW_RES].type = Sane.TYPE_INT
  scanner.opt[OPT_PREVIEW_RES].unit = Sane.UNIT_DPI
  scanner.opt[OPT_PREVIEW_RES].constraint_type = Sane.CONSTRAINT_WORD_LIST
...
/************** "Geometry" group: **************/
  ...
  scanner.opt[OPT_TL_X].cap = Sane.CAP_SOFT_SELECT | Sane.CAP_SOFT_DETECT
  ...
  scanner.opt[OPT_TL_Y].constraint.range = &y_range_fb
  scanner.opt[OPT_TL_Y].cap = Sane.CAP_SOFT_SELECT | Sane.CAP_SOFT_DETECT
  ...
  scanner.opt[OPT_BR_X].cap = Sane.CAP_SOFT_SELECT | Sane.CAP_SOFT_DETECT
  ...
  scanner.opt[OPT_BR_Y].constraint.range = &y_range_fb
  scanner.opt[OPT_BR_Y].cap = Sane.CAP_SOFT_SELECT | Sane.CAP_SOFT_DETECT
...
/************** "Enhancement" group: **************/
  ...
  scanner.opt[OPT_THRESHOLD].type = Sane.TYPE_INT
  scanner.opt[OPT_THRESHOLD].unit = Sane.UNIT_NONE
...
{
  if(s.use_adf == Sane.TRUE && s.autofeeder == 0)
    {
...
sp15c_set_window_param(struct sp15c *s, Int prescan)
{
  ...
  set_WD_adf(buffer_r, s.use_adf == Sane.TRUE ? 1 : 0)
...
static Sane.Status
apply_constraints(struct sp15c *scanner, Int opt,
