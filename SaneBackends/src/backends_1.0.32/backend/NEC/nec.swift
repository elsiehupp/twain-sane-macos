...
/*
   ...
   Version 0.11
   - get_data_buffer_status is not called in Sane.get_parameter and
     Sane.read_direct, Sane.read_shuffled.
   - change some import > to ""
   Version 0.10
   - First release!
   - supported scanner
...
static const Sane.String_Const paper_list_pcinxxx[] =
{
  "A3", "A4", "A5", "A6", "B4", "B5",
...
static const Sane.String_Const paper_list_pcin500[] =
{
  "A4", "A5", "A6", "B5",
  0
]
...
static const Sane.String_Const resolution_list_pcinxxx[] =
{
...
static const Sane.String_Const resolution_list_pcin500[] =
{
  ...
  if(dev.sensedat.model == PCIN500)
    {
      ...
      dev.info.tl_x_ranges[SCAN_SIMPLE].max = Sane.FIX(210); /* 304.8mm is the real max */
      dev.info.br_x_ranges[SCAN_SIMPLE].max = Sane.FIX(210); /* 304.8mm is the real max */
...
*/
static Sane.Status
init_string_option(NEC_Scanner *s, Sane.String_Const name,
...
init_options(NEC_Scanner * s)
{
  ...
  init_string_option(s, Sane.NAME_SCAN_MODE, Sane.TITLE_SCAN_MODE,
    Sane.DESC_SCAN_MODE, mode_list, OPT_MODE, MODES_COLOR)
  ...
  s.opt[OPT_OR].type = Sane.TYPE_BOOL
  s.val[OPT_OR].w = Sane.FALSE

  /* EDGE */
  s.opt[OPT_EDGE].name = "edge"
  s.opt[OPT_EDGE].title = "Edge"
  s.opt[OPT_EDGE].desc = "Select Edge emphancement"
  s.opt[OPT_EDGE].type = Sane.TYPE_BOOL
  s.val[OPT_EDGE].w = Sane.FALSE
  ...
  if(s.dev.sensedat.model != PCIN500)
    {
      ...
      s.opt[OPT_OR].cap |= Sane.CAP_INACTIVE
    }
  /* tint */
  s.opt[OPT_TINT].name = "tint"
  s.opt[OPT_TINT].title = "Tint"
  s.opt[OPT_TINT].desc = "Select tint"
  s.opt[OPT_TINT].type = Sane.TYPE_INT
  s.opt[OPT_TINT].unit = Sane.UNIT_NONE
  ...
  s.opt[OPT_COLOR].unit = Sane.UNIT_NONE
...
Sane.get_parameters(Sane.Handle handle, Sane.Parameters * params)
{
  ...
  if(strcmp(mode, M_LINEART) == 0)
  ...
  else if(strcmp(mode, M_LINEART_COLOR) == 0)
    {
       ...
       s.params.bytesPerLine = 3 * (s.params.pixels_per_line + 7) / 8
       s.params.depth = 8
...
static Sane.Status
send_binary_g_table(NEC_Scanner *s, Sane.Word *a, Int dtq)
{
  Sane.Status status
