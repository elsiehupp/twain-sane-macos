...
Sane.init (Int * version_code, Sane.Auth_Callback __Sane.unused__ authorize)
{
  ...
  var i: Int, j
  Sane.Byte primary, secondary, inmask, priMask, secMask
...
Sane.close (Sane.Handle handle)
{
  ...
  if (s.val[OPT_EJECT_BEFOREEXIT].w)
	...
	sanei_scsi_open (s.hw.sane.name, &s.fd, sense_handler, s.hw)
      ...
      if (status != Sane.STATUS_GOOD)
	{
	  DBG (1, "Sane.close: MEDIUM POSITION failed\n")
...
Sane.get_option_descriptor (Sane.Handle handle, Int option)
{
  ...
  DBG (21, ">> Sane.get_option_descriptor option number %d\n", option)

  if ((unsigned) option >= NUM_OPTIONS)
    return (0)

  DBG (21, "   Sane.get_option_descriptor option name %s\n",
       option_name[option])
		     ...
		     Sane.Action action, void *val, Int * info)
{
  ...
  Sane.Byte gbuf[4096]
  size_t buf_size
  var i: Int, neg, gamma_component, int_t, transfer_data_type
  time_t dtime, rt

  DBG (21, ">> Sane.control_option %s\n", option_name[option])
  ...
  if (s.scanning == Sane.TRUE)
    {
      DBG (21, ">> Sane.control_option: device is busy scanning\n")
  ...
  if (action == Sane.ACTION_GET_VALUE)
    {
      DBG (21, "Sane.control_option get value of %s\n", option_name[option])
      switch (option)
	{
	  /* word options: */
	...
	case OPT_GAMMA_VECTOR_B:
	  ...
	  if (s.val[OPT_CUSTOM_GAMMA_BIND].w == Sane.TRUE)
    ...
    {
      DBG (21, "Sane.control_option set value for %s\n", option_name[option])
	  ...
	  if (s.val[option].w != *(Sane.Word *) val)
		...
		*info |= Sane.INFO_RELOAD_OPTIONS
		...
		{		/* don't bind */
		  ...
		  s.opt[OPT_HILITE_G].title = Sane.TITLE_HIGHLIGHT_G
		  s.opt[OPT_HILITE_G].name = Sane.NAME_HIGHLIGHT_G
		  s.opt[OPT_HILITE_G].desc = Sane.DESC_HIGHLIGHT_G

		  s.opt[OPT_SHADOW_G].title = Sane.TITLE_SHADOW_G
		  s.opt[OPT_SHADOW_G].name = Sane.NAME_SHADOW_G
		  s.opt[OPT_SHADOW_G].desc = Sane.DESC_SHADOW_G
		}
	      else
		{		/* bind */
		  s.opt[OPT_HILITE_R].cap |= Sane.CAP_INACTIVE
	...
	case OPT_AF:
	  ...
	  if (w)
	    {
	      s.opt[OPT_AF_ONCE].cap &= ~Sane.CAP_INACTIVE
	...
	case OPT_TPU_ON:
	  ...
	  if (s.val[option].w)			/* switch on */
		...
		(s.hw.tpu.ControlMode == 3) ? ~Sane.CAP_INACTIVE : ~0
	      s.opt[OPT_TPU_FILMTYPE].cap &=
		(s.hw.tpu.ControlMode == 1) ? ~Sane.CAP_INACTIVE : ~0
	    }
	  else			/* switch off */
	    {
	      s.hw.tpu.Status = TPU_STAT_INACTIVE
	      s.opt[OPT_TPU_TRANSPARENCY].cap |= Sane.CAP_INACTIVE
	      s.opt[OPT_TPU_FILMTYPE].cap |= Sane.CAP_INACTIVE
	    }
	  s.opt[OPT_TPU_PN].cap ^= Sane.CAP_INACTIVE
	  s.opt[OPT_TPU_DCM].cap ^= Sane.CAP_INACTIVE
          if (info)
	...
	case OPT_MODE:
	  ...
	  || !strcmp (val, Sane.VALUE_SCAN_MODE_HALFTONE))
	    {
		...
		{
		  if (!strcmp (val, Sane.VALUE_SCAN_MODE_COLOR)
		  || !strcmp (val, Sane.I18N("Fine color")))
		    {
		      s.opt[OPT_CUSTOM_GAMMA_BIND].cap &= ~Sane.CAP_INACTIVE
		      if (s.val[OPT_CUSTOM_GAMMA_BIND].w == Sane.TRUE)
			{
	...
	case OPT_NEGATIVE:
	  ...
	  if (s.val[option].s)
	    free (s.val[option].s)
	  s.val[option].s = strdup (val)
	  if (!strcmp (val, Sane.I18N("Negatives")))
	    {
	      s.RIF = 0
	      s.opt[OPT_NEGATIVE_TYPE].cap &= ~Sane.CAP_INACTIVE
	      if (Sane.OPTION_IS_SETTABLE(s.opt[OPT_SCANNING_SPEED].cap))
		s.opt[OPT_SCANNING_SPEED].cap &= ~Sane.CAP_INACTIVE
	    }
	  else
	    {
	      s.RIF = 1
	...
	case OPT_RESET_SCANNER:
	  ...
	  if (status == Sane.STATUS_GOOD)
		...
		{
		  rt = sqrt (15 * 15 * (Sane.UNFIX (s.val[OPT_BR_Y].w))
		    / 297) + 0.5
		  rt = rt + 2
		}
	      else
		rt = 17
	      ...
	      DBG (11, "Sane.UNFIX(s.val[OPT_BR_Y].w) = %f\n",
	  ...
	  if (w)
	    {
	      ...
	      if (!strcmp (mode, Sane.VALUE_SCAN_MODE_GRAY))
		s.opt[OPT_GAMMA_VECTOR].cap &= ~Sane.CAP_INACTIVE
...
Sane.get_parameters (Sane.Handle handle, Sane.Parameters *params)
{
  ...
  if (!s.scanning)
	...
	* s.hw.info.mud / MM_PER_INCH
      length = Sane.UNFIX (s.val[OPT_BR_Y].w - s.val[OPT_TL_Y].w)
	* s.hw.info.mud / MM_PER_INCH
...
/******************************************************************/
  ...
  if ((s.hw.adf.Status != ADF_STAT_NONE)
      && (s.val[OPT_FLATBED_ONLY].w != Sane.TRUE)
      && (s.hw.adf.Problem != 0))
    {
  ...
  if (Sane.OPTION_IS_ACTIVE(s.opt[OPT_FOCUS_GROUP].cap)
    && !s.val[OPT_PREVIEW].w && s.AF_NOW)
    {
      if ((status = do_focus (s)) != Sane.STATUS_GOOD) return (status)
      if (s.val[OPT_AF_ONCE].w) s.AF_NOW = Sane.FALSE
    }
#endif

  if (s.val[OPT_CUSTOM_GAMMA].w)
    {
      if ((status = do_gamma (s)) != Sane.STATUS_GOOD) return (status)
    }
  ...
  s.ulx = Sane.UNFIX (s.val[OPT_TL_X].w) * s.hw.info.mud / MM_PER_INCH
  s.uly = Sane.UNFIX (s.val[OPT_TL_Y].w) * s.hw.info.mud / MM_PER_INCH

  s.width = Sane.UNFIX (s.val[OPT_BR_X].w - s.val[OPT_TL_X].w)
    * s.hw.info.mud / MM_PER_INCH
  ...
  if (s.hw.info.model != CS2700 && s.hw.info.model != FS2710)
    {
      ...
      || !strcmp (mode_str, Sane.VALUE_SCAN_MODE_HALFTONE))
  ...
  else if (!strcmp (mode_str, Sane.VALUE_SCAN_MODE_HALFTONE))
    {
  ...
  else if (!strcmp (mode_str, Sane.I18N("Raw")))
    {
      s.image_composition = 5
    }
  else
  ...
  if (s.hw.info.model == FB620)	/* modification for FB620S */
	...
	&& (strcmp (s.val[OPT_MODE].s, Sane.VALUE_SCAN_MODE_LINEART) != 0))
	? 12 : s.bpp
#endif
      wbuf[36] = 0
      wbuf[37] = (s.RIF << 7) + 0x3
  ...
  if (s.hw.tpu.Status == TPU_STAT_ACTIVE || s.hw.info.is_filmscanner)
    {
      DBG (3, "Sane.start: sending DEFINE SCAN MODE for transparency unit, "
  ...
  DBG (3, "Sane.start: sending DEFINE SCAN MODE for scan control "
    "conditions\n")
  memset (wbuf, 0, sizeof (wbuf))
  wbuf[0] = 0x20
  if (s.hw.info.model == FB1200)
  ...
  DBG (3, "Sane.start: sending GET SCAN MODE for scan control conditions\n")
  memset (ebuf, 0, sizeof (ebuf))
  buf_size = sizeof (ebuf)
  status = get_scan_mode (s.fd, SCAN_CONTROL_CONDITIONS, ebuf, &buf_size)
  if (status != Sane.STATUS_GOOD)
    {
      DBG (1, "Sane.start: GET SCAN MODE for scan control conditions "
	"failed\n")
      sanei_scsi_close (s.fd)
      return (Sane.STATUS_INVAL)
    }
  ...
  if (s.hw.info.can_calibrate)
    {
      DBG (3, "Sane.start: sending GET_CALIBRATION_STATUS\n")
      buf_size = sizeof (cbuf)
      memset (cbuf, 0, buf_size)
      status = get_calibration_status (s.fd, cbuf, &buf_size)
      if (status != Sane.STATUS_GOOD)
	{
	  DBG (1, "Sane.start: GET_CALIBRATION_STATUS failed: %s\n",
	       Sane.strstatus (status))
	  sanei_scsi_close (s.fd)
	  s.fd = -1
	  return (status)
	...
	{
	  status = execute_calibration (s.fd)
	  DBG (3, "Sane.start: EXECUTE_CALIBRATION\n")
	  if (status != Sane.STATUS_GOOD)
	    {
	      DBG (1, "Sane.start: EXECUTE_CALIBRATION failed\n")
	      sanei_scsi_close (s.fd)
	      s.fd = -1
	      return (status)
	    }
...
static Sane.Status
Sane.read_direct (Sane.Handle handle, Sane.Byte *buf, Int max_len,
		  Int *len)
{
  CANON_Scanner *s = handle
  Sane.Status status
  ...
  *len = 0
  ...
  DBG (21, "   Sane.read: nread=%d, bytes_to_read=%d\n", (Int) nread,
  (Int) s.bytes_to_read)
...
static Sane.Status
read_fs2710 (Sane.Handle handle, Sane.Byte *buf, Int max_len,
	     Int *len)
{
  CANON_Scanner *s = handle
  Sane.Status status
  ...
  *len = 0
  ...
  if (!strcmp (s.val[OPT_MODE].s, Sane.VALUE_SCAN_MODE_COLOR))
    {
	...
	{				/* extend buffer? */
	  ...
	  if ((s.auxbuf = (u_char *) malloc (2 * max_len)) == NULL)
	    {
	      DBG (1, "Sane.read buffer size insufficient\n")
	      do_cancel (s)
	      return Sane.STATUS_NO_MEM
	    }
	}
      ...
      nread2 = 2 * nread
      if ((status = read_data (s.fd, s.auxbuf, &nread2)) != Sane.STATUS_GOOD)
	{
	  do_cancel (s)
	  return (Sane.STATUS_IO_ERROR)
	}
    ...
    {
      if ((status = read_data (s.fd, buf, &nread)) != Sane.STATUS_GOOD)
	{
	  do_cancel (s)
	  return (Sane.STATUS_IO_ERROR)
	}
...
static Sane.Status
read_fb620 (Sane.Handle handle, Sane.Byte *buf, Int max_len,
	    Int *len)
{
  CANON_Scanner *s = handle
  Sane.Status status
  Sane.Byte *out, *red, *green, *blue
  Int ncopy
...
static Sane.Status
read_fb1200 (Sane.Handle handle, Sane.Byte *buf, Int max_len,
Int *len)
{
  CANON_Scanner *s = handle
  Sane.Status status
  Sane.Byte *firstimage, *secondimage/*, inmask, outmask, outbyte,
	   ...
	   Int *len)
{
  CANON_Scanner *s = handle
  Sane.Status status
  if (s.hw.info.model == FB620 && s.params.format == Sane.FRAME_RGB)
    ...
    status = Sane.read_direct (handle, buf, max_len, len)
  if (s.time0 == -1)
    s.time0 = 0
  else
    time (&(s.time0))

  DBG (11, "Sane.read: time0 = %ld\n", s.time0)
