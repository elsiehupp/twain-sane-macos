...
init (Int fd)
{
  ...
  if (gl640WriteReq (fd, GL640_GPIO_OE, 0x71) != Sane.STATUS_GOOD) {
      DBG(1, "Initial write request failed.\n")
      return -1
  }
  /* Gets 0x04 or 0x05 first run, gets 0x64 subsequent runs. */
  if (gl640ReadReq (fd, GL640_GPIO_READ, &rv) != Sane.STATUS_GOOD) {
...
static Sane.Status compute_ogn (char *calfilename)
...
CANON_start_scan (CANON_Handle * scanner)
{
  ...
  /* scan */
  if ((status = scan (scanner)) != Sane.STATUS_GOOD)
    {
...
static Sane.Status
CANON_read (CANON_Handle * scanner, Sane.Byte * data,
