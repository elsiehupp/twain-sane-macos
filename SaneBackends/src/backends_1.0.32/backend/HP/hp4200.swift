...
struct _user_parameters_t
{
  ...
  Bool no_reverse
  Sane.Word gamma[3][1024];	/* gamma table for rgb */
]
...
do_fine_calibration(HP4200_Scanner * s, struct coarse_t *coarse)
{
  ...
  unsigned char *cal_line_ptr
  Int *average
  Sane.Byte red_gain_offset[5460 * 2]
  Sane.Byte green_gain_offset[5460 * 2]
  Sane.Byte blue_gain_offset[5460 * 2]
  Int *corr_red = NULL
  Int *corr_green = NULL
  Int *corr_blue = NULL
...
init_options(HP4200_Scanner * s)
{
  ...
  s.opt[OPT_RES].name = Sane.NAME_SCAN_RESOLUTION
  s.opt[OPT_RES].title = Sane.TITLE_SCAN_RESOLUTION
