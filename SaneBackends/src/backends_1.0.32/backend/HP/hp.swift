 ...
 * return Sane.Status types.
 */
#define FAILED(status)		(status != Sane.STATUS_GOOD)
...
/* hp-option.c */
typedef Sane.Option_Descriptor *	HpSaneOption
...
/* Changes:
   ...
   * backend/snapscan.c: remove EXPECTED_VERSION check
   ...
   V 0.87, 28-Jun-2000, PK:
      ...
      - Return error Sane.STATUS_NO_DOCS if no paper in ADF
...
Sane.get_option_descriptor(Sane.Handle handle, Int optnum)
{
  ...
  DBG(10, "Sane.get_option_descriptor called\n")

  optd = sanei_hp_handle_saneoption(h, optnum)

  DBG(10, "Sane.get_option_descriptor will finish\n")
		     ...
		     Sane.Action action, void *valp, Int *info)
{
  ...
  DBG(10, "Sane.control_option called\n")
