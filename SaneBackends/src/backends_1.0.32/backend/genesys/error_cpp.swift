...
SaneException::SaneException(Sane.Status status) : status_(status)
{
    set_msg()
}

SaneException::SaneException(Sane.Status status, const char* format, ...) : status_(status)
{
    std::va_list args
...
SaneException::SaneException(const char* format, ...) : status_(Sane.STATUS_INVAL)
{
    std::va_list args
    va_start(args, format)
    set_msg(format, args)
...
Sane.Status SaneException::status() const
{
    return status_
}
...
{
    const char* status_msg = Sane.strstatus(status_)
