...
/* #define Sane.UMAX_DEBUG_S12 */
/* #define UMAX_CALIBRATION_MODE_SELECTABLE */

/* --------------------------------------------------------------------------------------------------------- */

#define Sane.UMAX_SCSI_MAXQUEUE 		8

/* --------------------------------------------------------------------------------------------------------- */

#define Sane.UMAX_FIX_ROUND(val) ((Sane.Word) ((val) * (1 << Sane.FIXED_SCALE_SHIFT) + 1.0 / (1 << (Sane.FIXED_SCALE_SHIFT+1))))
...
typedef struct Umax_Device
{
  ...
  Int connection_type
#define Sane.UMAX_UNKNOWN 0
#define Sane.UMAX_SCSI 1
#define Sane.UMAX_USB 2

  Sane.Range		x_dpi_range
  Sane.Range		y_dpi_range
  Sane.Range		x_range
  Sane.Range		y_range
  Sane.Range		analog_gamma_range
  unsigned		flags

  unsigned char		*buffer[Sane.UMAX_SCSI_MAXQUEUE];		    /* buffer used for scsi-transfer */
  void                  *queue_id[Sane.UMAX_SCSI_MAXQUEUE];				    /* scsi queue id */
  size_t		length_queued[Sane.UMAX_SCSI_MAXQUEUE];			    /* length of queued data */
  size_t		length_read[Sane.UMAX_SCSI_MAXQUEUE];			  /* length of returned data */
...
typedef struct Umax_Scanner
{
  ...
  Sane.Range			output_range
  unsigned Int			output_bytes
  Sane.Range			exposure_time_range
