...
#define Sane.NAME_BATCH_SCAN_START	"batch-scan-start"
#define Sane.TITLE_BATCH_SCAN_START	"Batch scan start"
#define Sane.DESC_BATCH_SCAN_START	"set for first scan of batch"

#define Sane.NAME_BATCH_SCAN_LOOP	"batch-scan-loop"
#define Sane.TITLE_BATCH_SCAN_LOOP	"Batch scan loop"
#define Sane.DESC_BATCH_SCAN_LOOP	"set for middle scans of batch"

#define Sane.NAME_BATCH_SCAN_END	"batch-scan-end"
#define Sane.TITLE_BATCH_SCAN_END	"Batch scan end"
#define Sane.DESC_BATCH_SCAN_END	"set for last scan of batch"

#define Sane.NAME_BATCH_NEXT_TL_Y	"batch-scan-next-tl-y"
#define Sane.TITLE_BATCH_NEXT_TL_Y	"Batch scan next top left Y"
#define Sane.DESC_BATCH_NEXT_TL_Y	"Set top left Y position for next scan"

#define Sane.UMAX_NAME_SELECT_CALIBRATON_EXPOSURE_TIME		"select-calibration-exposure-time"
#define Sane.UMAX_TITLE_SELECT_CALIBRATION_EXPOSURE_TIME	"Set calibration exposure time"
#define Sane.UMAX_DESC_SELECT_CALIBRATION_EXPOSURE_TIME		"Allow different settings for calibration and scan exposure times"

/* ------------------------------------------------------------ STRING DEFINITIONS ------------------------- */

#define FLB_STR			Sane.I18N("Flatbed")
#define UTA_STR			Sane.I18N("Transparency Adapter")
...
#define COLOR_LINEART_STR	Sane.VALUE_SCAN_MODE_COLOR_LINEART
#define COLOR_HALFTONE_STR	Sane.VALUE_SCAN_MODE_COLOR_HALFTONE
#define COLOR_STR		Sane.VALUE_SCAN_MODE_COLOR

/* ------------------------------------------------------------ DEFINITIONS -------------------------------- */

#define P_200_TO_255(per) ( (Sane.UNFIX(per) + 100) * 255/200 )
#define P_100_TO_255(per) Sane.UNFIX(per * 255/100 )
#define P_100_TO_254(per) 1+Sane.UNFIX(per * 254/100 )

/* ------------------------------------------------------------ GLOBAL VARIABLES --------------------------- */


static String scan_mode_list[7]
static Sane.String_Const source_list[4]
static Int bit_depth_list[9]
static Sane.Auth_Callback frontend_authorize_callback
...
#define CALIB_MODE_0000		Sane.I18N("Use Image Composition")
#define CALIB_MODE_1111		Sane.I18N("Bi-level black and white (lineart mode)")
#define CALIB_MODE_1110		Sane.I18N("Dithered/halftone black & white (halftone mode)")
#define CALIB_MODE_1101		Sane.I18N("Multi-level black & white (grayscale mode)")
#define CALIB_MODE_1010		Sane.I18N("Multi-level RGB color (one pass color)")
#define CALIB_MODE_1001		Sane.I18N("Ignore calibration")

static Sane.String_Const calibration_list[] =
{
    CALIB_MODE_0000,
    CALIB_MODE_1111,
    CALIB_MODE_1110,
...
static const Int pattern_dim_list[] =
{
  5, /* # of elements */
  2, 4, 6, 8, 12
]
...
static Bool umax_test_little_endian(void)
{
  Int testvalue = 255
...
static Sane.Status sense_handler(Int scsi_fd, unsigned char *result, void *arg)	  /* is called by sanei_scsi */
{
...
{
  if ( (scanner.val[OPT_RGB_BIND].w == Sane.FALSE) &&
       (strcmp(scanner.val[OPT_MODE].s, COLOR_STR) == 0) ) /* enable rgb options */
  {
    if (scanner.device.inquiry_analog_gamma)
    {
      ...
      scanner.opt[OPT_ANALOG_GAMMA_R].cap &= ~Sane.CAP_INACTIVE
      scanner.opt[OPT_ANALOG_GAMMA_G].cap &= ~Sane.CAP_INACTIVE
      scanner.opt[OPT_ANALOG_GAMMA_B].cap &= ~Sane.CAP_INACTIVE
    }
    if (scanner.device.inquiry_highlight)
    {
      scanner.opt[OPT_HIGHLIGHT].cap   |= Sane.CAP_INACTIVE
      scanner.opt[OPT_HIGHLIGHT_R].cap &= ~Sane.CAP_INACTIVE
      scanner.opt[OPT_HIGHLIGHT_G].cap &= ~Sane.CAP_INACTIVE
      scanner.opt[OPT_HIGHLIGHT_B].cap &= ~Sane.CAP_INACTIVE
    }
    if (scanner.device.inquiry_shadow)
    {
      scanner.opt[OPT_SHADOW].cap   |= Sane.CAP_INACTIVE
      scanner.opt[OPT_SHADOW_R].cap &= ~Sane.CAP_INACTIVE
      scanner.opt[OPT_SHADOW_G].cap &= ~Sane.CAP_INACTIVE
      scanner.opt[OPT_SHADOW_B].cap &= ~Sane.CAP_INACTIVE
    }
  }
  ...
  if ( (scanner.device.inquiry_exposure_adj) && (scanner.val[OPT_SELECT_EXPOSURE_TIME].w) )
  {
         ...
         (strcmp(scanner.val[OPT_MODE].s, COLOR_STR) == 0) ) /* enable rgb exposure time options */
    {
      ...
      {
        scanner.opt[OPT_CAL_EXPOS_TIME].cap    |= Sane.CAP_INACTIVE
        scanner.opt[OPT_CAL_EXPOS_TIME_R].cap  &= ~Sane.CAP_INACTIVE
        scanner.opt[OPT_CAL_EXPOS_TIME_G].cap  &= ~Sane.CAP_INACTIVE
        scanner.opt[OPT_CAL_EXPOS_TIME_B].cap  &= ~Sane.CAP_INACTIVE
      }
      ...
      scanner.opt[OPT_SCAN_EXPOS_TIME].cap   |= Sane.CAP_INACTIVE
      scanner.opt[OPT_SCAN_EXPOS_TIME_R].cap &= ~Sane.CAP_INACTIVE
      scanner.opt[OPT_SCAN_EXPOS_TIME_G].cap &= ~Sane.CAP_INACTIVE
      scanner.opt[OPT_SCAN_EXPOS_TIME_B].cap &= ~Sane.CAP_INACTIVE
    }
...
static Sane.Status umax_scsi_cmd(Umax_Device *dev, const void *src, size_t src_size, void *dst, size_t * dst_size)
{
...
static Sane.Status umax_scsi_open_extended(const char *devicename, Umax_Device *dev,
...
static Sane.Status umax_scsi_open(const char *devicename, Umax_Device *dev,
...
static Sane.Status umax_scsi_req_enter(Umax_Device *dev, const void *src, size_t src_size,
...
static Sane.Status umax_scsi_req_wait(Umax_Device *dev, void *id)
{
  switch (dev.connection_type)
  {
    case Sane.UMAX_SCSI:
...
static Sane.Status umax_get_data_buffer_status(Umax_Device *dev)
{
 Sane.Status status
...
static Sane.Status umax_wait_scanner(Umax_Device *dev)
{
 Sane.Status status
 Int cnt = 0
...
static Sane.Status umax_queue_read_image_data_req(Umax_Device *dev, unsigned Int length, Int bufnr)
{
 Sane.Status status
...
static Sane.Status umax_set_window_param(Umax_Device *dev)
{
 Sane.Status status
...
static Sane.Status umax_start_scan(Umax_Device *dev)
{
 Int size = 1
 Sane.Status status
...
static Sane.Status umax_do_calibration(Umax_Device *dev)
{
 Sane.Status status
 ...
 unsigned Int bytespp = 0
  ...
  if ((status == Sane.STATUS_GOOD) && (dev.do_calibration != 0))			    /* calibration by driver */
  {
...
static void umax_correct_inquiry(Umax_Device *dev, char *vendor, char *product, char *version)
{
  ...
  if (!strncmp(vendor, "UMAX ", 5))
  {
    ...
    else if (!strncmp(product, "Astra 2200 ", 11))
    {
      ...
      if (dev.connection_type == Sane.UMAX_USB)
      {
...
static void umax_init(Umax_Device *dev)		     /* umax_init is called once while driver-initialization */
{
  ...
  if (dev.connection_type == Sane.UMAX_SCSI)
  {
    dev.request_scsi_maxqueue  = umax_scsi_maxqueue
  }
  else /* Sane.UMAX_USB, USB does not support command queueing */
  {
  ...
  if (umax_test_little_endian() == Sane.TRUE)
  {
...
static size_t max_string_size(Sane.String_Const strings[])
{
...
static Sane.Status do_cancel(Umax_Scanner *scanner)
{
  ...
  if (sanei_thread_is_valid (scanner.reader_pid))
  {
    ...
    {
      DBG(DBG_Sane.info, "do_cancel: sanei_thread_waitpid failed, already terminated ? (%s)\n", strerror(errno))
    }
    else
    {
      DBG(DBG_Sane.info, "do_cancel: reader_process terminated with status: %s\n", Sane.strstatus(status))
    }
...
static Sane.Status attach_scanner(const char *devicename, Umax_Device **devp, Int connection_type)
{
 Umax_Device *dev
 var i: Int

  DBG(DBG_Sane.proc,"attach_scanner: %s, connection_type %d\n", devicename, connection_type)
  ...
  if (dev.connection_type != Sane.UMAX_SCSI)
  {
    ...
    if (sanei_umaxusb_open(devicename, &dev.sfd, sense_handler, dev) == Sane.STATUS_GOOD)
    {
      dev.connection_type = Sane.UMAX_USB
    }
    else /* opening usb device failed */
    {
      if (dev.connection_type == Sane.UMAX_USB) /* we know it is not a scsi device: error */
      {
...
#else
  dev.connection_type = Sane.UMAX_SCSI
#endif

  if (dev.connection_type != Sane.UMAX_USB) /* not an USB device, then try as SCSI */
  {
#ifdef HAVE_SANEI_SCSI_OPEN_EXTENDED
    dev.bufsize = 16384; /* 16KB */
...
#endif
    dev.connection_type = Sane.UMAX_SCSI; /* set connection type (may have been unknown == 0) */
  }

  DBG(DBG_info, "attach_scanner: allocating SCSI buffer[0]\n")
  dev.buffer[0] = malloc(dev.bufsize);								/* allocate buffer */

  for (i=1; i<Sane.UMAX_SCSI_MAXQUEUE; i++)
  {
    dev.buffer[i] = NULL
  }
  ...
  dev.x_dpi_range.min           = Sane.FIX(dev.inquiry_optical_res/100)
  dev.x_dpi_range.quant         = Sane.FIX(dev.inquiry_optical_res/100)
#else
  dev.x_dpi_range.min           = Sane.FIX(5)
  dev.x_dpi_range.quant         = Sane.FIX(5)
#endif
  dev.x_dpi_range.max           = Sane.FIX(dev.inquiry_x_res)

#if UMAX_RESOLUTION_PERCENT_STEP
  dev.y_dpi_range.min           = Sane.FIX(dev.inquiry_optical_res/100)
  dev.y_dpi_range.quant         = Sane.FIX(dev.inquiry_optical_res/100)
#else
  dev.y_dpi_range.min           = Sane.FIX(5)
  dev.y_dpi_range.quant         = Sane.FIX(5)
#endif
  dev.y_dpi_range.max           = Sane.FIX(dev.inquiry_y_res)

  dev.analog_gamma_range.min    = Sane.FIX(1.0)
  dev.analog_gamma_range.quant  = Sane.FIX(0.01)
  dev.analog_gamma_range.max    = Sane.FIX(2.0)

  DBG(DBG_info,"x_range.max     = %f\n", Sane.UNFIX(dev.x_range.max))
  DBG(DBG_info,"y_range.max     = %f\n", Sane.UNFIX(dev.y_range.max))
  DBG(DBG_info,"x_dpi_range.max = %f\n", Sane.UNFIX(dev.x_dpi_range.max))
  DBG(DBG_info,"y_dpi_range.max = %f\n", Sane.UNFIX(dev.y_dpi_range.max))
...
static Int reader_process(void *data) /* executed as a child process or as thread */
{
 ...
 unsigned var i: Int
  ...
  {
    DBG(DBG_Sane.proc,"reader_process started (forked)\n")
  ...
  {
    DBG(DBG_Sane.proc,"reader_process started (as thread)\n")
  }
...
static Sane.Status init_options(Umax_Scanner *scanner)
{
 var i: Int
 Int scan_modes
 Int bit_depths
  ...
  scanner.opt[OPT_NUM_OPTS].name  = Sane.NAME_NUM_OPTIONS; /* empty string */
  ...
  /* "Mode" group: */
  scanner.opt[OPT_MODE_GROUP].title = Sane.I18N("Scan Mode")
...
*/
  ...
  scanner.opt[OPT_MODE].size  = max_string_size((Sane.String_Const *) scan_mode_list)
  scanner.opt[OPT_MODE].constraint_type = Sane.CONSTRAINT_STRING_LIST
  scanner.opt[OPT_MODE].constraint.string_list = (Sane.String_Const *) scan_mode_list
  scanner.val[OPT_MODE].s     = (Sane.Char*)strdup(scan_mode_list[0])

  /* source */
  scanner.opt[OPT_SOURCE].name  = Sane.NAME_SCAN_SOURCE
  ...
  scanner.val[OPT_SOURCE].s     = (Sane.Char*)strdup(source_list[0])

  /* x-resolution */
  scanner.opt[OPT_X_RESOLUTION].name  = Sane.NAME_SCAN_RESOLUTION
  scanner.opt[OPT_X_RESOLUTION].title = Sane.TITLE_SCAN_RESOLUTION
  scanner.opt[OPT_X_RESOLUTION].desc  = Sane.DESC_SCAN_RESOLUTION
  scanner.opt[OPT_X_RESOLUTION].type  = Sane.TYPE_FIXED
  scanner.opt[OPT_X_RESOLUTION].unit  = Sane.UNIT_DPI
  scanner.opt[OPT_X_RESOLUTION].constraint_type  = Sane.CONSTRAINT_RANGE
  scanner.opt[OPT_X_RESOLUTION].constraint.range = &scanner.device.x_dpi_range
  scanner.val[OPT_X_RESOLUTION].w     = 100 << Sane.FIXED_SCALE_SHIFT

  /* y-resolution */
  scanner.opt[OPT_Y_RESOLUTION].name  = Sane.NAME_SCAN_Y_RESOLUTION
  scanner.opt[OPT_Y_RESOLUTION].title = Sane.TITLE_SCAN_Y_RESOLUTION
  scanner.opt[OPT_Y_RESOLUTION].desc  = Sane.DESC_SCAN_Y_RESOLUTION
  scanner.opt[OPT_Y_RESOLUTION].type  = Sane.TYPE_FIXED
  scanner.opt[OPT_Y_RESOLUTION].unit  = Sane.UNIT_DPI
  scanner.opt[OPT_Y_RESOLUTION].constraint_type  = Sane.CONSTRAINT_RANGE
  scanner.opt[OPT_Y_RESOLUTION].constraint.range = &scanner.device.y_dpi_range
  scanner.val[OPT_Y_RESOLUTION].w     = 100 << Sane.FIXED_SCALE_SHIFT
  scanner.opt[OPT_Y_RESOLUTION].cap  |= Sane.CAP_INACTIVE

  /* bind resolution */
  scanner.opt[OPT_RESOLUTION_BIND].name  = Sane.NAME_RESOLUTION_BIND
  scanner.opt[OPT_RESOLUTION_BIND].title = Sane.TITLE_RESOLUTION_BIND
  scanner.opt[OPT_RESOLUTION_BIND].desc  = Sane.DESC_RESOLUTION_BIND
  scanner.opt[OPT_RESOLUTION_BIND].type  = Sane.TYPE_BOOL
  scanner.val[OPT_RESOLUTION_BIND].w     = Sane.TRUE
  if (scanner.device.common_xy_resolutions)	/* disable bind if x and y res have to be the same */
  {
    scanner.opt[OPT_RESOLUTION_BIND].cap  |= Sane.CAP_INACTIVE
  }


  /* negative */
  scanner.opt[OPT_NEGATIVE].name  = Sane.NAME_NEGATIVE
  scanner.opt[OPT_NEGATIVE].title = Sane.TITLE_NEGATIVE
  scanner.opt[OPT_NEGATIVE].desc  = Sane.DESC_NEGATIVE
  scanner.opt[OPT_NEGATIVE].type  = Sane.TYPE_BOOL
  scanner.val[OPT_NEGATIVE].w     = Sane.FALSE

  if (scanner.device.inquiry_reverse_multi == 0)
  {
    scanner.opt[OPT_NEGATIVE].cap  |= Sane.CAP_INACTIVE
  }

  /* ------------------------------ */

  /* "Geometry" group: */
  scanner.opt[OPT_GEOMETRY_GROUP].title = Sane.I18N("Geometry")
  ...
  /* "Enhancement" group: */
  scanner.opt[OPT_ENHANCEMENT_GROUP].title = Sane.I18N("Enhancement")
  ...
  scanner.opt[OPT_BIT_DEPTH].name  = Sane.NAME_BIT_DEPTH
  scanner.opt[OPT_BIT_DEPTH].title = Sane.TITLE_BIT_DEPTH
  scanner.opt[OPT_BIT_DEPTH].desc  = Sane.DESC_BIT_DEPTH
  scanner.opt[OPT_BIT_DEPTH].type  = Sane.TYPE_INT
  scanner.opt[OPT_BIT_DEPTH].unit  = Sane.UNIT_BIT
  scanner.opt[OPT_BIT_DEPTH].constraint_type = Sane.CONSTRAINT_WORD_LIST
  scanner.opt[OPT_BIT_DEPTH].constraint.word_list = bit_depth_list
  scanner.val[OPT_BIT_DEPTH].w     = bit_depth_list[1]


  /* quality-calibration */
  scanner.opt[OPT_QUALITY].name  = Sane.NAME_QUALITY_CAL
  scanner.opt[OPT_QUALITY].title = Sane.TITLE_QUALITY_CAL
  scanner.opt[OPT_QUALITY].desc  = Sane.DESC_QUALITY_CAL
  scanner.opt[OPT_QUALITY].type  = Sane.TYPE_BOOL
  scanner.val[OPT_QUALITY].w     = Sane.FALSE

  if ((scanner.device.inquiry_quality_ctrl == 0) || (scanner.device.force_quality_calibration) )
  {
    scanner.opt[OPT_QUALITY].cap  |= Sane.CAP_INACTIVE
  }
  else /* enable quality calibration when available */
  {
    scanner.val[OPT_QUALITY].w = Sane.TRUE
  }


  /* double optical resolution */
  scanner.opt[OPT_DOR].name  = Sane.NAME_DOR
  scanner.opt[OPT_DOR].title = Sane.TITLE_DOR
  scanner.opt[OPT_DOR].desc  = Sane.DESC_DOR
  scanner.opt[OPT_DOR].type  = Sane.TYPE_BOOL
  scanner.val[OPT_DOR].w     = Sane.FALSE

  if (scanner.device.inquiry_dor == 0)
  {
    scanner.opt[OPT_DOR].cap  |= Sane.CAP_INACTIVE
  }


  /* warmup */
  scanner.opt[OPT_WARMUP].name  = Sane.NAME_WARMUP
  scanner.opt[OPT_WARMUP].title = Sane.TITLE_WARMUP
  scanner.opt[OPT_WARMUP].desc  = Sane.DESC_WARMUP
  scanner.opt[OPT_WARMUP].type  = Sane.TYPE_BOOL
  scanner.val[OPT_WARMUP].w     = Sane.FALSE

  if (scanner.device.inquiry_max_warmup_time == 0)
  {
    scanner.opt[OPT_WARMUP].cap  |= Sane.CAP_INACTIVE
  }

  scanner.opt[OPT_RGB_BIND].name  = Sane.NAME_RGB_BIND
  scanner.opt[OPT_RGB_BIND].title = Sane.TITLE_RGB_BIND
  scanner.opt[OPT_RGB_BIND].desc  = Sane.DESC_RGB_BIND
  scanner.opt[OPT_RGB_BIND].type  = Sane.TYPE_BOOL
  scanner.val[OPT_RGB_BIND].w     = Sane.FALSE

  /* brightness */
  scanner.opt[OPT_BRIGHTNESS].name  = Sane.NAME_BRIGHTNESS
  scanner.opt[OPT_BRIGHTNESS].title = Sane.TITLE_BRIGHTNESS
  scanner.opt[OPT_BRIGHTNESS].desc  = Sane.DESC_BRIGHTNESS
  scanner.opt[OPT_BRIGHTNESS].type  = Sane.TYPE_FIXED
  scanner.opt[OPT_BRIGHTNESS].unit  = Sane.UNIT_PERCENT
  scanner.opt[OPT_BRIGHTNESS].constraint_type = Sane.CONSTRAINT_RANGE
  scanner.opt[OPT_BRIGHTNESS].constraint.range = &percentage_range
  scanner.val[OPT_BRIGHTNESS].w     = 0

  /* contrast */
  scanner.opt[OPT_CONTRAST].name  = Sane.NAME_CONTRAST
  scanner.opt[OPT_CONTRAST].title = Sane.TITLE_CONTRAST
  scanner.opt[OPT_CONTRAST].desc  = Sane.DESC_CONTRAST
  scanner.opt[OPT_CONTRAST].type  = Sane.TYPE_FIXED
  scanner.opt[OPT_CONTRAST].unit  = Sane.UNIT_PERCENT
  scanner.opt[OPT_CONTRAST].constraint_type = Sane.CONSTRAINT_RANGE
  ...
  scanner.opt[OPT_HIGHLIGHT].name  = Sane.NAME_HIGHLIGHT
  scanner.opt[OPT_HIGHLIGHT].title = Sane.TITLE_HIGHLIGHT
  scanner.opt[OPT_HIGHLIGHT].desc  = Sane.DESC_HIGHLIGHT
  scanner.opt[OPT_HIGHLIGHT].type  = Sane.TYPE_FIXED
  scanner.opt[OPT_HIGHLIGHT].unit  = Sane.UNIT_PERCENT
  scanner.opt[OPT_HIGHLIGHT].constraint_type = Sane.CONSTRAINT_RANGE
  scanner.opt[OPT_HIGHLIGHT].constraint.range = &percentage_range_100
  scanner.val[OPT_HIGHLIGHT].w     = Sane.FIX(100)

  scanner.opt[OPT_HIGHLIGHT_R].name  = Sane.NAME_HIGHLIGHT_R
  scanner.opt[OPT_HIGHLIGHT_R].title = Sane.TITLE_HIGHLIGHT_R
  scanner.opt[OPT_HIGHLIGHT_R].desc  = Sane.DESC_HIGHLIGHT_R
  scanner.opt[OPT_HIGHLIGHT_R].type  = Sane.TYPE_FIXED
  scanner.opt[OPT_HIGHLIGHT_R].unit  = Sane.UNIT_PERCENT
  scanner.opt[OPT_HIGHLIGHT_R].constraint_type = Sane.CONSTRAINT_RANGE
  scanner.opt[OPT_HIGHLIGHT_R].constraint.range = &percentage_range_100
  scanner.val[OPT_HIGHLIGHT_R].w     = Sane.FIX(100)

  scanner.opt[OPT_HIGHLIGHT_G].name  = Sane.NAME_HIGHLIGHT_G
  scanner.opt[OPT_HIGHLIGHT_G].title = Sane.TITLE_HIGHLIGHT_G
  scanner.opt[OPT_HIGHLIGHT_G].desc  = Sane.DESC_HIGHLIGHT_G
  scanner.opt[OPT_HIGHLIGHT_G].type  = Sane.TYPE_FIXED
  scanner.opt[OPT_HIGHLIGHT_G].unit  = Sane.UNIT_PERCENT
  scanner.opt[OPT_HIGHLIGHT_G].constraint_type = Sane.CONSTRAINT_RANGE
  scanner.opt[OPT_HIGHLIGHT_G].constraint.range = &percentage_range_100
  scanner.val[OPT_HIGHLIGHT_G].w     = Sane.FIX(100)

  scanner.opt[OPT_HIGHLIGHT_B].name  = Sane.NAME_HIGHLIGHT_B
  scanner.opt[OPT_HIGHLIGHT_B].title = Sane.TITLE_HIGHLIGHT_B
  scanner.opt[OPT_HIGHLIGHT_B].desc  = Sane.DESC_HIGHLIGHT_B
  scanner.opt[OPT_HIGHLIGHT_B].type  = Sane.TYPE_FIXED
  scanner.opt[OPT_HIGHLIGHT_B].unit  = Sane.UNIT_PERCENT
  scanner.opt[OPT_HIGHLIGHT_B].constraint_type = Sane.CONSTRAINT_RANGE
  scanner.opt[OPT_HIGHLIGHT_B].constraint.range = &percentage_range_100
  scanner.val[OPT_HIGHLIGHT_B].w     = Sane.FIX(100)


  /* shadow, black level */
  scanner.opt[OPT_SHADOW].name  = Sane.NAME_SHADOW
  scanner.opt[OPT_SHADOW].title = Sane.TITLE_SHADOW
  scanner.opt[OPT_SHADOW].desc  = Sane.DESC_SHADOW
  scanner.opt[OPT_SHADOW].type  = Sane.TYPE_FIXED
  scanner.opt[OPT_SHADOW].unit  = Sane.UNIT_PERCENT
  scanner.opt[OPT_SHADOW].constraint_type = Sane.CONSTRAINT_RANGE
  scanner.opt[OPT_SHADOW].constraint.range = &percentage_range_100
  scanner.val[OPT_SHADOW].w     = 0

  scanner.opt[OPT_SHADOW_R].name  = Sane.NAME_SHADOW_R
  scanner.opt[OPT_SHADOW_R].title = Sane.TITLE_SHADOW_R
  scanner.opt[OPT_SHADOW_R].desc  = Sane.DESC_SHADOW_R
  scanner.opt[OPT_SHADOW_R].type  = Sane.TYPE_FIXED
  scanner.opt[OPT_SHADOW_R].unit  = Sane.UNIT_PERCENT
  scanner.opt[OPT_SHADOW_R].constraint_type = Sane.CONSTRAINT_RANGE
  scanner.opt[OPT_SHADOW_R].constraint.range = &percentage_range_100
  scanner.val[OPT_SHADOW_R].w     = 0

  scanner.opt[OPT_SHADOW_G].name  = Sane.NAME_SHADOW_G
  scanner.opt[OPT_SHADOW_G].title = Sane.TITLE_SHADOW_G
  scanner.opt[OPT_SHADOW_G].desc  = Sane.DESC_SHADOW_G
  scanner.opt[OPT_SHADOW_G].type  = Sane.TYPE_FIXED
  scanner.opt[OPT_SHADOW_G].unit  = Sane.UNIT_PERCENT
  scanner.opt[OPT_SHADOW_G].constraint_type = Sane.CONSTRAINT_RANGE
  scanner.opt[OPT_SHADOW_G].constraint.range = &percentage_range_100
  scanner.val[OPT_SHADOW_G].w     = 0

  scanner.opt[OPT_SHADOW_B].name  = Sane.NAME_SHADOW_B
  scanner.opt[OPT_SHADOW_B].title = Sane.TITLE_SHADOW_B
  scanner.opt[OPT_SHADOW_B].desc  = Sane.DESC_SHADOW_B
  scanner.opt[OPT_SHADOW_B].type  = Sane.TYPE_FIXED
  scanner.opt[OPT_SHADOW_B].unit  = Sane.UNIT_PERCENT
  scanner.opt[OPT_SHADOW_B].constraint_type = Sane.CONSTRAINT_RANGE
  ...
  /* analog-gamma */
  scanner.opt[OPT_ANALOG_GAMMA].name  = Sane.NAME_ANALOG_GAMMA
  scanner.opt[OPT_ANALOG_GAMMA].title = Sane.TITLE_ANALOG_GAMMA
  scanner.opt[OPT_ANALOG_GAMMA].desc  = Sane.DESC_ANALOG_GAMMA
  scanner.opt[OPT_ANALOG_GAMMA].type  = Sane.TYPE_FIXED
  scanner.opt[OPT_ANALOG_GAMMA].unit  = Sane.UNIT_NONE
  scanner.opt[OPT_ANALOG_GAMMA].constraint_type  = Sane.CONSTRAINT_RANGE
  scanner.opt[OPT_ANALOG_GAMMA].constraint.range = &(scanner.device.analog_gamma_range)
  scanner.val[OPT_ANALOG_GAMMA].w     = 1 << Sane.FIXED_SCALE_SHIFT

  scanner.opt[OPT_ANALOG_GAMMA_R].name  = Sane.NAME_ANALOG_GAMMA_R
  scanner.opt[OPT_ANALOG_GAMMA_R].title = Sane.TITLE_ANALOG_GAMMA_R
  scanner.opt[OPT_ANALOG_GAMMA_R].desc  = Sane.DESC_ANALOG_GAMMA_R
  scanner.opt[OPT_ANALOG_GAMMA_R].type  = Sane.TYPE_FIXED
  scanner.opt[OPT_ANALOG_GAMMA_R].unit  = Sane.UNIT_NONE
  scanner.opt[OPT_ANALOG_GAMMA_R].constraint_type  = Sane.CONSTRAINT_RANGE
  scanner.opt[OPT_ANALOG_GAMMA_R].constraint.range = &(scanner.device.analog_gamma_range)
  scanner.val[OPT_ANALOG_GAMMA_R].w     = 1 << Sane.FIXED_SCALE_SHIFT

  scanner.opt[OPT_ANALOG_GAMMA_G].name  = Sane.NAME_ANALOG_GAMMA_G
  scanner.opt[OPT_ANALOG_GAMMA_G].title = Sane.TITLE_ANALOG_GAMMA_G
  scanner.opt[OPT_ANALOG_GAMMA_G].desc  = Sane.DESC_ANALOG_GAMMA_G
  scanner.opt[OPT_ANALOG_GAMMA_G].type  = Sane.TYPE_FIXED
  scanner.opt[OPT_ANALOG_GAMMA_G].unit  = Sane.UNIT_NONE
  scanner.opt[OPT_ANALOG_GAMMA_G].constraint_type  = Sane.CONSTRAINT_RANGE
  scanner.opt[OPT_ANALOG_GAMMA_G].constraint.range = &(scanner.device.analog_gamma_range)
  scanner.val[OPT_ANALOG_GAMMA_G].w     = 1 << Sane.FIXED_SCALE_SHIFT

  scanner.opt[OPT_ANALOG_GAMMA_B].name  = Sane.NAME_ANALOG_GAMMA_B
  scanner.opt[OPT_ANALOG_GAMMA_B].title = Sane.TITLE_ANALOG_GAMMA_B
  scanner.opt[OPT_ANALOG_GAMMA_B].desc  = Sane.DESC_ANALOG_GAMMA_B
  scanner.opt[OPT_ANALOG_GAMMA_B].type  = Sane.TYPE_FIXED
  scanner.opt[OPT_ANALOG_GAMMA_B].unit  = Sane.UNIT_NONE
  scanner.opt[OPT_ANALOG_GAMMA_B].constraint_type  = Sane.CONSTRAINT_RANGE
  scanner.opt[OPT_ANALOG_GAMMA_B].constraint.range = &(scanner.device.analog_gamma_range)
  scanner.val[OPT_ANALOG_GAMMA_B].w     = 1 << Sane.FIXED_SCALE_SHIFT


  /* custom-gamma table */
  scanner.opt[OPT_CUSTOM_GAMMA].name  = Sane.NAME_CUSTOM_GAMMA
  scanner.opt[OPT_CUSTOM_GAMMA].title = Sane.TITLE_CUSTOM_GAMMA
  scanner.opt[OPT_CUSTOM_GAMMA].desc  = Sane.DESC_CUSTOM_GAMMA
  scanner.opt[OPT_CUSTOM_GAMMA].type  = Sane.TYPE_BOOL
  scanner.val[OPT_CUSTOM_GAMMA].w     = Sane.TRUE
  ...
  scanner.opt[OPT_GAMMA_VECTOR].size  = scanner.gamma_length * sizeof(Sane.Word)
  ...
  scanner.opt[OPT_GAMMA_VECTOR_R].size  = scanner.gamma_length * sizeof(Sane.Word)
  ...
  scanner.opt[OPT_GAMMA_VECTOR_G].size  = scanner.gamma_length * sizeof(Sane.Word)
  ...
  scanner.opt[OPT_GAMMA_VECTOR_B].size  = scanner.gamma_length * sizeof(Sane.Word)

  /* halftone dimension */
  scanner.opt[OPT_HALFTONE_DIMENSION].name  = Sane.NAME_HALFTONE_DIMENSION
  scanner.opt[OPT_HALFTONE_DIMENSION].title = Sane.TITLE_HALFTONE_DIMENSION
  scanner.opt[OPT_HALFTONE_DIMENSION].desc  = Sane.DESC_HALFTONE_DIMENSION
  scanner.opt[OPT_HALFTONE_DIMENSION].type  = Sane.TYPE_INT
  scanner.opt[OPT_HALFTONE_DIMENSION].unit  = Sane.UNIT_PIXEL
  scanner.opt[OPT_HALFTONE_DIMENSION].constraint_type = Sane.CONSTRAINT_WORD_LIST
  ...
  scanner.opt[OPT_HALFTONE_PATTERN].type  = Sane.TYPE_INT
  scanner.opt[OPT_HALFTONE_PATTERN].size  = 0
  scanner.opt[OPT_HALFTONE_PATTERN].constraint_type = Sane.CONSTRAINT_RANGE
  ...
  /* "Advanced" group: */
  scanner.opt[OPT_ADVANCED_GROUP].title = Sane.I18N("Advanced")
  ...
  scanner.opt[OPT_SELECT_EXPOSURE_TIME].name  = Sane.NAME_SELECT_EXPOSURE_TIME
  scanner.opt[OPT_SELECT_EXPOSURE_TIME].title = Sane.TITLE_SELECT_EXPOSURE_TIME
  scanner.opt[OPT_SELECT_EXPOSURE_TIME].desc  = Sane.DESC_SELECT_EXPOSURE_TIME
  scanner.opt[OPT_SELECT_EXPOSURE_TIME].type  = Sane.TYPE_BOOL
  scanner.val[OPT_SELECT_EXPOSURE_TIME].w     = Sane.FALSE

  /* select calibration exposure time */
  scanner.opt[OPT_SELECT_CAL_EXPOSURE_TIME].name  = Sane.UMAX_NAME_SELECT_CALIBRATON_EXPOSURE_TIME
  scanner.opt[OPT_SELECT_CAL_EXPOSURE_TIME].title = Sane.UMAX_TITLE_SELECT_CALIBRATION_EXPOSURE_TIME
  scanner.opt[OPT_SELECT_CAL_EXPOSURE_TIME].desc  = Sane.UMAX_DESC_SELECT_CALIBRATION_EXPOSURE_TIME
  scanner.opt[OPT_SELECT_CAL_EXPOSURE_TIME].type  = Sane.TYPE_BOOL
  scanner.val[OPT_SELECT_CAL_EXPOSURE_TIME].w     = Sane.FALSE
  scanner.opt[OPT_SELECT_CAL_EXPOSURE_TIME].cap  |= Sane.CAP_INACTIVE

  /* calibration exposure time */
  scanner.opt[OPT_CAL_EXPOS_TIME].name  = Sane.NAME_CAL_EXPOS_TIME
  scanner.opt[OPT_CAL_EXPOS_TIME].title = Sane.TITLE_CAL_EXPOS_TIME
  scanner.opt[OPT_CAL_EXPOS_TIME].desc  = Sane.DESC_CAL_EXPOS_TIME
  scanner.opt[OPT_CAL_EXPOS_TIME].type  = Sane.TYPE_INT
  scanner.opt[OPT_CAL_EXPOS_TIME].unit  = Sane.UNIT_MICROSECOND
  scanner.opt[OPT_CAL_EXPOS_TIME].constraint_type = Sane.CONSTRAINT_RANGE
  ...
  scanner.opt[OPT_CAL_EXPOS_TIME_R].name  = Sane.NAME_CAL_EXPOS_TIME_R
  scanner.opt[OPT_CAL_EXPOS_TIME_R].title = Sane.TITLE_CAL_EXPOS_TIME_R
  scanner.opt[OPT_CAL_EXPOS_TIME_R].desc  = Sane.DESC_CAL_EXPOS_TIME_R
  scanner.opt[OPT_CAL_EXPOS_TIME_R].type  = Sane.TYPE_INT
  scanner.opt[OPT_CAL_EXPOS_TIME_R].unit  = Sane.UNIT_MICROSECOND
  scanner.opt[OPT_CAL_EXPOS_TIME_R].constraint_type = Sane.CONSTRAINT_RANGE
  ...
  scanner.opt[OPT_CAL_EXPOS_TIME_G].name  = Sane.NAME_CAL_EXPOS_TIME_G
  scanner.opt[OPT_CAL_EXPOS_TIME_G].title = Sane.TITLE_CAL_EXPOS_TIME_G
  scanner.opt[OPT_CAL_EXPOS_TIME_G].desc  = Sane.DESC_CAL_EXPOS_TIME_G
  scanner.opt[OPT_CAL_EXPOS_TIME_G].type  = Sane.TYPE_INT
  scanner.opt[OPT_CAL_EXPOS_TIME_G].unit  = Sane.UNIT_MICROSECOND
  scanner.opt[OPT_CAL_EXPOS_TIME_G].constraint_type = Sane.CONSTRAINT_RANGE
  ...
  scanner.opt[OPT_CAL_EXPOS_TIME_B].name  = Sane.NAME_CAL_EXPOS_TIME_B
  scanner.opt[OPT_CAL_EXPOS_TIME_B].title = Sane.TITLE_CAL_EXPOS_TIME_B
  scanner.opt[OPT_CAL_EXPOS_TIME_B].desc  = Sane.DESC_CAL_EXPOS_TIME_B
  scanner.opt[OPT_CAL_EXPOS_TIME_B].type  = Sane.TYPE_INT
  scanner.opt[OPT_CAL_EXPOS_TIME_B].unit  = Sane.UNIT_MICROSECOND
  scanner.opt[OPT_CAL_EXPOS_TIME_B].constraint_type = Sane.CONSTRAINT_RANGE
  ...
  /* scan exposure time */
  scanner.opt[OPT_SCAN_EXPOS_TIME].name  = Sane.NAME_SCAN_EXPOS_TIME
  scanner.opt[OPT_SCAN_EXPOS_TIME].title = Sane.TITLE_SCAN_EXPOS_TIME
  scanner.opt[OPT_SCAN_EXPOS_TIME].desc  = Sane.DESC_SCAN_EXPOS_TIME
  scanner.opt[OPT_SCAN_EXPOS_TIME].type  = Sane.TYPE_INT
  scanner.opt[OPT_SCAN_EXPOS_TIME].unit  = Sane.UNIT_MICROSECOND
  scanner.opt[OPT_SCAN_EXPOS_TIME].constraint_type = Sane.CONSTRAINT_RANGE
  ...
  scanner.opt[OPT_SCAN_EXPOS_TIME_R].name  = Sane.NAME_SCAN_EXPOS_TIME_R
  scanner.opt[OPT_SCAN_EXPOS_TIME_R].title = Sane.TITLE_SCAN_EXPOS_TIME_R
  scanner.opt[OPT_SCAN_EXPOS_TIME_R].desc  = Sane.DESC_SCAN_EXPOS_TIME_R
  scanner.opt[OPT_SCAN_EXPOS_TIME_R].type  = Sane.TYPE_INT
  scanner.opt[OPT_SCAN_EXPOS_TIME_R].unit  = Sane.UNIT_MICROSECOND
  scanner.opt[OPT_SCAN_EXPOS_TIME_R].constraint_type = Sane.CONSTRAINT_RANGE
  ...
  scanner.opt[OPT_SCAN_EXPOS_TIME_G].name  = Sane.NAME_SCAN_EXPOS_TIME_G
  scanner.opt[OPT_SCAN_EXPOS_TIME_G].title = Sane.TITLE_SCAN_EXPOS_TIME_G
  scanner.opt[OPT_SCAN_EXPOS_TIME_G].desc  = Sane.DESC_SCAN_EXPOS_TIME_G
  scanner.opt[OPT_SCAN_EXPOS_TIME_G].type  = Sane.TYPE_INT
  scanner.opt[OPT_SCAN_EXPOS_TIME_G].unit  = Sane.UNIT_MICROSECOND
  scanner.opt[OPT_SCAN_EXPOS_TIME_G].constraint_type = Sane.CONSTRAINT_RANGE
  ...
  scanner.opt[OPT_SCAN_EXPOS_TIME_B].name  = Sane.NAME_SCAN_EXPOS_TIME_B
  scanner.opt[OPT_SCAN_EXPOS_TIME_B].title = Sane.TITLE_SCAN_EXPOS_TIME_B
  scanner.opt[OPT_SCAN_EXPOS_TIME_B].desc  = Sane.DESC_SCAN_EXPOS_TIME_B
  scanner.opt[OPT_SCAN_EXPOS_TIME_B].type  = Sane.TYPE_INT
  scanner.opt[OPT_SCAN_EXPOS_TIME_B].unit  = Sane.UNIT_MICROSECOND
  scanner.opt[OPT_SCAN_EXPOS_TIME_B].constraint_type = Sane.CONSTRAINT_RANGE
  ...
  {
    scanner.opt[OPT_SELECT_EXPOSURE_TIME].cap   |= Sane.CAP_INACTIVE
  }
  ...
  scanner.opt[OPT_SELECT_LAMP_DENSITY].name  = Sane.NAME_SELECT_LAMP_DENSITY
  scanner.opt[OPT_SELECT_LAMP_DENSITY].title = Sane.TITLE_SELECT_LAMP_DENSITY
  scanner.opt[OPT_SELECT_LAMP_DENSITY].desc  = Sane.DESC_SELECT_LAMP_DENSITY
  scanner.opt[OPT_SELECT_LAMP_DENSITY].type  = Sane.TYPE_BOOL
  scanner.val[OPT_SELECT_LAMP_DENSITY].w     = Sane.FALSE

  /* calibration lamp density */
  scanner.opt[OPT_CAL_LAMP_DEN].name  = Sane.NAME_CAL_LAMP_DEN
  scanner.opt[OPT_CAL_LAMP_DEN].title = Sane.TITLE_CAL_LAMP_DEN
  scanner.opt[OPT_CAL_LAMP_DEN].desc  = Sane.DESC_CAL_LAMP_DEN
  scanner.opt[OPT_CAL_LAMP_DEN].type  = Sane.TYPE_FIXED
  scanner.opt[OPT_CAL_LAMP_DEN].unit  = Sane.UNIT_PERCENT
  scanner.opt[OPT_CAL_LAMP_DEN].constraint_type = Sane.CONSTRAINT_RANGE
  scanner.opt[OPT_CAL_LAMP_DEN].constraint.range = &percentage_range_100
  scanner.val[OPT_CAL_LAMP_DEN].w     = Sane.FIX(50)
  scanner.opt[OPT_CAL_LAMP_DEN].cap  |= Sane.CAP_INACTIVE

  /* scan lamp density */
  scanner.opt[OPT_SCAN_LAMP_DEN].name  = Sane.NAME_SCAN_LAMP_DEN
  scanner.opt[OPT_SCAN_LAMP_DEN].title = Sane.TITLE_SCAN_LAMP_DEN
  scanner.opt[OPT_SCAN_LAMP_DEN].desc  = Sane.DESC_SCAN_LAMP_DEN
  scanner.opt[OPT_SCAN_LAMP_DEN].type  = Sane.TYPE_FIXED
  scanner.opt[OPT_SCAN_LAMP_DEN].unit  = Sane.UNIT_PERCENT
  scanner.opt[OPT_SCAN_LAMP_DEN].constraint_type = Sane.CONSTRAINT_RANGE
  scanner.opt[OPT_SCAN_LAMP_DEN].constraint.range = &percentage_range_100
  scanner.val[OPT_SCAN_LAMP_DEN].w     = Sane.FIX(50)
  scanner.opt[OPT_SCAN_LAMP_DEN].cap  |= Sane.CAP_INACTIVE

  if (scanner.device.inquiry_lamp_ctrl == 0)
  {
    scanner.opt[OPT_SELECT_LAMP_DENSITY].cap  |= Sane.CAP_INACTIVE
  }
  ...
  scanner.opt[OPT_DISABLE_PRE_FOCUS].title = Sane.I18N("Disable pre focus")
  scanner.opt[OPT_DISABLE_PRE_FOCUS].desc  = Sane.I18N("Do not calibrate focus")
  scanner.opt[OPT_DISABLE_PRE_FOCUS].type  = Sane.TYPE_BOOL
  scanner.val[OPT_DISABLE_PRE_FOCUS].w     = Sane.FALSE

  if (scanner.device.inquiry_manual_focus == 0)
  {
    scanner.opt[OPT_DISABLE_PRE_FOCUS].cap  |= Sane.CAP_INACTIVE
  }

  /* manual pre focus */
  scanner.opt[OPT_MANUAL_PRE_FOCUS].name  = "manual-pre-focus"
  scanner.opt[OPT_MANUAL_PRE_FOCUS].title = Sane.I18N("Manual pre focus")
  scanner.opt[OPT_MANUAL_PRE_FOCUS].desc  = ""
  scanner.opt[OPT_MANUAL_PRE_FOCUS].type  = Sane.TYPE_BOOL
  scanner.val[OPT_MANUAL_PRE_FOCUS].w     = Sane.FALSE

  if (scanner.device.inquiry_manual_focus == 0)
  {
    scanner.opt[OPT_MANUAL_PRE_FOCUS].cap  |= Sane.CAP_INACTIVE
  }

  /* fix focus position */
  scanner.opt[OPT_FIX_FOCUS_POSITION].name  = "fix-focus-position"
  scanner.opt[OPT_FIX_FOCUS_POSITION].title = Sane.I18N("Fix focus position")
  scanner.opt[OPT_FIX_FOCUS_POSITION].desc  = ""
  scanner.opt[OPT_FIX_FOCUS_POSITION].type  = Sane.TYPE_BOOL
  scanner.val[OPT_FIX_FOCUS_POSITION].w     = Sane.FALSE

  if (scanner.device.inquiry_manual_focus == 0)
  {
    scanner.opt[OPT_FIX_FOCUS_POSITION].cap  |= Sane.CAP_INACTIVE
  }

  /* lens calibration in doc position */
  scanner.opt[OPT_LENS_CALIBRATION_DOC_POS].name  = "lens-calibration-in-doc-position"
  scanner.opt[OPT_LENS_CALIBRATION_DOC_POS].title = Sane.I18N("Lens calibration in doc position")
  scanner.opt[OPT_LENS_CALIBRATION_DOC_POS].desc  = Sane.I18N("Calibrate lens focus in document position")
  scanner.opt[OPT_LENS_CALIBRATION_DOC_POS].type  = Sane.TYPE_BOOL
  scanner.val[OPT_LENS_CALIBRATION_DOC_POS].w     = Sane.FALSE

  if (scanner.device.inquiry_lens_cal_in_doc_pos == 0)
  {
    scanner.opt[OPT_LENS_CALIBRATION_DOC_POS].cap  |= Sane.CAP_INACTIVE
  }

  /* 0mm holder focus position */
  scanner.opt[OPT_HOLDER_FOCUS_POS_0MM].name  = "holder-focus-position-0mm"
  scanner.opt[OPT_HOLDER_FOCUS_POS_0MM].title = Sane.I18N("Holder focus position 0mm")
  scanner.opt[OPT_HOLDER_FOCUS_POS_0MM].desc  = Sane.I18N("Use 0mm holder focus position instead of 0.6mm")
  scanner.opt[OPT_HOLDER_FOCUS_POS_0MM].type  = Sane.TYPE_BOOL
  scanner.val[OPT_HOLDER_FOCUS_POS_0MM].w     = Sane.FALSE

  if (scanner.device.inquiry_sel_uta_lens_cal_pos == 0)
  {
    scanner.opt[OPT_HOLDER_FOCUS_POS_0MM].cap  |= Sane.CAP_INACTIVE
  }
  ...
  scanner.opt[OPT_LAMP_ON].title = Sane.I18N("Lamp on")
  scanner.opt[OPT_LAMP_ON].desc  = Sane.I18N("Turn on scanner lamp")
  scanner.opt[OPT_LAMP_ON].type  = Sane.TYPE_BUTTON
  scanner.opt[OPT_LAMP_ON].unit  = Sane.UNIT_NONE
  scanner.opt[OPT_LAMP_ON].size  = 0
  scanner.opt[OPT_LAMP_ON].constraint_type  = Sane.CONSTRAINT_NONE
  scanner.val[OPT_LAMP_ON].w     = 0

  if (!scanner.device.lamp_control_available)		/* lamp state can not be controlled by driver */
  {
    scanner.opt[OPT_LAMP_ON].cap  |= Sane.CAP_INACTIVE
  }
  ...
  scanner.opt[OPT_LAMP_OFF].title = Sane.I18N("Lamp off")
  scanner.opt[OPT_LAMP_OFF].desc  = Sane.I18N("Turn off scanner lamp")
  scanner.opt[OPT_LAMP_OFF].type  = Sane.TYPE_BUTTON
  scanner.opt[OPT_LAMP_OFF].unit  = Sane.UNIT_NONE
  scanner.opt[OPT_LAMP_OFF].size  = 0
  scanner.opt[OPT_LAMP_OFF].constraint_type  = Sane.CONSTRAINT_NONE
  scanner.val[OPT_LAMP_OFF].w     = 0

  if (!scanner.device.lamp_control_available)		/* lamp state can not be controlled by driver */
  {
    scanner.opt[OPT_LAMP_OFF].cap  |= Sane.CAP_INACTIVE
  }
  ...
  scanner.opt[OPT_LAMP_OFF_AT_EXIT].title = Sane.I18N("Lamp off at exit")
  scanner.opt[OPT_LAMP_OFF_AT_EXIT].desc  = Sane.I18N("Turn off lamp when program exits")
  scanner.opt[OPT_LAMP_OFF_AT_EXIT].type  = Sane.TYPE_BOOL
  scanner.val[OPT_LAMP_OFF_AT_EXIT].w     = Sane.FALSE

  if (!scanner.device.lamp_control_available)		/* lamp state can not be controlled by driver */
  {
    scanner.opt[OPT_LAMP_OFF_AT_EXIT].cap  |= Sane.CAP_INACTIVE
  }

  /* ------------------------------ */

  /* batch-scan-start */
  scanner.opt[OPT_BATCH_SCAN_START].name  = Sane.NAME_BATCH_SCAN_START
  scanner.opt[OPT_BATCH_SCAN_START].title = Sane.TITLE_BATCH_SCAN_START
  scanner.opt[OPT_BATCH_SCAN_START].desc  = Sane.DESC_BATCH_SCAN_START
  scanner.opt[OPT_BATCH_SCAN_START].type  = Sane.TYPE_BOOL
  scanner.val[OPT_BATCH_SCAN_START].w     = Sane.FALSE

  /* batch-scan-loop */
  scanner.opt[OPT_BATCH_SCAN_LOOP].name  = Sane.NAME_BATCH_SCAN_LOOP
  scanner.opt[OPT_BATCH_SCAN_LOOP].title = Sane.TITLE_BATCH_SCAN_LOOP
  scanner.opt[OPT_BATCH_SCAN_LOOP].desc  = Sane.DESC_BATCH_SCAN_LOOP
  scanner.opt[OPT_BATCH_SCAN_LOOP].type  = Sane.TYPE_BOOL
  scanner.val[OPT_BATCH_SCAN_LOOP].w     = Sane.FALSE

  /* batch-scan-end */
  scanner.opt[OPT_BATCH_SCAN_END].name  = Sane.NAME_BATCH_SCAN_END
  scanner.opt[OPT_BATCH_SCAN_END].title = Sane.TITLE_BATCH_SCAN_END
  scanner.opt[OPT_BATCH_SCAN_END].desc  = Sane.DESC_BATCH_SCAN_END
  scanner.opt[OPT_BATCH_SCAN_END].type  = Sane.TYPE_BOOL
  scanner.val[OPT_BATCH_SCAN_END].w     = Sane.FALSE

  /* batch-scan-next-y */
  scanner.opt[OPT_BATCH_NEXT_TL_Y].name  = Sane.NAME_BATCH_NEXT_TL_Y
  scanner.opt[OPT_BATCH_NEXT_TL_Y].title = Sane.TITLE_BATCH_NEXT_TL_Y
  scanner.opt[OPT_BATCH_NEXT_TL_Y].desc  = Sane.DESC_BATCH_NEXT_TL_Y
  scanner.opt[OPT_BATCH_NEXT_TL_Y].type  = Sane.TYPE_FIXED
  scanner.opt[OPT_BATCH_NEXT_TL_Y].unit  = Sane.UNIT_MM
  scanner.opt[OPT_BATCH_NEXT_TL_Y].constraint_type = Sane.CONSTRAINT_RANGE
  ...
  {
    scanner.opt[OPT_BATCH_SCAN_START].cap  |= Sane.CAP_INACTIVE
    scanner.opt[OPT_BATCH_SCAN_LOOP].cap   |= Sane.CAP_INACTIVE
    scanner.opt[OPT_BATCH_SCAN_END].cap    |= Sane.CAP_INACTIVE
    scanner.opt[OPT_BATCH_NEXT_TL_Y].cap      |= Sane.CAP_INACTIVE
  }
  ...
  scanner.opt[OPT_CALIB_MODE].title = Sane.I18N("Calibration mode")
  scanner.opt[OPT_CALIB_MODE].desc  = Sane.I18N("Define calibration mode")
  scanner.opt[OPT_CALIB_MODE].type  = Sane.TYPE_STRING
  scanner.opt[OPT_CALIB_MODE].size  = max_string_size(calibration_list)
  scanner.opt[OPT_CALIB_MODE].constraint_type = Sane.CONSTRAINT_STRING_LIST
  scanner.opt[OPT_CALIB_MODE].constraint.string_list = calibration_list
  scanner.val[OPT_CALIB_MODE].s     = (Sane.Char*)strdup(calibration_list[0])

  if (scanner.device.inquiry_calibration == 0)
  {
    scanner.opt[OPT_CALIB_MODE].cap  |= Sane.CAP_INACTIVE
  }
#endif
  ...
  scanner.opt[OPT_PREVIEW].cap   = Sane.CAP_SOFT_DETECT | Sane.CAP_SOFT_SELECT
  scanner.val[OPT_PREVIEW].w     = Sane.FALSE

  Sane.control_option(scanner, OPT_MODE, Sane.ACTION_SET_VALUE,
                      (String *) scan_mode_list[scan_modes], NULL )
...
{
  attach_scanner(name, 0, Sane.UMAX_SCSI)
 return Sane.STATUS_GOOD
}
...
{
  attach_scanner(name, 0, Sane.UMAX_USB)
 return Sane.STATUS_GOOD
}

/* ------------------------------------------------------------ UMAX TEST CONFIGURE OPTION ------------------ */

static Sane.Status umax_test_configure_option(const char *option_str, char *test_name, Int *test_value, Int test_min, Int test_max)
...
Sane.Status Sane.init(Int *version_code, Sane.Auth_Callback authorize)
{
 ...
 FILE *fp
  ...
  /* we have to initialize these global variables here because Sane.init can be called several times */
  num_devices  = 0
  devlist      = NULL
  first_dev    = NULL
  first_handle = NULL
  ...
  if (!fp)
  {
    ...
    attach_scanner("/dev/scanner",    0, Sane.UMAX_SCSI)
#ifdef UMAX_ENABLE_USB
    attach_scanner("/dev/usbscanner", 0, Sane.UMAX_USB)
#endif
   return Sane.STATUS_GOOD
  }
  ...
  while(sanei_config_read(config_line, sizeof(config_line), fp))
  {
    ...
    if (strncmp(config_line, "option", 6) == 0)
    {
      ...
      if      (umax_test_configure_option(option_str, "scsi-maxqueue",                  &umax_scsi_maxqueue, 1, Sane.UMAX_SCSI_MAXQUEUE))
...
Sane.Status Sane.get_devices(const Sane.Device ***device_list, Bool local_only)
{
 ...
 var i: Int
  ...
  DBG(DBG_Sane.init,"Sane.get_devices(local_only = %d)\n", local_only)
...
Sane.Status Sane.open(Sane.String_Const devicename, Sane.Handle *handle)
{
 ...
 unsigned var i: Int, j
  ...
  DBG(DBG_Sane.init,"Sane.open\n")

  if (devicename[0])   /* search for devicename */
  {
    DBG(DBG_Sane.info,"Sane.open: devicename=%s\n", devicename)
  ...
  scanner.gamma_table[0] = (Int *) malloc(scanner.gamma_length * sizeof(Int))
  scanner.gamma_table[1] = (Int *) malloc(scanner.gamma_length * sizeof(Int))
  scanner.gamma_table[2] = (Int *) malloc(scanner.gamma_length * sizeof(Int))
  scanner.gamma_table[3] = (Int *) malloc(scanner.gamma_length * sizeof(Int))
...
void Sane.close(Sane.Handle handle)
{
 Umax_Scanner *prev, *scanner
  ...
  if (!first_handle)
  {
    DBG(DBG_error, "ERROR: Sane.close: no handles opened\n")
    return
  }
  ...
  if (!scanner)
  {
    DBG(DBG_error, "ERROR: Sane.close: invalid handle %p\n", handle)
  ...
  {
    if (scanner.val[OPT_LAMP_OFF_AT_EXIT].w)			      /* turn off scanner lamp on Sane.close */
    {
...
static void umax_set_max_geometry(Umax_Scanner *scanner)
{
  ...
  {
    scanner.device.x_range.min = Sane.FIX(scanner.device.inquiry_dor_x_off * MM_PER_INCH)
    scanner.device.x_range.max = Sane.FIX( (scanner.device.inquiry_dor_x_off + scanner.device.inquiry_dor_width)  * MM_PER_INCH)
    scanner.device.y_range.min = Sane.FIX(scanner.device.inquiry_dor_y_off * MM_PER_INCH)
    scanner.device.y_range.max = Sane.FIX( (scanner.device.inquiry_dor_y_off + scanner.device.inquiry_dor_length) * MM_PER_INCH)

    scanner.device.x_dpi_range.max = Sane.FIX(scanner.device.inquiry_dor_x_res)
    scanner.device.y_dpi_range.max = Sane.FIX(scanner.device.inquiry_dor_y_res)
  }
  else if ( (strcmp(scanner.val[OPT_SOURCE].s, FLB_STR) == 0) || (strcmp(scanner.val[OPT_SOURCE].s, ADF_STR) == 0) )
  {
    scanner.device.x_range.min = 0
    scanner.device.x_range.max = Sane.FIX(scanner.device.inquiry_fb_width  * MM_PER_INCH)
    scanner.device.y_range.min = 0
    scanner.device.y_range.max = Sane.FIX(scanner.device.inquiry_fb_length * MM_PER_INCH)

    scanner.device.x_dpi_range.max = Sane.FIX(scanner.device.inquiry_x_res)
    scanner.device.y_dpi_range.max = Sane.FIX(scanner.device.inquiry_y_res)
  }
  else if (strcmp(scanner.val[OPT_SOURCE].s, UTA_STR) == 0)
  {
    scanner.device.x_range.min = Sane.FIX(scanner.device.inquiry_uta_x_off  * MM_PER_INCH)
    scanner.device.x_range.max = Sane.FIX( (scanner.device.inquiry_uta_x_off + scanner.device.inquiry_uta_width) * MM_PER_INCH)
    scanner.device.y_range.min = Sane.FIX(scanner.device.inquiry_uta_y_off  * MM_PER_INCH)
    scanner.device.y_range.max = Sane.FIX( ( scanner.device.inquiry_uta_y_off + scanner.device.inquiry_uta_length) * MM_PER_INCH)

    scanner.device.x_dpi_range.max = Sane.FIX(scanner.device.inquiry_x_res)
    scanner.device.y_dpi_range.max = Sane.FIX(scanner.device.inquiry_y_res)
  }

  DBG(DBG_info,"x_range     = [%f .. %f]\n", Sane.UNFIX(scanner.device.x_range.min), Sane.UNFIX(scanner.device.x_range.max))
  DBG(DBG_info,"y_range     = [%f .. %f]\n", Sane.UNFIX(scanner.device.y_range.min), Sane.UNFIX(scanner.device.y_range.max))
  DBG(DBG_info,"x_dpi_range = [1 .. %f]\n", Sane.UNFIX(scanner.device.x_dpi_range.max))
  DBG(DBG_info,"y_dpi_range = [1 .. %f]\n", Sane.UNFIX(scanner.device.y_dpi_range.max))
...
Sane.Status Sane.control_option(Sane.Handle handle, Int option, Sane.Action action,
                                void *val, Int *info)
{
 Umax_Scanner *scanner = handle
 Sane.Status status
 ...
 Sane.String_Const name
  ...
  else if (action == Sane.ACTION_SET_VALUE)
  {
    ...
    switch (option)
    {
      ...
      case OPT_DOR:
      {
        if (scanner.val[option].w != *(Sane.Word *) val)
        {
          scanner.val[option].w = *(Sane.Word *) val; /* update valure for umax_set_max_geometry */
          ...
          DBG(DBG_info,"Sane.control_option: set DOR = %d\n", scanner.val[option].w)
        ...
        if (scanner.val[option].w != *(Sane.Word *) val)
	{
          ...
          { /* don't bind */
            ...
            scanner.opt[OPT_X_RESOLUTION].name  = Sane.NAME_SCAN_RESOLUTION
            scanner.opt[OPT_X_RESOLUTION].desc  = Sane.DESC_SCAN_X_RESOLUTION
          }
          else
          { /* bind */
            scanner.opt[OPT_Y_RESOLUTION].cap |= Sane.CAP_INACTIVE
        ...
        if (scanner.val[option].w != *(Sane.Word *) val)
	{
          ...
          if (scanner.val[OPT_SELECT_EXPOSURE_TIME].w == Sane.FALSE)
	  {
            scanner.opt[OPT_SELECT_CAL_EXPOSURE_TIME].cap |= Sane.CAP_INACTIVE
	  ...
	  else /* exposure time selection active */
	  {
            ...
            if ( (strcmp(scanner.val[OPT_MODE].s, COLOR_STR) != 0) ||
	         (scanner.val[OPT_RGB_BIND].w == Sane.TRUE) ||
        ...
        if (scanner.val[option].w != *(Sane.Word *) val)
	{
      ...
      case OPT_SOURCE:
      {
        ...
        scanner.val[option].s = (Sane.Char *) strdup(val); /* update string for umax_set_max_geometry */

        DBG(DBG_info,"Sane.control_option: set SOURCE = %s\n", (Sane.Char *) val)
        umax_set_max_geometry(scanner)
      ...
      {
        w = *(Sane.Word *) val
        if (w == scanner.val[OPT_CUSTOM_GAMMA].w) { return Sane.STATUS_GOOD; }
      ...
      case OPT_MODE:
      {
       Int halftoning
        ...
        scanner.opt[OPT_BIT_DEPTH].cap          |= Sane.CAP_INACTIVE

        scanner.opt[OPT_CUSTOM_GAMMA].cap       |= Sane.CAP_INACTIVE
        scanner.opt[OPT_GAMMA_VECTOR].cap       |= Sane.CAP_INACTIVE
        scanner.opt[OPT_GAMMA_VECTOR_R].cap     |= Sane.CAP_INACTIVE
        scanner.opt[OPT_GAMMA_VECTOR_G].cap     |= Sane.CAP_INACTIVE
        scanner.opt[OPT_GAMMA_VECTOR_B].cap     |= Sane.CAP_INACTIVE
        ...
        scanner.opt[OPT_RGB_BIND].cap           |= Sane.CAP_INACTIVE
        ...
        scanner.opt[OPT_HALFTONE_DIMENSION].cap |= Sane.CAP_INACTIVE
...
/* disable halftone pattern download options */
            ...
            if (scanner.val[OPT_SELECT_EXPOSURE_TIME].w == Sane.TRUE)
            {
              scanner.opt[OPT_CAL_EXPOS_TIME].cap   &= ~Sane.CAP_INACTIVE
              scanner.opt[OPT_SCAN_EXPOS_TIME].cap  &= ~Sane.CAP_INACTIVE
            }
	  ...
	  {										    /* lineart modes */
        ...
        {								   /* multi-bit modes(gray or color) */
          ...
          {
            scanner.val[OPT_CUSTOM_GAMMA].w = Sane.FALSE
          }
      ...
      case OPT_HALFTONE_DIMENSION: /* halftone pattern dimension affects halftone pattern option: */
      {
        unsigned dim = *(Sane.Word *) val
         ...
         {
           scanner.opt[OPT_HALFTONE_PATTERN].cap &= ~Sane.CAP_INACTIVE
           scanner.opt[OPT_HALFTONE_PATTERN].size = dim * sizeof (Sane.Word)
         }
        return Sane.STATUS_GOOD
      }
...
Sane.Status Sane.get_parameters(Sane.Handle handle, Sane.Parameters *params)
{
 ...
 const char *mode
  ...
  {								  /* not scanning, so lets use recent values */
    ...
    width  = Sane.UNFIX(scanner.val[OPT_BR_X].w - scanner.val[OPT_TL_X].w)
    length = Sane.UNFIX(scanner.val[OPT_BR_Y].w - scanner.val[OPT_TL_Y].w)
    x_dpi  = Sane.UNFIX(scanner.val[OPT_X_RESOLUTION].w)
    y_dpi  = Sane.UNFIX(scanner.val[OPT_Y_RESOLUTION].w)

    if ( (scanner.val[OPT_RESOLUTION_BIND].w == Sane.TRUE) || (scanner.val[OPT_PREVIEW].w == Sane.TRUE) )
    {
      y_dpi = x_dpi
    }
  ...
  scanner.params.last_frame = (scanner.params.format != Sane.FRAME_RED && scanner.params.format != Sane.FRAME_GREEN)
...
Sane.Status Sane.start(Sane.Handle handle)
{
 ...
 Int fds[2]
  ...
  if (scanner.device.sfd == -1)   /* first call, don`t run this routine again on multi frame or multi image scan */
  {
    ...
    scanner.device.batch_next_tl_y   = Sane.UNFIX(scanner.val[OPT_BATCH_NEXT_TL_Y].w) * scanner.device.y_coordinate_base / MM_PER_INCH

    if (scanner.val[OPT_BATCH_NEXT_TL_Y].w == 0xFFFF)  /* option not set: use br_y => scanhead stops at end of batch area */
    {
      scanner.device.batch_next_tl_y = Sane.UNFIX(scanner.val[OPT_BR_Y].w) * scanner.device.y_coordinate_base / MM_PER_INCH
    }
    ...
    scanner.device.analog_gamma_b = umax_calculate_analog_gamma(Sane.UNFIX(scanner.val[OPT_ANALOG_GAMMA].w))
    ...
    if (scanner.val[OPT_SELECT_LAMP_DENSITY].w == Sane.TRUE)
    {
    ...
    else if (strcmp(mode, COLOR_STR) == 0)
    {
      ...
      {
        scanner.device.analog_gamma_r =
                 umax_calculate_analog_gamma( Sane.UNFIX(scanner.val[OPT_ANALOG_GAMMA_R].w) )
        scanner.device.analog_gamma_g =
                 umax_calculate_analog_gamma( Sane.UNFIX(scanner.val[OPT_ANALOG_GAMMA_G].w) )
        scanner.device.analog_gamma_b =
                 umax_calculate_analog_gamma( Sane.UNFIX(scanner.val[OPT_ANALOG_GAMMA_B].w) )
        ...
        if ((scanner.val[OPT_SELECT_EXPOSURE_TIME].w == Sane.TRUE) && (!scanner.device.exposure_time_rgb_bind))
        {
          if (scanner.val[OPT_SELECT_CAL_EXPOSURE_TIME].w) /* separate calibration exposure time */
          {
    ...
    if (scanner.device.force_preview_bit_rgb != 0)          /* in RGB-mode set preview bit, eg. for UMAX S6E */
    {
      ...
      {
        DBG(DBG_Sane.info,"setting preview bit = 1 (option force-preview-bit-rgb)\n")
        scanner.device.preview = Sane.TRUE
      }
    }
    ...
    scanner.device.x_resolution = Sane.UNFIX(scanner.val[OPT_X_RESOLUTION].w)
    scanner.device.y_resolution = Sane.UNFIX(scanner.val[OPT_Y_RESOLUTION].w)
...
#if 0
    scanner.device.upper_left_x = ((Int) (Sane.UNFIX(scanner.val[OPT_TL_X].w) * xbasedots)) & 65534
    scanner.device.upper_left_y = ((Int) (Sane.UNFIX(scanner.val[OPT_TL_Y].w) * ybasedots)) & 65534

    scanner.device.scanwidth    = ((Int)((Sane.UNFIX(scanner.val[OPT_BR_X].w - scanner.val[OPT_TL_X].w)) * xbasedots)) & 65534
    scanner.device.scanlength   = ((Int)((Sane.UNFIX(scanner.val[OPT_BR_Y].w - scanner.val[OPT_TL_Y].w)) * ybasedots)) & 65534
#endif

    scanner.device.upper_left_x = (Int) (Sane.UNFIX(scanner.val[OPT_TL_X].w) * xbasedots)
    scanner.device.upper_left_y = (Int) (Sane.UNFIX(scanner.val[OPT_TL_Y].w) * ybasedots)

    scanner.device.scanwidth    = (Int)((Sane.UNFIX(scanner.val[OPT_BR_X].w - scanner.val[OPT_TL_X].w)) * xbasedots)
    scanner.device.scanlength   = (Int)((Sane.UNFIX(scanner.val[OPT_BR_Y].w - scanner.val[OPT_TL_Y].w)) * ybasedots)
    ...
    DBG(DBG_Sane.info,"threshold  (lineart)    = %d\n", scanner.device.threshold)
    DBG(DBG_Sane.info,"brightness (halftone)   = %d\n", scanner.device.brightness)
    DBG(DBG_Sane.info,"contrast   (halftone)   = %d\n", scanner.device.contrast)

    DBG(DBG_Sane.info,"analog_gamma            = %d %d %d\n",
            scanner.device.analog_gamma_r,
            scanner.device.analog_gamma_g,
            scanner.device.analog_gamma_b)
    DBG(DBG_Sane.info,"highlight               = %d %d %d\n",
            scanner.device.highlight_r,
            scanner.device.highlight_g,
            scanner.device.highlight_b)
    DBG(DBG_Sane.info,"shadow                  = %d %d %d\n",
            scanner.device.shadow_r,
            scanner.device.shadow_g,
            scanner.device.shadow_b)
    DBG(DBG_Sane.info,"calibrat. exposure time = %d %d %d\n",
            scanner.device.exposure_time_calibration_r,
            scanner.device.exposure_time_calibration_g,
            scanner.device.exposure_time_calibration_b)
    DBG(DBG_Sane.info,"scan exposure time      = %d %d %d\n",
            scanner.device.exposure_time_scan_r,
            scanner.device.exposure_time_scan_g,
            scanner.device.exposure_time_scan_b)

#ifdef UMAX_CALIBRATION_MODE_SELECTABLE
    DBG(DBG_Sane.info,"calibration             = %s\n", scanner.val[OPT_CALIB_MODE].s)
#endif
    DBG(DBG_Sane.info,"calibration mode number = %d\n", scanner.device.calibration)

    DBG(DBG_Sane.info,"batch scan              = %d\n", scanner.device.batch_scan)
    DBG(DBG_Sane.info,"batch end               = %d\n", scanner.device.batch_end)
    DBG(DBG_Sane.info,"batch next top left y   = %d\n", scanner.device.batch_next_tl_y)
    DBG(DBG_Sane.info,"quality calibration     = %d\n", scanner.device.quality)
    DBG(DBG_Sane.info,"warm up                 = %d\n", scanner.device.warmup)
    DBG(DBG_Sane.info,"fast preview function   = %d\n", scanner.device.preview)
    DBG(DBG_Sane.info,"DOR                     = %d\n", scanner.device.dor)
    DBG(DBG_Sane.info,"ADF                     = %d\n", scanner.device.adf)
    DBG(DBG_Sane.info,"manual focus            = %d\n", scanner.device.manual_focus)
    DBG(DBG_Sane.info,"fix focus position      = %d\n", scanner.device.fix_focus_position)
    DBG(DBG_Sane.info,"disable pre focus       = %d\n", scanner.device.disable_pre_focus)
    DBG(DBG_Sane.info,"lens cal in doc pos     = %d\n", scanner.device.lens_cal_in_doc_pos)
    DBG(DBG_Sane.info,"holder focus pos 0mm    = %d\n", scanner.device.holder_focus_pos_0mm)
...
#ifdef HAVE_SANEI_SCSI_OPEN_EXTENDED
    {
     unsigned Int scsi_bufsize  = 0
      ...
      {
        DBG(DBG_error, "ERROR: Sane.start: umax_scsi_open_extended returned too small scsi buffer\n")
        umax_scsi_close((scanner.device))
        scanner.scanning = Sane.FALSE
        return Sane.STATUS_NO_MEM
      }
      DBG(DBG_info, "Sane.start: umax_scsi_open_extended returned scsi buffer size = %d\n", scsi_bufsize)

      if (scsi_bufsize < scanner.device.width_in_pixels) /* print warning when buffer is smaller than one scanline */
      {
        DBG(DBG_warning, "WARNING: Sane.start: scsi buffer is smaller than one scanline\n")
      }

      if (scsi_bufsize != scanner.device.bufsize)
      {
        DBG(DBG_info, "Sane.start: buffer size has changed, reallocating buffer\n")

        if (scanner.device.buffer[0])
        {
          DBG(DBG_info, "Sane.start: freeing SCSI buffer[0]\n")
          free(scanner.device.buffer[0]);									     /* free buffer */
        }

        scanner.device.bufsize = scsi_bufsize

        DBG(DBG_info, "Sane.start: allocating SCSI buffer[0]\n")
        scanner.device.buffer[0]  = malloc(scanner.device.bufsize);					  /* allocate buffer */

        if (!scanner.device.buffer[0]) /* malloc failed */
        {
          DBG(DBG_error, "ERROR: Sane.start: could not allocate buffer[0]\n")
          umax_scsi_close(scanner.device)
          scanner.device.bufsize = 0
          scanner.scanning = Sane.FALSE
         return Sane.STATUS_NO_MEM
    ...
    {
      scanner.scanning = Sane.FALSE
      DBG(DBG_error, "ERROR: Sane.start: open of %s failed:\n", scanner.device.sane.name)
      return Sane.STATUS_INVAL
    }
...
/* halftone pattern download is not ready in this version */
  ...
  /* send gammacurves */
  if (scanner.val[OPT_CUSTOM_GAMMA].w == Sane.TRUE)
  {
    if (strcmp(mode, COLOR_STR) == 0)
    {
      if (scanner.device.three_pass == 0)					      /* one pass color scan */
      {
       unsigned var i: Int, dest, color, value
       char *gamma
        ...
        else
        {
          ...
          DBG(DBG_Sane.info,"sending 3 * %d bytes of gamma data for RGB\n",
      ...
      else									    /* three pass color scan */
      {
       ...
       char *gamma
        ...
        else
        {
          ...
          DBG(DBG_Sane.info,"sending %d bytes of gamma data for color %d\n",
    ...
    else if (strcmp(mode, GRAY_STR) == 0) /* grayscale scan */
    {
     ...
     char *gamma
      ...
      else
      {
        ...
        DBG(DBG_Sane.info,"sending %d bytes of gamma data for gray\n",
            scanner.gamma_length * scanner.output_bytes)
  ...
  if (!sanei_thread_is_valid (scanner.reader_pid))
  {
    ...
    umax_give_scanner(scanner.device); /* reposition and release scanner */
    umax_scsi_close(scanner.device)
    return Sane.STATUS_NO_MEM; /* any other reason than no memory possible ? */
  }
...
Sane.Status Sane.read(Sane.Handle handle, Sane.Byte *buf, Int max_len, Int *len)
{
  ...
  *len = nread
  ...
  if (nread == 0) /* EOF */
  {
    ...
    DBG(DBG_Sane.proc,"closing read end of pipe\n")
...
Sane.Status Sane.set_io_mode(Sane.Handle handle, Bool non_blocking)
{
 Umax_Scanner *scanner = handle
  ...
  if (!scanner.scanning) { return Sane.STATUS_INVAL; }
