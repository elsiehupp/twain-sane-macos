...
struct GT68xx_Command_Set
{
  ...
  Sane.Word send_cmd_index;		/**< Send normal command - wIndex */
  Sane.Word recv_res_value;		/**< Receive normal result - wValue */
  Sane.Word recv_res_index;		/**< Receive normal result - wIndex */
  Sane.Word send_small_cmd_value;	/**< Send small command - wValue */
  Sane.Word send_small_cmd_index;	/**< Send small command - wIndex */
  Sane.Word recv_small_res_value;	/**< Receive small result - wValue */
  Sane.Word recv_small_res_index;	/**< Receive small result - wIndex */
   ...
   * should be set to #NULL; the wrapper will return #Sane.STATUS_UNSUPPORTED
   * in this case.
   */
  /*@{ */
   ...
   * - #Sane.TRUE - the external power supply is connected, or the scanner does
   ...
   * - #Sane.TRUE - the transparency adapter is connected.
   * - #Sane.FALSE - the transparency adapter is not connected.
   *
   * @return
   * - #Sane.STATUS_GOOD - transparency adapter status was checked
   ...
   *
   * @param dev Device object.
   * @param fb_lamp #Sane.TRUE turns on the flatbed lamp.
   * @param ta_lamp #Sane.TRUE turns on the transparency adapter lamp.
   *
   * @return
   ...
   * - #Sane.TRUE  - the scanner carriage is still moving.
   * - #Sane.FALSE - the scanner carriage has stopped.
   *
   * @return
   ...
   *
   * @return
   * - #Sane.STATUS_GOOD - the command completed successfully; the movement is
...
struct GT68xx_Model
{
   ...
   *
   * This flag must be set to Sane.TRUE if the structure is dynamically
  ...
  Int optical_xdpi;	/* maximum resolution in x-direction */
  Int optical_ydpi;	/* maximum resolution in y-direction */
  Int base_xdpi;		/* x-resolution used to calculate geometry */
  Int base_ydpi;		/* y-resolution used to calculate geometry */
  Int ydpi_no_backtrack;	/* if ydpi is equal or higher, disable backtracking */
  Bool constant_ydpi;	/* Use base_ydpi for all resolutions        */

  Int xdpi_values[MAX_RESOLUTIONS];	/* possible x resolutions */
  Int ydpi_values[MAX_RESOLUTIONS];	/* possible y resolutions */
  Int bpp_gray_values[MAX_DPI];	/* possible depths in gray mode */
  Int bpp_color_values[MAX_DPI];	/* possible depths in color mode */
  ...
  Sane.Fixed y_offset_calib;	/* Start of white strip in mm */
  Sane.Fixed x_offset_mark;	/* Start of black mark in mm */
  ...
  Int ld_shift_b;		/* blue */
  Int ld_shift_double;	/* distance between two CCD lines of one color
...
struct GT68xx_Device
{
   ...
   * after Sane.init and before Sane.exit */
  Bool missing
...
struct GT68xx_Scan_Parameters
{
  ...
  Bool line_mode;		/**< Use line mode instead of pixel mode */
  Int overscan_lines;	/**< Number of extra scan lines */
  Int ld_shift_r
  Int ld_shift_g
  Int ld_shift_b
  Int ld_shift_double
  Int double_column
  Int pixel_x0;		/**< x start position */
]


#define GT68XX_PACKET_SIZE 64

typedef Sane.Byte GT68xx_Packet[GT68XX_PACKET_SIZE]
 ...
 *
 * @return
 * - #Sane.TRUE  - model successfully found.
 ...
 * - #Sane.TRUE  - the TA is connected.
 * - #Sane.FALSE - the TA is not connected.
 *
 * @return
 * - #Sane.STATUS_GOOD        - success.
 * - #Sane.STATUS_UNSUPPORTED - the scanner does not support TA connection.
 * - #Sane.STATUS_IO_ERROR    - a communication error occurred.
 */
static Sane.Status
 ...
 *
 * @return
 * - #Sane.STATUS_GOOD     - success; the status in @a *moving is valid.
 * - #Sane.STATUS_IO_ERROR - a communication error occurred.
 */
static Sane.Status
 ...
 *
 * @return
 * - #Sane.STATUS_GOOD - success; the movement is started.  Call
 ...
 *
 * @return
 * - #Sane.STATUS_GOOD - success; the value in @a *ready is valid.
 * - #Sane.STATUS_IO_ERROR - a communication error occurred.
 */
static Sane.Status
 ...
 * @return
 * - #Sane.STATUS_GOOD - success.
 * - #Sane.STATUS_UNSUPPORTED - the requested scanning parameters in @a request
 *   are not supported by hardware.
 * - #Sane.STATUS_INVAL - some of the parameters in @a request, or the @a
...
gt68xx_device_new(GT68xx_Device ** dev_return)
{
  ...
  *dev_return = dev
  ...
  dev.manual_selection = Sane.FALSE
