...
#ifdef HAVE_STDINT_H
import stdint            /* available in ISO C99 */
#else
import sys/types
...
typedef struct Avision_Connection {
  ...
  Int usb_dn;		/* USB (libusb or scanner.c) device number */
  enum {
...
typedef struct Avision_Device
{
  ...
  Sane.Range x_range
  Sane.Range y_range
  Sane.Range speed_range

  asic_type inquiry_asic_type
  Bool inquiry_new_protocol

  Bool inquiry_nvram_read
  Bool inquiry_power_save_time

  Bool inquiry_light_box
  Bool inquiry_adf
  Bool inquiry_duplex
  Bool inquiry_duplex_interlaced
  Bool inquiry_paper_length
  Bool inquiry_batch_scan
  Bool inquiry_detect_accessories
  Bool inquiry_needs_calibration
  Bool inquiry_needs_gamma
  Bool inquiry_keeps_gamma
  Bool inquiry_keeps_window
  Bool inquiry_calibration
  Bool inquiry_3x3_matrix
  Bool inquiry_needs_software_colorpack
  Bool inquiry_needs_line_pack
  Bool inquiry_adf_need_mirror
  Bool inquiry_adf_bgr_order
  Bool inquiry_light_detect
  Bool inquiry_light_control
  Bool inquiry_exposure_control

  Int       inquiry_max_shading_target
  Bool inquiry_button_control
  unsigned Int inquiry_buttons
  Bool inquiry_tune_scan_length
  Bool inquiry_background_raster
  ...
  Sane.Range frame_range
  Sane.Word current_frame
  Sane.Word holder_type
...
#define Sane.NAME_FRAME "frame"
#define Sane.TITLE_FRAME Sane.I18N("Number of the frame to scan")
#define Sane.DESC_FRAME  Sane.I18N("Selects the number of the frame to scan")

#define Sane.NAME_DUPLEX "duplex"
#define Sane.TITLE_DUPLEX Sane.I18N("Duplex scan")
#define Sane.DESC_DUPLEX Sane.I18N("Duplex scan provides a scan of the front and back side of the document")
...
typedef enum
{
  Sane.STATUS_LAMP_WARMING = Sane.STATUS_ACCESS_DENIED + 1	/* lamp is warming up */
}
Sane.Avision_Status

/* public API extension */

public Sane.Status ENTRY(media_check) (Sane.Handle handle)
 ...
 * - Sane.exit() : terminate use of backend, free devicename and device-structure
 */
...
static Bool disable_gamma_table = Sane.FALSE

/* disable the calibration */
static Bool disable_calibration = Sane.FALSE
static Bool force_calibration = Sane.FALSE

/* force scanable areas to ISO(DIN) A4/A3 */
static Bool force_a4 = Sane.FALSE
static Bool force_a3 = Sane.FALSE

/* trust ADF-presence flag, even if ADF model is nonzero */
static Bool skip_adf = Sane.FALSE
...
static Bool static_calib_list[3] =
  {
    Sane.FALSE, Sane.FALSE, Sane.FALSE
  ]
...
static const Sane.Range overscan_range =
  {
    Sane.FIX (0), /* minimum */
    Sane.FIX (4), /* maximum */ /* 4mm, measured on AV122, AV220C2, i40 */
    0 /* quantization */
  ]
...
/* The 0x32 is a random guess based on USB logs. Might need a
   ...
   DM152, AV220. */
static const Sane.Range background_range =
  {
    0, /* minimum */
    0x32, /* maximum */
    0 /* quantization */
...
static Sane.Status
constrain_value (Avision_Scanner* s, Int option, void* value,
		 Int* info)
{
...
static void debug_print_params (Int dbg_level, char* func, Sane.Parameters* params)
{
...
{
  Sane.Status status = Sane.STATUS_IO_ERROR; /* default case */
  ...
  switch (sense_key)
    ...
    case 0x0b:
      ...
      status = Sane.STATUS_CANCELLED; /* AV610C2 cancel button */
      break
    default:
...
#define ADDITIONAL_SENSE(asc,ascq,txt)			\
	...
	ADDITIONAL_SENSE (0x80,0x01, "ADF paper jam"; status = Sane.STATUS_JAMMED)
	ADDITIONAL_SENSE (0x80,0x02, "ADF cover open"; status = Sane.STATUS_COVER_OPEN)
	ADDITIONAL_SENSE (0x80,0x03, "ADF chute empty"; status = Sane.STATUS_NO_DOCS)
	ADDITIONAL_SENSE (0x80,0x04, "ADF paper end"; status = Sane.STATUS_EOF)
	...
	ADDITIONAL_SENSE (0x80,0x08, "FW module doesn't match with scanner")
        ...
        /* film scanner */
	ADDITIONAL_SENSE (0x81,0x00, "ADF/MFP front door open"; status = Sane.STATUS_COVER_OPEN)
	ADDITIONAL_SENSE (0x81,0x01, "ADF holder cartridge open"; status = Sane.STATUS_COVER_OPEN)
	ADDITIONAL_SENSE (0x81,0x02, "ADF no film inside"; status = Sane.STATUS_NO_DOCS)
	ADDITIONAL_SENSE (0x81,0x03, "ADF initial load fail")
	ADDITIONAL_SENSE (0x81,0x04, "ADF film end"; status = Sane.STATUS_NO_DOCS)
...
#define valid_status(status,a) (status == Sane.STATUS_GOOD ? a : 0)
...
static Sane.Status avision_open (const char* device_name,
				 Avision_Connection* av_con,
...
static Sane.Status avision_open_extended (const char* device_name,
					  Avision_Connection* av_con,
					  SANEI_SCSI_Sense_Handler sense_handler,
...
static Bool avision_is_open (Avision_Connection* av_con)
{
  if (av_con.connection_type == AV_SCSI) {
...
static Sane.Status avision_cmd (Avision_Connection* av_con,
...
write_usb_cmd:
    ...
    if (status != Sane.STATUS_GOOD || count != cmd_size) {
      DBG (3, "=== Got error %d trying to write, wrote: %ld. ===\n",
           status, (long)count)

      if (status != Sane.STATUS_GOOD) /* == Sane.STATUS_EOF) */ {
    ...
    if (status == Sane.STATUS_GOOD && dst != NULL && *dst_size > 0) {
      out_count = 0
...
static Sane.Status
add_color_mode (Avision_Device* dev, color_mode mode, String name)
{
  var i: Int
...
static color_mode
match_color_mode (Avision_Device* dev, String name)
{
  var i: Int
...
static Sane.Status
add_source_mode (Avision_Device* dev, source_mode mode, String name)
{
  var i: Int
...
static source_mode
match_source_mode (Avision_Device* dev, String name)
{
  var i: Int
...
compute_parameters (Avision_Scanner* s)
{
  ...
  Bool gray_mode = color_mode_is_shaded (s.c_mode)
  ...
  DBG (3, "Sane.compute_parameters:\n")

  DBG (3, "Sane.compute_parameters: boundary %d, gray_mode: %d, \n",
       boundary, gray_mode)
  ...
  if (dev.hw.feature_type & AV_SOFT_SCALE) {
    ...
    DBG (3, "Sane.compute_parameters: soft scale, hw res: %dx%d\n",
	 s.avdimen.hw_xres,
	 s.avdimen.hw_yres)

    if (!s.avdimen.hw_xres || ! s.avdimen.hw_yres) {
      DBG (1, "Sane.compute_parameters: no matching HW res for: %dx%d\n",
	   s.avdimen.xres,
	   s.avdimen.yres)
      return Sane.STATUS_INVAL
    }
  ...
  DBG (3, "Sane.compute_parameters: tlx: %f, tly: %f, brx: %f, bry: %f\n",
  ...
  s.avdimen.tlx = s.avdimen.hw_xres * Sane.UNFIX (s.val[OPT_TL_X].w)
    / MM_PER_INCH
  s.avdimen.tly = s.avdimen.hw_yres * Sane.UNFIX (s.val[OPT_TL_Y].w)
    / MM_PER_INCH
  s.avdimen.brx = s.avdimen.hw_xres * Sane.UNFIX (s.val[OPT_BR_X].w)
    / MM_PER_INCH
  s.avdimen.bry = s.avdimen.hw_yres * Sane.UNFIX (s.val[OPT_BR_Y].w)
    / MM_PER_INCH
  ...
  if (color_mode_is_color (s.c_mode) &&
	...
	DBG (3, "Sane.compute_parameters: y_max: %ld, bry: %ld, line_difference: %d\n",
	     y_max, s.avdimen.bry, s.avdimen.line_difference)

	if (s.avdimen.bry + 2 * s.avdimen.line_difference > y_max) {
	  DBG (1, "Sane.compute_parameters: bry limited!\n")
  ...
  if (dev.inquiry_tune_scan_length && is_adf_scan (s)) {
    ...
    Int overscan = (s.avdimen.hw_yres *
		    (Sane.UNFIX (s.val[OPT_OVERSCAN_TOP].w) +
		     Sane.UNFIX (s.val[OPT_OVERSCAN_BOTTOM].w)) + (MM_PER_INCH - 1)
		    ) / MM_PER_INCH
    DBG (3, "Sane.compute_parameters: overscan lines: %d\n", overscan)
  ...
  if (dev.adf_offset_compensation && s.avdimen.interlaced_duplex) {
    ...
    DBG (1, "Sane.compute_parameters: front offset: top: %d!\n",
         s.avdimen.offset.front.top)
    DBG (1, "Sane.compute_parameters: front offset: bottom: %d!\n",
         s.avdimen.offset.front.bottom)
    DBG (1, "Sane.compute_parameters: rear offset: top: %d!\n",
         s.avdimen.offset.rear.top)
    DBG (1, "Sane.compute_parameters: rear offset: bottom: %d!\n",
  ...
  debug_print_avdimen (1, "Sane.compute_parameters", &s.avdimen)
  ...
  debug_print_params (1, "Sane.compute_parameters", &s.params)
  return Sane.STATUS_GOOD
}
...
inquiry (Avision_Connection av_con, uint8_t* data, size_t len)
{
  ...
  do {
    ...
    status = avision_cmd (&av_con, &inquiry, sizeof (inquiry), 0, 0,
			  data, &size)
    if (status == Sane.STATUS_GOOD && size == inquiry.len)
      break
...
static Sane.Status
get_power_save_time (Avision_Scanner* s, Sane.Word* time)
{
  Sane.Status status
  nvram_data nvram
...
adf_reset (Avision_Scanner* s)
{
  ...
  for (i=1; i >= 0; i--) {
    ...
    if (status != Sane.STATUS_GOOD || size != (4-n)) {
...
static Sane.Status
set_frame (Avision_Scanner* s, Sane.Word frame)
{
  struct {
    struct command_send cmd
    uint8_t data[8]
...
static Sane.Status
attach (Sane.String_Const devname, Avision_ConnectionType con_type,
        Avision_Device** devp)
{
...
send_tune_scan_length (Avision_Scanner* s)
{
  ...
  */
  ...
  top = dpi * Sane.UNFIX (s.val[OPT_OVERSCAN_TOP].w) / MM_PER_INCH
  ...
  bottom = dpi * Sane.UNFIX (s.val[OPT_OVERSCAN_BOTTOM].w) / MM_PER_INCH
...
additional_probe (Avision_Scanner* s)
{
  ...
  if (!dev.inquiry_no_gray_modes)
  {
    if (dev.inquiry_bits_per_channel > 0) {
      add_color_mode (dev, AV_THRESHOLDED, Sane.VALUE_SCAN_MODE_LINEART)
      add_color_mode (dev, AV_DITHERED, "Dithered")
    }

    if (dev.inquiry_bits_per_channel >= 8)
      add_color_mode (dev, AV_GRAYSCALE, Sane.VALUE_SCAN_MODE_GRAY)
  ...
  if (dev.inquiry_channels_per_pixel > 1) {
    add_color_mode (dev, AV_TRUECOLOR, Sane.VALUE_SCAN_MODE_COLOR)
			    ...
			    struct calibration_format* format, uint8_t* data)
{
  ...
  for (i = 0; i < elements_per_line; ++ i)
	...
	++ values_limitted
      ...
      if (static_calib_list [i % 3] == Sane.TRUE)
	result = 0xA000
...
send_gamma (Avision_Scanner* s)
{
  ...
  brightness = Sane.UNFIX (s.val[OPT_BRIGHTNESS].w)
  brightness /= 100
  contrast = Sane.UNFIX (s.val[OPT_CONTRAST].w)
  contrast /= 100
  ...
  for (color = 0; color < 3 && status == Sane.STATUS_GOOD; ++ color)
    {
...
set_window (Avision_Scanner* s)
{
  ...
  if (s.val[OPT_PAPERLEN].w != Sane.FALSE) {
  ...
  if ( !(dev.hw.feature_type & AV_FUJITSU) )
    {
      ...
      if (s.val[OPT_QSCAN].w == Sane.TRUE &&
	...
	SET_BIT (cmd.window.avision.type.normal.bitset2, 4)
      ...
      if (s.val[OPT_QCALIB].w == Sane.FALSE &&
...
get_background_raster (Avision_Scanner* s)
{
  ...
  for (i = 0; i < lines;)
	...
	this_lines = s.val[OPT_BACKGROUND].w
      ...
      if (status != Sane.STATUS_GOOD || read_size != this_read) {
...
flush_media (Avision_Scanner* s)
{
  ...
  if (s.source_mode_dim == AV_ADF_DIM && dev.inquiry_batch_scan)
	...
	if (status == Sane.STATUS_GOOD) {
	  Sane.Status status2 = reserve_unit (s)
...
start_scan (Avision_Scanner* s)
{
  ...
  if (s.val[OPT_PREVIEW].w == Sane.TRUE && s.hw.inquiry_asic_type != AV_ASIC_C7) {
...
do_eof (Avision_Scanner *s)
{
  ...
  s.prepared = s.scanning = Sane.FALSE
  ...
  return (Sane.Status)exit_status
}
...
do_cancel (Avision_Scanner* s)
{
  ...
  s.prepared = s.scanning = Sane.FALSE
  s.duplex_rear_valid = Sane.FALSE
  s.page = 0
...
static Sane.Status
read_data (Avision_Scanner* s, Sane.Byte* buf, size_t* count)
{
  struct command_read rcmd
  Sane.Status status
...
init_options (Avision_Scanner* s)
{
  ...
  dev.speed_range.min = (Int)0
  dev.speed_range.max = (Int)4
  dev.speed_range.quant = (Int)1
  ...
  s.opt[OPT_NUM_OPTS].size = sizeof(Sane.TYPE_INT)
  ...
  dev.x_range.max = Sane.FIX ( (Int)dev.inquiry_x_ranges[s.source_mode_dim])
  dev.x_range.quant = 0
  dev.y_range.max = Sane.FIX ( (Int)dev.inquiry_y_ranges[s.source_mode_dim])
  dev.y_range.quant = 0
  ...
  s.opt[OPT_SPEED].type  = Sane.TYPE_INT
  s.opt[OPT_SPEED].constraint_type  = Sane.CONSTRAINT_RANGE
  ...
  s.opt[OPT_OVERSCAN_TOP].type = Sane.TYPE_FIXED
  s.opt[OPT_OVERSCAN_TOP].unit = Sane.UNIT_MM
  s.opt[OPT_OVERSCAN_TOP].constraint_type = Sane.CONSTRAINT_RANGE
  s.opt[OPT_OVERSCAN_TOP].constraint.range = &overscan_range
  s.val[OPT_OVERSCAN_TOP].w = Sane.FIX(0)
  ...
  s.opt[OPT_OVERSCAN_BOTTOM].type = Sane.TYPE_FIXED
  s.opt[OPT_OVERSCAN_BOTTOM].unit = Sane.UNIT_MM
  s.opt[OPT_OVERSCAN_BOTTOM].constraint_type = Sane.CONSTRAINT_RANGE
  s.opt[OPT_OVERSCAN_BOTTOM].constraint.range = &overscan_range
  s.val[OPT_OVERSCAN_BOTTOM].w = Sane.FIX(0)

  if (!dev.inquiry_tune_scan_length)
    s.opt[OPT_OVERSCAN_TOP].cap |= Sane.CAP_INACTIVE
  if (!dev.inquiry_tune_scan_length)
    s.opt[OPT_OVERSCAN_BOTTOM].cap |= Sane.CAP_INACTIVE
  ...
  s.opt[OPT_BACKGROUND].type = Sane.TYPE_INT
  s.opt[OPT_BACKGROUND].unit = Sane.UNIT_PIXEL
  s.opt[OPT_BACKGROUND].constraint_type = Sane.CONSTRAINT_RANGE
  s.opt[OPT_BACKGROUND].constraint.range = &background_range
  s.val[OPT_BACKGROUND].w = 0

  if (!dev.inquiry_background_raster) {
    s.opt[OPT_BACKGROUND].cap |= Sane.CAP_INACTIVE
  }
  ...
  s.opt[OPT_QSCAN].type   = Sane.TYPE_BOOL
  s.opt[OPT_QSCAN].unit   = Sane.UNIT_NONE
  s.val[OPT_QSCAN].w      = Sane.TRUE
  if (dev.hw.feature_type & AV_NO_QSCAN_MODE)
    s.opt[OPT_QSCAN].cap |= Sane.CAP_INACTIVE

  /* Quality Calibration */
  s.opt[OPT_QCALIB].name  = Sane.NAME_QUALITY_CAL
  s.opt[OPT_QCALIB].title = Sane.TITLE_QUALITY_CAL
  s.opt[OPT_QCALIB].desc  = Sane.DESC_QUALITY_CAL
  s.opt[OPT_QCALIB].type  = Sane.TYPE_BOOL
  s.opt[OPT_QCALIB].unit  = Sane.UNIT_NONE
  s.val[OPT_QCALIB].w     = Sane.TRUE
  if (dev.hw.feature_type & AV_NO_QCALIB_MODE)
    s.opt[OPT_QCALIB].cap |= Sane.CAP_INACTIVE
  ...
  s.opt[OPT_MULTISAMPLE].type  = Sane.TYPE_BOOL
  s.opt[OPT_MULTISAMPLE].unit  = Sane.UNIT_NONE
  s.val[OPT_MULTISAMPLE].w     = Sane.FALSE

  /* TODO: No idea how to detect, assume exposure control devices are
     new enough to support this, for now. -ReneR */
  if (!dev.inquiry_exposure_control) {
    s.opt[OPT_MULTISAMPLE].cap |= Sane.CAP_INACTIVE
  }
  ...
  s.opt[OPT_IR].type  = Sane.TYPE_BOOL
  s.opt[OPT_IR].unit  = Sane.UNIT_NONE
  s.val[OPT_IR].w     = Sane.FALSE

  /* TODO: No idea how to detect, assume exposure control devices are
     new enough to support this, for now. -ReneR */
  if (!dev.inquiry_exposure_control) {
    s.opt[OPT_IR].cap |= Sane.CAP_INACTIVE
  }

  /* "MISC" group: */
  s.opt[OPT_MISC_GROUP].title = Sane.TITLE_SCAN_MODE
  ...
  /* film holder control */
  if (dev.scanner_type != AV_FILM)
    s.opt[OPT_FRAME].cap |= Sane.CAP_INACTIVE
  s.opt[OPT_FRAME].name = Sane.NAME_FRAME
  s.opt[OPT_FRAME].title = Sane.TITLE_FRAME
  s.opt[OPT_FRAME].desc = Sane.DESC_FRAME
  s.opt[OPT_FRAME].type = Sane.TYPE_INT
  s.opt[OPT_FRAME].unit = Sane.UNIT_NONE
  s.opt[OPT_FRAME].constraint_type = Sane.CONSTRAINT_RANGE
  ...
  s.opt[OPT_MESSAGE].type = Sane.TYPE_STRING
  s.opt[OPT_MESSAGE].cap = Sane.CAP_SOFT_DETECT | Sane.CAP_ADVANCED
  s.opt[OPT_MESSAGE].size = 129
  s.opt[OPT_MESSAGE].constraint_type = Sane.CONSTRAINT_NONE
  s.val[OPT_MESSAGE].s = malloc(s.opt[OPT_MESSAGE].size)
  s.val[OPT_MESSAGE].s[0] = 0

  /* NVRAM */
  s.opt[OPT_NVRAM].cap = Sane.CAP_SOFT_DETECT | Sane.CAP_ADVANCED
  ...
  s.opt[OPT_NVRAM].constraint_type = Sane.CONSTRAINT_NONE
  s.val[OPT_NVRAM].s = malloc(s.opt[OPT_NVRAM].size)
  s.val[OPT_NVRAM].s[0] = 0

  /* paper_length */
  s.opt[OPT_PAPERLEN].cap = Sane.CAP_SOFT_SELECT | Sane.CAP_SOFT_DETECT | Sane.CAP_ADVANCED
  if (!dev.inquiry_paper_length)
    s.opt[OPT_PAPERLEN].cap |= Sane.CAP_INACTIVE
  ...
  s.opt[OPT_PAPERLEN].type  = Sane.TYPE_BOOL
  s.opt[OPT_PAPERLEN].unit  = Sane.UNIT_NONE
  s.opt[OPT_PAPERLEN].size = sizeof(Sane.Word)
  s.opt[OPT_PAPERLEN].constraint_type = Sane.CONSTRAINT_NONE
  s.val[OPT_PAPERLEN].w     = Sane.FALSE

  /* ADF page flipping */
  s.opt[OPT_ADF_FLIP].cap = Sane.CAP_SOFT_SELECT | Sane.CAP_SOFT_DETECT | Sane.CAP_AUTOMATIC | Sane.CAP_ADVANCED
  ...
  s.opt[OPT_ADF_FLIP].constraint_type = Sane.CONSTRAINT_NONE
  s.val[OPT_ADF_FLIP].w = Sane.TRUE
...
reader_process (void *data)
{
  ...
  Sane.Status status
  Sane.Status exit_status = Sane.STATUS_GOOD
  ...
  while (exit_status == Sane.STATUS_GOOD && processed_bytes < total_size)
    {
             ...
             exit_status  == Sane.STATUS_GOOD)
	{
	  ...
	  /* only EOF on the second stripe, as otherwise the rear page
	     is shorter */
	  if (status == Sane.STATUS_EOF && deinterlace == STRIPE) {
	    if (dev.inquiry_asic_type > AV_ASIC_C7 && dev.inquiry_asic_type < AV_ASIC_OA980) {
	      this_read = 0
	    } else {
	  ...
	  if (status == Sane.STATUS_EOF || this_read == 0) {
	    DBG (1, "reader_process: read_data failed due to EOF\n")
	    exit_status = Sane.STATUS_EOF
	  }
  ...
  if (exit_status == Sane.STATUS_GOOD)
    exit_status = Sane.STATUS_EOF
  ...
  if ((deinterlace != NONE && s.duplex_rear_valid) ||
	...
	if (get_button_status (s) == Sane.STATUS_CANCELLED)
	  exit_status =  Sane.STATUS_CANCELLED
      }
...
Sane.reload_devices (void)
{
  ...
  if (fp <= (FILE*)0)
    {
      DBG (1, "Sane.reload_devices: No config file present!\n")
    }
  else
    {
      ...
      while (sanei_config_read  (line, sizeof (line), fp))
	{
	  ...
	  DBG (5, "Sane.reload_devices: parsing config line \"%s\"\n",
	       line)

	  cp = sanei_config_get_string (line, &word)

	  if (!word || cp == line) {
	    DBG (5, "Sane.reload_devices: config file line %d: ignoring empty line\n",
		 linenumber)
	    if (word) {
	      free (word)
	      word = NULL
	  ...
	  if (!word) {
	    DBG (1, "Sane.reload_devices: config file line %d: could not be parsed\n",
		 linenumber)
	    continue
	  }
	  ...
	  if (strcmp (word, "option") == 0)
		...
		DBG (3, "Sane.reload_devices: config file line %d: disable-gamma-table\n",
		     linenumber)
		disable_gamma_table = Sane.TRUE
	      }
	      else if (strcmp (word, "disable-calibration") == 0) {
		DBG (3, "Sane.reload_devices: config file line %d: disable-calibration\n",
		     linenumber)
		disable_calibration = Sane.TRUE
	      }
	      else if (strcmp (word, "force-calibration") == 0) {
		DBG (3, "Sane.reload_devices: config file line %d: force-calibration\n",
		     linenumber)
		force_calibration = Sane.TRUE
	      }
	      else if (strcmp (word, "force-a4") == 0) {
		DBG (3, "Sane.reload_devices: config file line %d: enabling force-a4\n",
		     linenumber)
		force_a4 = Sane.TRUE
	      }
	      else if (strcmp (word, "force-a3") == 0) {
		DBG (3, "Sane.reload_devices: config file line %d: enabling force-a3\n",
		     linenumber)
		force_a3 = Sane.TRUE
	      }
	      else if (strcmp (word, "skip-adf") == 0) {
		DBG (3, "Sane.reload_devices: config file line %d: enabling skip-adf\n",
		     linenumber)
		skip_adf = Sane.TRUE
	      }
	      else if (strcmp (word, "static-red-calib") == 0) {
		DBG (3, "Sane.reload_devices: config file line %d: static red calibration\n",
		     linenumber)
		static_calib_list [0] = Sane.TRUE
	      }
	      else if (strcmp (word, "static-green-calib") == 0) {
		DBG (3, "Sane.reload_devices: config file line %d: static green calibration\n",
		    linenumber)
		static_calib_list [1] = Sane.TRUE
	      }
	      else if (strcmp (word, "static-blue-calib") == 0) {
		DBG (3, "Sane.reload_devices: config file line %d: static blue calibration\n",
		    linenumber)
		static_calib_list [2] = Sane.TRUE
	      }
	      else
		DBG (1, "Sane.reload_devices: config file line %d: options unknown!\n",
		    linenumber)
	    }
  ...
  while (Avision_Device_List [model_num].scsi_mfg != NULL ||
	...
	{
	  DBG (1, "Sane.reload_devices: Trying to find USB device %.4x %.4x ...\n",
	  ...
	  if (sanei_usb_find_devices (attaching_hw.usb_vendor,
				      attaching_hw.usb_product,
				      attach_one_usb) != Sane.STATUS_GOOD) {
	    DBG (1, "Sane.reload_devices: error during USB device detection!\n")
	  }
	}
      ++ model_num
...
Sane.init (Int* version_code, Sane.Auth_Callback authorize)
{
  ...
  DBG (3, "Sane.init:(Version: %i.%i Build: %i)\n",
       Sane.CURRENT_MAJOR, V_MINOR, BACKEND_BUILD)

  /* must come first */
  sanei_thread_init ()

  if (version_code)
    *version_code = Sane.VERSION_CODE (Sane.CURRENT_MAJOR, V_MINOR, BACKEND_BUILD)
...
Sane.open (Sane.String_Const devicename, Sane.Handle *handle)
{
  ...
  *handle = s
  ...
  if (! avision_is_open (&s.av_con) ) {
    ...
    DBG (1, "Sane.open: using open_extended\n")
    ...
    DBG (1, "Sane.open: got %d scsi_max_request_size\n", dev.scsi_buffer_size)
  }
  ...
  status = inquiry (s.av_con, inquiry_result, sizeof(inquiry_result))
  if (status != Sane.STATUS_GOOD) {
	DBG (1, "Sane.open: awakening inquiry failed: %s\n", Sane.strstatus (status))
	return status
  }

  status = wait_ready (&s.av_con, 1)
  if (status != Sane.STATUS_GOOD) {
    DBG (1, "Sane.open: wait_ready() failed: %s\n", Sane.strstatus (status))
    return status
  }
  ...
  if (status != Sane.STATUS_GOOD) {
    DBG (1, "Sane.open: additional probe failed: %s\n", Sane.strstatus (status))
    return status
  }
  ...
  if (dev.inquiry_duplex_interlaced &&
    ...
    dev.adf_offset_compensation = Sane.TRUE

  if (dev.adf_offset_compensation) {
    strncpy(s.duplex_offtmp_fname, "/tmp/avision-offtmp-XXXXXX", PATH_MAX)

    if (! mktemp(s.duplex_offtmp_fname) ) {
      DBG (1, "Sane.open: failed to generate temporary fname for ADF offset compensation temp file\n")
      return Sane.STATUS_NO_MEM
    }
    else {
      DBG (1, "Sane.open: temporary fname for ADF offset compensation temp file: %s\n",
  ...
  if (dev.inquiry_duplex_interlaced || dev.scanner_type == AV_FILM ||
    ...
    if (! mkstemp(s.duplex_rear_fname) ) {
      DBG (1, "Sane.open: failed to generate temporary fname for duplex scans\n")
      return Sane.STATUS_NO_MEM
    }
    else {
      DBG (1, "Sane.open: temporary fname for duplex scans: %s\n",
	   s.duplex_rear_fname)
    }
  }
  ...
  if (dev.scanner_type == AV_FILM)
    {
      ...
      DBG (1, "Sane.open: early calibration for film scanner.\n")

      compute_parameters (s)

      status = set_window (s)
      if (status != Sane.STATUS_GOOD) {
	DBG (1, "Sane.open: set scan window command failed: %s\n",
	     Sane.strstatus (status))
	return status
      }
      ...
      if (!(dev.hw.feature_type & AV_NO_CALIB))
	{
	  ...
	  if (status != Sane.STATUS_GOOD) {
	    DBG (1, "Sane.open: perform calibration failed: %s\n",
		 Sane.strstatus (status))
	    return status
	  }
	}
...
Sane.get_option_descriptor (Sane.Handle handle, Int option)
{
  ...
  DBG (3, "Sane.get_option_descriptor: %d\n", option)
		     ...
		     Sane.Action action, void* val, Int* info)
{
  ...
  DBG (5, "Sane.control_option: option=%s, action=%s\n",
       s.opt[option].name,
       action == Sane.ACTION_GET_VALUE ? "GET" :
       (action == Sane.ACTION_SET_VALUE ? "SET" :
	(action == Sane.ACTION_SET_AUTO ? "AUTO" : "UNKNOWN") ) )
  ...
  if (action == Sane.ACTION_GET_VALUE)
	...
	case OPT_SOURCE:
	  ...
	  /* set side-effects */
	  dev.x_range.max =
	    Sane.FIX ( dev.inquiry_x_ranges[s.source_mode_dim])
	  dev.y_range.max =
	    Sane.FIX ( dev.inquiry_y_ranges[s.source_mode_dim])
...
Sane.get_parameters (Sane.Handle handle, Sane.Parameters* params)
{
  ...
  /* During an actual scan these parameters will have been computed in
     Sane.start().  Otherwise, the values must be computed on demand.  The
     values cannot be changed during a scan to avoid inconsistency. */
  if (!s.scanning)
    {
      DBG (3, "Sane.get_parameters: computing parameters\n")
      compute_parameters (s)
    }
...
Sane.start (Sane.Handle handle)
{
  ...
  if (s.page > 0 && s.duplex_rear_valid) {
    DBG (1, "Sane.start: virtual duplex rear data valid.\n")
    goto start_scan_end
  }
  ...
  if (is_adf_scan (s)) {
    status = media_check (s)
    if (status != Sane.STATUS_GOOD) {
      DBG (1, "Sane.start: media_check failed: %s\n",
	   Sane.strstatus (status))
      return status
    }
    else
      DBG (1, "Sane.start: media_check ok\n")
  }

  /* Check the light early, to return to the GUI and notify the user. */
  if (s.prepared == Sane.FALSE) {
  ...
  if (s.page > 0 && dev.inquiry_keeps_window) {
     DBG (1, "Sane.start: Optimized set_window away.\n")
  }
  else
  {
    status = set_window (s)
    if (status != Sane.STATUS_GOOD) {
      DBG (1, "Sane.start: set scan window command failed: %s\n",
	   Sane.strstatus (status))
      goto stop_scanner_and_return
    }
    /* Re-check the light, as setting the window may have changed
     * which light is to be turned on. */
    if (s.prepared == Sane.FALSE && dev.inquiry_light_control) {
      status = wait_4_light (s)
      if (status != Sane.STATUS_GOOD) {
	return status
      }
  ...
  if (dev.inquiry_new_protocol)
    {
      ...
      DBG (5, "Sane.start: reading scanner window size\n")

      status = simple_read (s, 0x80, 0, &size, result)

      if (status != Sane.STATUS_GOOD) {
	DBG (1, "Sane.start: get pixel size command failed: %s\n",
	     Sane.strstatus (status))
	goto stop_scanner_and_return
      }
      debug_print_raw (5, "Sane.start: pixel_size:", result, size)
      DBG (5, "Sane.start: x-pixels: %d, y-pixels: %d\n",
  ...
  if (s.page > 0) {
    DBG (1, "Sane.start: optimized calibration away.\n")
    goto calib_end
  }

  /* check whether the user enforces calibration */
  if (force_calibration) {
    DBG (1, "Sane.start: calibration enforced in config!\n")
    goto calib
  }
  ...
  if (!dev.inquiry_new_protocol) {
    DBG (1, "Sane.start: old protocol no calibration needed!\n")
    goto calib_end
  }

  if (!dev.inquiry_needs_calibration) {
    DBG (1, "Sane.start: due to inquiry no calibration needed!\n")
    goto calib_end
  }

  /* calibration allowed for this scanner? */
  if (dev.hw.feature_type & AV_NO_CALIB) {
    DBG (1, "Sane.start: calibration disabled in device list!!\n")
    goto calib_end
  }

  /* Not for film scanners, ... */
  if (dev.scanner_type == AV_FILM) {
    DBG (1, "Sane.start: no calibration for film scanner!\n")
    goto calib_end
  }

  /* check whether calibration is disabled by the user */
  if (disable_calibration) {
    DBG (1, "Sane.start: calibration disabled in config - skipped!\n")
    goto calib_end
  }
 ...
 calib:
  ...
  if (status != Sane.STATUS_GOOD) {
    DBG (1, "Sane.start: perform calibration failed: %s\n",
	 Sane.strstatus (status))
	goto stop_scanner_and_return
  }
 ...
 calib_end:
  ...
  if (disable_gamma_table) {
    DBG (1, "Sane.start: gamma-table disabled in config - skipped!\n")
    goto gamma_end
  }

  if (dev.hw.feature_type & AV_NO_GAMMA) {
    DBG (1, "Sane.start: gamma table skipped due to device-list!!\n")
    goto gamma_end
  }

  if (s.page > 0 && dev.inquiry_keeps_gamma)
    DBG (1, "Sane.start: Optimized send_gamma away.\n")
  else
  {
    status = send_gamma (s)
    if (status != Sane.STATUS_GOOD) {
      DBG (1, "Sane.start: send gamma failed: %s\n",
	   Sane.strstatus (status))
      goto stop_scanner_and_return
    }
  }
 ...
 gamma_end:
  ...
  if (dev.inquiry_tune_scan_length && is_adf_scan (s)) {
    ...
    if (status != Sane.STATUS_GOOD) {
      DBG (1, "Sane.start: tune_scan_length command failed: %s\n",
	   Sane.strstatus (status))
      goto stop_scanner_and_return
    }
  }
  ...
  if (s.page == 0 && dev.inquiry_background_raster) {
    ...
    if (status != Sane.STATUS_GOOD) {
      DBG (1, "Sane.start: get background raster command failed: %s\n",
	   Sane.strstatus (status))
      goto stop_scanner_and_return
    }
  }
  ...
  if (dev.scanner_type == AV_FILM && dev.holder_type == 0xff) {
    DBG (1, "Sane.start: no film holder or APS cassette!\n")
    ...
    if (dev.inquiry_new_protocol) {
      status = object_position (s, AVISION_SCSI_OP_GO_HOME)
      if (status != Sane.STATUS_GOOD)
	DBG (1, "Sane.start: go home failed: %s\n",
	     Sane.strstatus (status))
    }
    goto stop_scanner_and_return
  }
 ...
 start_scan_end:
  ...
  DBG (3, "Sane.start: starting thread\n")
...
Sane.read (Sane.Handle handle, Sane.Byte* buf, Int max_len, Int* len)
{
  ...
  *len = 0
  ...
  DBG (8, "Sane.read: max_len: %d\n", max_len)

  nread = read (s.read_fds, buf, max_len)
  if (nread > 0) {
    DBG (8, "Sane.read: got %ld bytes\n", (long) nread)
  }
  else {
    DBG (3, "Sane.read: got %ld bytes, err: %d %s\n", (long) nread, errno, strerror(errno))
  }
...
Sane.set_io_mode (Sane.Handle handle, Bool non_blocking)
{
  ...
  DBG (3, "Sane.set_io_mode:\n")
  if (!s.scanning) {
    DBG (3, "Sane.set_io_mode: not yet scanning\n")
    return Sane.STATUS_INVAL
  }
...
Sane.get_select_fd (Sane.Handle handle, Int* fd)
{
  ...
  if (!s.scanning) {
    DBG (3, "Sane.get_select_fd: not yet scanning\n")
    return Sane.STATUS_INVAL
  }
