...
- (BOOL) authorizedFileMoveFrom:(NSString *) src to:(NSString *) dst setRoot:(BOOL) setRoot force:(BOOL) force
{
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        FILE * pipe = nil;
        stat = AuthorizationExecuteWithPrivileges ([[auth authorization] authorizationRef], path, kAuthorizationFlagDefaults, (char **) args, &pipe);
...
- (void) savePreferences
{
    if (savedllconf)
    {
        char tmpfilec[17]; strcpy (tmpfilec, "/tmp/sane.XXXXXX"); int fd = mkstemp (tmpfilec);
    ...
    if ([[sanedActive objectValue] boolValue] != sanedwasactive)
    {
        ...
        if (!error)
        {
            char tmpfilec[17]; strcpy (tmpfilec, "/tmp/sane.XXXXXX"); close (mkstemp (tmpfilec));
            NSString * tmpfile = [[NSFileManager defaultManager] stringWithFileSystemRepresentation:tmpfilec length:16];
            ...
            if (sanedwasactive)
            {
                char * const args[] = { "unload", "/Library/LaunchDaemons/org.sane-project.saned.plist", nil };
                FILE * pipe = nil;
            ...
            {
                char * const args[] = { "load", "/Library/LaunchDaemons/org.sane-project.saned.plist", nil };
                FILE * pipe = nil;
                OSStatus stat = AuthorizationExecuteWithPrivileges ([[auth authorization] authorizationRef], "/bin/launchctl", kAuthorizationFlagDefaults, args, &pipe);
                if (stat == errAuthorizationSuccess)
                    while (!feof (pipe))
...
- (IBAction) buttonPressed:(id) sender
{
    ...
    else if (sender == sanedHelp)
    {
        ...
        char tmpfilec[22]; strcpy (tmpfilec, "/tmp/sane.XXXXXX.html"); int fd = mkstemps (tmpfilec, 5);
