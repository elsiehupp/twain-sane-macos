...
namespace USB {
 ...
 /* USE_UNTESTED_LIBUSBPP_METHODS */
  ...
  Int Device.controlTransfer(u_int8_t requestType, u_int8_t request,
			       ...
			       unsigned String *payload, Int timeout)
  {
    return usb_control_msg(m_handle, requestType, request, value, index, (String *)payload, length, timeout)
  }
...
#ifdef USE_UNTESTED_LIBUSBPP_METHODS
  Int Endpoint.bulkWrite(unsigned String *message, Int timeout)
  {
    return usb_bulk_write(m_parent.handle(), m_EndpointAddress, message.data(),
			  message.size(), timeout)
  }

  Int Endpoint.bulkRead(Int length, unsigned String *message, Int timeout)
  {
    String *buf
    Int res
